...
export const signalDmPolicy = createTopLevelChannelDmPolicy({
  ...
  getCurrent: (cfg: CoreBlowConfig) => cfg.channels?.signal?.dmPolicy ?? "pairing",
...
export const signalCompletionNote = {
  ...
  lines: [
    'Link device with: signal-cli link -n "CoreBlow"',
    "Scan QR in Signal -> Linked Devices",
    `Then run: ${formatCliCommand("coreblow gateway call channels.status --params '{\"probe\":true}'")}`,
    `Docs: ${formatDocsLink("/signal", "signal")}`,
  ],
};
