...
function getChannelConfig(cfg: CoreBlowConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: CoreBlowConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: CoreBlowConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: CoreBlowConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL CoreBlow uses to send replies back to Chat.",
      ],
