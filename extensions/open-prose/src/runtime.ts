...
export class OpenProseRuntime {
  [k: string]: any;
  private extension: OpenProseExtension;
  private channels = new Map<string, OpenProseChannelImpl>();
  private running = false;

  constructor(extension: OpenProseExtension) {
  ...
  getChannel(id: string) {
    if (!this.channels.has(id)) {
      this.channels.set(id, new OpenProseChannelImpl(id));
    }
