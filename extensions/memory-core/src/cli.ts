...
import {
  ...
  formatHelpExamples,
  theme,
} from "coreblow/plugin-sdk/memory-core-host-runtime-cli";
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["coreblow memory status", "Show index and provider status."],
          ["coreblow memory status --deep", "Probe embedding provider readiness."],
          ["coreblow memory index --force", "Force a full reindex."],
          ['coreblow memory search "meeting notes"', "Quick search using positional query."],
          [
            'coreblow memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          ["coreblow memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.coreblow.ai/cli/memory")}\n`,
    );
