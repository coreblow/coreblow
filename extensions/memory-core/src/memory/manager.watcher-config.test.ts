...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): CoreBlowConfig {
    const defaults: NonNullable<NonNullable<CoreBlowConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: CoreBlowConfig) {
