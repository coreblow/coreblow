...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another CoreBlow runtime is using the same account. " +
    `Check 'coreblow matrix verify status --verbose --account ${accountId}' and 'coreblow matrix devices list --account ${accountId}'.`
  );
}
