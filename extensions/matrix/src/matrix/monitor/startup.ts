...
export async function runMatrixStartupMaintenance(params: {
  ...
  try {
    ...
    if (deviceHealth.staleCoreBlowDevices.length > 0) {
      params.logger.warn(
        `matrix: stale CoreBlow devices detected for ${params.auth.userId}: ${deviceHealth.staleCoreBlowDevices.map((device) => device.deviceId).join(", ")}. Run 'coreblow matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
