...
describe("createDiscordDraftStream", () => {
  ...
  it("does not re-send identical text twice", async () => {
    const rest = {
      post: vi.fn(async () => ({ id: "m1" })),
      patch: vi.fn(async () => undefined),
      delete: vi.fn(async () => undefined),
  ...
  it("forceNewMessage resets so next update posts a new message", async () => {
    const rest = {
      post: vi.fn(async () => ({ id: "m1" })).mockResolvedValueOnce({ id: "m1" }).mockResolvedValueOnce({ id: "m2" }),
      patch: vi.fn(async () => undefined),
      delete: vi.fn(async () => undefined),
    };
    const stream = createDiscordDraftStream({
  ...
  it("skips empty or whitespace text", async () => {
    const rest = {
      post: vi.fn(async () => ({ id: "m1" })),
      patch: vi.fn(async () => undefined),
      delete: vi.fn(async () => undefined),
