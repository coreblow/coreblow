import { DEFAULT_ACCOUNT_ID } from "coreblow/plugin-sdk/account-id";
import type { DiscordGuildEntry } from "coreblow/plugin-sdk/config-runtime";
import type { CoreBlowConfig } from "coreblow/plugin-sdk/config-runtime";
import { createEnvPatchedAccountSetupAdapter } from "coreblow/plugin-sdk/setup-adapter-runtime";
import type {
  ChannelSetupAdapter,
  ChannelSetupDmPolicy,
  ChannelSetupWizard,
} from "coreblow/plugin-sdk/setup-runtime";
import { createStandardChannelSetupStatus } from "coreblow/plugin-sdk/setup-runtime";
import { formatDocsLink } from "coreblow/plugin-sdk/setup-tools";
import {
...
export function createDiscordSetupWizardBase(handlers: {
  ...
  return {
    ...
    credentials: [
      {
        ...
        inspect: ({ cfg, accountId }: { cfg: CoreBlowConfig; accountId: string }) => {
          const account = inspectDiscordSetupAccount({ cfg, accountId });
          return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: CoreBlowConfig; accountId: string }) =>
        resolveDiscordSetupAccountConfig({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: CoreBlowConfig; accountId: string }) =>
        Object.entries(
      ...
      updatePrompt: ({ cfg, accountId }: { cfg: CoreBlowConfig; accountId: string }) =>
    ...
    disable: (cfg: CoreBlowConfig) => setSetupChannelEnabled(cfg, channel, false),
