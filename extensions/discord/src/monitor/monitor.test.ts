...
vi.mock("coreblow/plugin-sdk/channel-runtime", createChannelRuntimeMock);
...
describe("discord component interactions", () => {
  ...
  const createCfg = (): CoreBlowConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(async () => {
    ...
    resolveStorePathMock.mockClear().mockReturnValue("/tmp/coreblow-sessions-test.json");
  ...
  it("does not fall through to CoreBlow when a plugin Discord interaction already replied", async () => {
    registerDiscordComponentEntries({
      entries: [createButtonEntry({ callbackData: "codex:approve" })],
      modals: [],
    });
  ...
  it("resolves plugin binding approvals without falling through to CoreBlow", async () => {
    registerDiscordComponentEntries({
      entries: [
        createButtonEntry({
