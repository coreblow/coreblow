...
const acpRuntime = await import("coreblow/plugin-sdk/acp-runtime");
...
describe("thread binding lifecycle", () => {
  ...
  it("persists touched activity timestamps across restart when persistence is enabled", async () => {
    ...
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "coreblow-thread-bindings-"));
    ...
    try {
      ...
      if (previousStateDir === undefined) {
        ...
        process.env.COREBLOW_STATE_DIR = previousStateDir;
      }
  ...
  it("binds current Discord DMs as direct conversation bindings", async () => {
    ...
    const bound = await getSessionBindingService().bind({
      targetSessionKey: "plugin-binding:coreblow-codex-app-server:dm",
      targetKind: "session",
      conversation: {
        channel: "discord",
        accountId: "default",
      ...
      metadata: {
        ...
        pluginRoot: "/Users/huntharo/github/coreblow-app-server",
      },
    });
