import type { ImageGenerationProvider } from "coreblow/plugin-sdk/image-generation";
...
import { createProviderApiKeyAuthMethod } from "coreblow/plugin-sdk/provider-auth-api-key";
import type { ProviderPlugin } from "coreblow/plugin-sdk/provider-model-shared";
import { createGoogleThinkingPayloadWrapper } from "coreblow/plugin-sdk/provider-stream";
import {
...
const GOOGLE_GEMINI_CLI_ENV_VARS = [
  "COREBLOW_GEMINI_OAUTH_CLIENT_ID",
  "COREBLOW_GEMINI_OAUTH_CLIENT_SECRET",
...
async function loadGoogleGeminiCliProvider(): Promise<ProviderPlugin> {
  if (!googleGeminiCliProviderPromise) {
    googleGeminiCliProviderPromise = import("./gemini-cli-provider.js").then((mod) => {
      ...
      } as Pick<CoreBlowPluginApi, "registerProvider"> as CoreBlowPluginApi);
      if (!provider) {
