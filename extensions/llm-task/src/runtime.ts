...
export class LlmTaskRuntime {
  [k: string]: any;
  private extension: LlmTaskExtension;
  private channels = new Map<string, LlmTaskChannelImpl>();
  private running = false;

  constructor(extension: LlmTaskExtension) {
  ...
  getChannel(id: string) {
    if (!this.channels.has(id)) {
      this.channels.set(id, new LlmTaskChannelImpl(id));
    }
