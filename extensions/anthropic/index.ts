...
import { parseDurationMs } from "coreblow/plugin-sdk/cli-runtime";
import {
  definePluginEntry,
...
import { cloneFirstTemplateModel } from "coreblow/plugin-sdk/provider-model-shared";
import { fetchClaudeUsage } from "coreblow/plugin-sdk/provider-usage";
...
function buildAnthropicAuthDoctorHint(params: {
  ...
  return [
    ...
    `Fix: run "${formatCliCommand("coreblow doctor --yes")}"`,
  ].join("\n");
}
