...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["coreblowPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["coreblow", "pollId"]) ??
    readNestedString(value, ["coreblow", "poll", "id"]) ??
    readNestedString(value, ["data", "coreblowPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "coreblow", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          coreblowPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "coreblow poll vote",
          displayText: "Vote recorded",
          value: { coreblowPollId: pollId, pollId },
        },
      },
    ],
  };
