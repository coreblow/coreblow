...
import { formatAllowFromLowercase } from "coreblow/plugin-sdk/allow-from";
import { createMessageToolCardSchema } from "coreblow/plugin-sdk/channel-actions";
import { createTopLevelChannelConfigAdapter } from "coreblow/plugin-sdk/channel-config-helpers";
import type {
  ChannelMessageActionAdapter,
  ChannelMessageToolDiscovery,
} from "coreblow/plugin-sdk/channel-contract";
import { createPairingPrefixStripper } from "coreblow/plugin-sdk/channel-pairing";
import {
  createAllowlistProviderGroupPolicyWarningCollector,
  projectConfigWarningCollector,
} from "coreblow/plugin-sdk/channel-policy";
import { createChatChannelPlugin } from "coreblow/plugin-sdk/core";
import {
  createChannelDirectoryAdapter,
  createRuntimeDirectoryLiveAdapter,
  listDirectoryEntriesFromSources,
} from "coreblow/plugin-sdk/directory-runtime";
import { createLazyRuntimeNamedExport } from "coreblow/plugin-sdk/lazy-runtime";
import { createRuntimeOutboundDelegates } from "coreblow/plugin-sdk/outbound-runtime";
import { createComputedAccountStatusAdapter } from "coreblow/plugin-sdk/status-helpers";
import type { ChannelMessageActionName, ChannelPlugin, CoreBlowConfig } from "../runtime-api.js";
import {
...
const resolveMSTeamsChannelConfig = (cfg: CoreBlowConfig) => ({
...
export const msteamsPlugin: ChannelPlugin<ResolvedMSTeamsAccount, ProbeMSTeamsResult> =
  createChatChannelPlugin({
    ...
    security: {
      collectWarnings: projectConfigWarningCollector<{ cfg: CoreBlowConfig }>(
...
/**
 * Channel implementation used by msteams runtime for per-channel state.
 */
export class MsteamsChannelImpl {
  readonly id: string;
