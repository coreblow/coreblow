import type { AnyAgentTool, CoreBlowPluginApi } from "../runtime-api.js";
...
export function createToolFactoryHarness(cfg: CoreBlowPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<CoreBlowPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
