...
describe("diffs tool", () => {
  ...
  beforeEach(async () => {
    ({ store, cleanup: cleanupRootDir } = await createDiffStoreHarness("coreblow-diffs-tool-"));
  });
...
function createApi(): CoreBlowPluginApi {
  return createTestPluginApi({
    ...
    runtime: {} as CoreBlowPluginApi["runtime"],
...
function createToolWithScreenshotter(
  ...
  defaults = DEFAULT_DIFFS_TOOL_DEFAULTS,
  context: CoreBlowPluginToolContext | undefined = {
    agentId: "main",
    sessionId: "session-123",
    messageChannel: "discord",
    agentAccountId: "default",
