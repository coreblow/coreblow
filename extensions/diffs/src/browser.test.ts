...
import type { CoreBlowPluginApi, CoreBlowPluginToolContext } from "../api.js";
...
describe("PlaywrightDiffScreenshotter", () => {
  ...
  beforeEach(async () => {
    vi.useFakeTimers();
    ({ rootDir, cleanup: cleanupRootDir } = await createTempDiffRoot("coreblow-diffs-browser-"));
...
describe("diffs plugin registration", () => {
  ...
  it("applies plugin-config defaults through registered tool and viewer handler", async () => {
    ...
    type RegisteredHttpRouteParams = Parameters<CoreBlowPluginApi["registerHttpRoute"]>[0];

    let registeredToolFactory:
      | ((ctx: CoreBlowPluginToolContext) => RegisteredTool | RegisteredTool[] | null | undefined)
      | undefined;
    ...
    const api = createTestPluginApi({
      ...
      runtime: {} as never,
      registerTool(tool: Parameters<CoreBlowPluginApi["registerTool"]>[0]) {
    ...
    plugin.register?.(api as unknown as CoreBlowPluginApi);
