...
} from "coreblow/plugin-sdk/secret-input";
...
type WebSearchConfig = NonNullable<CoreBlowConfig["tools"]>["web"] extends infer Web
  ? Web extends { search?: infer Search }
    ? Search
    : undefined
  : undefined;

type WebFetchConfig = NonNullable<CoreBlowConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: CoreBlowConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: CoreBlowConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: CoreBlowConfig): FirecrawlSearchConfig {
...
export function resolveFirecrawlFetchConfig(cfg?: CoreBlowConfig): FirecrawlFetchConfig {
...
export function resolveFirecrawlApiKey(cfg?: CoreBlowConfig): string | undefined {
...
export function resolveFirecrawlBaseUrl(cfg?: CoreBlowConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: CoreBlowConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: CoreBlowConfig, override?: number): number {
