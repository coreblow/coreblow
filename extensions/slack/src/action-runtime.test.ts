...
describe("handleSlackAction", () => {
  function slackConfig(overrides?: Record<string, unknown>): CoreBlowConfig {
    return {
      channels: {
        slack: {
          botToken: "tok",
  ...
  function createReplyToFirstScenario() {
    const cfg = { channels: { slack: { botToken: "tok" } } } as CoreBlowConfig;
  ...
  async function resolveReadToken(cfg: CoreBlowConfig): Promise<string | undefined> {
  ...
  async function resolveSendToken(cfg: CoreBlowConfig): Promise<string | undefined> {
    sendSlackMessage.mockClear();
