import type { SlackSlashCommandConfig } from "coreblow/plugin-sdk/config-runtime";
...
export function resolveSlackSlashCommandConfig(
  ...
  const normalizedName = normalizeSlackSlashCommandName(raw?.name?.trim() || "coreblow");
  const name = normalizedName || "coreblow";
  return {
