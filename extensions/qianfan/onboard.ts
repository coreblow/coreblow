...
function resolveQianfanPreset(cfg: CoreBlowConfig): {
  api: ModelApi;
  baseUrl: string;
...
const qianfanPresetAppliers = createDefaultModelsPresetAppliers({
  ...
  resolveParams: (cfg: CoreBlowConfig) => {
...
export function applyQianfanProviderConfig(cfg: CoreBlowConfig): CoreBlowConfig {
  return qianfanPresetAppliers.applyProviderConfig(cfg);
}

export function applyQianfanConfig(cfg: CoreBlowConfig): CoreBlowConfig {
