...
vi.mock("coreblow/plugin-sdk/provider-web-search", () => ({
...
vi.mock("coreblow/plugin-sdk/security-runtime", () => ({
...
describe("tavily client X-Client-Source header", () => {
  ...
  it("runTavilySearch sends X-Client-Source: coreblow", async () => {
    ...
    expect(params.extraHeaders).toEqual({ "X-Client-Source": "coreblow" });
  });

  it("runTavilyExtract sends X-Client-Source: coreblow", async () => {
