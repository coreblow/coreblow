...
function fakeApi(): CoreBlowPluginApi {
  return {
    config: {},
...
describe("tavily tools", () => {
  ...
  it("normalizes optional parameters before invoking Tavily", async () => {
    ...
    const result = await tool.execute("call-1", {
      ...
      include_answer: true,
      time_range: "week",
      include_domains: ["docs.coreblow.ai", "", "coreblow.ai"],
    ...
    expect(runTavilySearch).toHaveBeenCalledWith({
      ...
      includeAnswer: true,
      timeRange: "week",
      includeDomains: ["docs.coreblow.ai", "coreblow.ai"],
  ...
  it("falls back to environment values and defaults", () => {
    ...
    expect(resolveTavilyBaseUrl({} as CoreBlowConfig)).not.toBe(DEFAULT_TAVILY_BASE_URL);
