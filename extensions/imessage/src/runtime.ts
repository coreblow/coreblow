...
export class ImessageRuntime {
  [k: string]: any;
  private extension: ImessageExtension;
  private channels = new Map<string, ImessageChannelImpl>();
  private running = false;

  constructor(extension: ImessageExtension) {
  ...
  getChannel(id: string) {
    if (!this.channels.has(id)) {
      this.channels.set(id, new ImessageChannelImpl(id));
    }
