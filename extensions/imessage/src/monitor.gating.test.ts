...
function baseCfg(): CoreBlowConfig {
  return {
    channels: {
      imessage: {
        dmPolicy: "open",
    ...
    messages: {
      groupChat: { mentionPatterns: ["@coreblow"] },
    },
...
function buildDispatchContextPayload(params: { cfg: CoreBlowConfig; message: IMessagePayload }) {
...
describe("imessage monitor gating + envelope builders", () => {
  ...
  it("blocks group messages when imessage.groups is set without a wildcard", () => {
    ...
    const decision = resolveIMessageInboundDecision({
      ...
      opts: {},
      messageText: "@coreblow hello",
      bodyText: "@coreblow hello",
      allowFrom: ["*"],
      groupAllowFrom: [],
      groupPolicy: "open",
      dmPolicy: "open",
  ...
  it("honors group allowlist and ignores pairing-store senders in groups", () => {
    ...
    const denied = resolveIMessageInboundDecision({
      ...
      opts: {},
      messageText: "@coreblow hi",
      bodyText: "@coreblow hi",
      allowFrom: ["*"],
    ...
    const allowed = resolveIMessageInboundDecision({
      ...
      opts: {},
      messageText: "@coreblow ok",
      bodyText: "@coreblow ok",
      allowFrom: ["*"],
      groupAllowFrom: ["chat_id:101"],
      groupPolicy: "allowlist",
      dmPolicy: "pairing",
