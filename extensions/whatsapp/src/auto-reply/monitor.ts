import { resolveInboundDebounceMs } from "coreblow/plugin-sdk/channel-inbound";
import { enqueueSystemEvent } from "coreblow/plugin-sdk/channel-runtime";
import { formatCliCommand } from "coreblow/plugin-sdk/cli-runtime";
import { waitForever } from "coreblow/plugin-sdk/cli-runtime";
import { hasControlCommand } from "coreblow/plugin-sdk/command-auth";
import { loadConfig } from "coreblow/plugin-sdk/config-runtime";
import { DEFAULT_GROUP_HISTORY_LIMIT } from "coreblow/plugin-sdk/reply-history";
import { getReplyFromConfig } from "coreblow/plugin-sdk/reply-runtime";
import { resolveAgentRoute } from "coreblow/plugin-sdk/routing";
import { logVerbose } from "coreblow/plugin-sdk/runtime-env";
import { registerUnhandledRejectionHandler } from "coreblow/plugin-sdk/runtime-env";
import { getChildLogger } from "coreblow/plugin-sdk/runtime-env";
import {
  defaultRuntime,
  formatDurationPrecise,
  type RuntimeEnv,
} from "coreblow/plugin-sdk/runtime-env";
...
export async function monitorWebChannel(
  ...
  while (true) {
    ...
    if (loggedOut) {
      ...
      runtime.error(
        `WhatsApp session logged out. Run \`${formatCliCommand("coreblow channels login --channel web")}\` to relink.`,
      );
      await closeListener();
      break;
    }
    ...
    if (isNonRetryableWebCloseStatus(statusCode)) {
      ...
      runtime.error(
        `WhatsApp Web connection closed (status ${statusCode}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("coreblow channels login --channel web")}\`. Stopping web monitoring.`,
      );
      await closeListener();
      break;
    }
