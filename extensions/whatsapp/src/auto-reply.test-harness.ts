...
import { createPinnedLookup } from "coreblow/plugin-sdk/fetch-runtime";
...
vi.mock("coreblow/plugin-sdk/agent-runtime", async (importOriginal) => {
...
export function installWebAutoReplyTestHomeHooks() {
  beforeAll(async () => {
    tempHomeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-web-home-suite-"));
  });
...
export async function makeSessionStore(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-session-"));
