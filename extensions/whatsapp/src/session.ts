...
import { VERSION } from "coreblow/plugin-sdk/cli-runtime";
import { danger, success } from "coreblow/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "coreblow/plugin-sdk/runtime-env";
import { ensureDir, resolveUserPath } from "coreblow/plugin-sdk/text-runtime";
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("coreblow channels login")}`,
              ),
            );
          }
        }
