...
const CONFIG_KEY = Symbol.for("coreblow:testConfigMock");
...
vi.mock("coreblow/plugin-sdk/state-paths", async (importOriginal) => {
  const actual = await importOriginal<typeof import("coreblow/plugin-sdk/state-paths")>();
  return {
    ...actual,
    resolveOAuthDir: () => "/tmp/coreblow-oauth",
  };
});
...
vi.mock("@whiskeysockets/baileys", async (importOriginal) => {
  ...
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("coreblow:lastSocket")] =
    created.lastSocket;
  return {
    ...actual,
    ...created.mod,
...
export function getLastSocket(): MockBaileysSocket {
  const getter = (globalThis as Record<PropertyKey, unknown>)[Symbol.for("coreblow:lastSocket")];
  if (typeof getter === "function") {
