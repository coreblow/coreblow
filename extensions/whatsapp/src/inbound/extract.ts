...
import { formatLocationText, type NormalizedLocation } from "coreblow/plugin-sdk/channel-inbound";
import { logVerbose } from "coreblow/plugin-sdk/runtime-env";
import { jidToE164 } from "coreblow/plugin-sdk/text-runtime";
...
function extractContactPlaceholder(rawMessage: proto.IMessage | undefined): string | undefined {
  ...
  const labels = contactsArray
    .map((entry: any) => describeContact({ displayName: entry.displayName, vcard: entry.vcard }))
    .map((entry: any) => formatContactLabel(entry.name, entry.phones))
