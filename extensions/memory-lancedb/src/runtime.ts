...
export class MemoryLancedbRuntime {
  [k: string]: any;
  private extension: MemoryLancedbExtension;
  private channels = new Map<string, MemoryLancedbChannelImpl>();
  private running = false;

  constructor(extension: MemoryLancedbExtension) {
  ...
  getChannel(id: string) {
    if (!this.channels.has(id)) {
      this.channels.set(id, new MemoryLancedbChannelImpl(id));
    }
