...
import type { GetReplyOptions, MsgContext } from "coreblow/plugin-sdk/reply-runtime";
...
async function withIsolatedStateDirAsync<T>(fn: () => Promise<T>): Promise<T> {
  const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "coreblow-telegram-state-"));
  return await withEnvAsync({ COREBLOW_STATE_DIR: stateDir }, async () => {
    try {
      return await fn();
    } finally {
...
async function withConfigPathAsync<T>(cfg: unknown, fn: () => Promise<T>): Promise<T> {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "coreblow-telegram-cfg-"));
  const configPath = path.join(dir, "coreblow.json");
  fs.writeFileSync(configPath, JSON.stringify(cfg), "utf-8");
  return await withEnvAsync({ COREBLOW_CONFIG_PATH: configPath }, async () => {
    try {
      return await fn();
    } finally {
...
describe("createTelegramBot", () => {
  ...
  it("reloads callback model routing bindings without recreating the bot", async () => {
    const buildModelsProviderDataMock =
      telegramBotDepsForTest.buildModelsProviderData as unknown as Mock;
  ...
  it("handles pairing DM flows for new and already-pending requests", async () => {
    ...
    await withIsolatedStateDirAsync(async () => {
      for (const [index, testCase] of cases.entries()) {
        ...
        expect(pairingText, testCase.name).toContain("coreblow pairing approve telegram");
    ...
    await withIsolatedStateDirAsync(async () => {
      ...
      await handler({
        message: {
          ...
          message_id: 1884,
          date: 1736380800,
          from: { id: 7, is_bot: true, first_name: "CoreBlow", username: "coreblow_bot" },
          pinned_message: {
            message_id: 1883,
            date: 1736380799,
            chat: { id: 1234, type: "private", first_name: "Harold" },
            from: { id: 7, is_bot: true, first_name: "CoreBlow", username: "coreblow_bot" },
            text: "Binding: Review pull request 54118 (coreblow)",
          },
        },
        me: { id: 7, is_bot: true, first_name: "CoreBlow", username: "coreblow_bot" },
        getFile: async () => ({ download: async () => new Uint8Array() }),
      });
  ...
  async function dispatchMessage(params: {
    ...
    await handler({
      ...
      me: params.me ?? { username: "coreblow_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  }
  ...
  it("honors routed group activation from session store", async () => {
    const storeDir = fs.mkdtempSync(path.join(os.tmpdir(), "coreblow-telegram-"));
