// @ts-nocheck
import { resolveDefaultModelForAgent } from "coreblow/plugin-sdk/agent-runtime";
...
import type { MsgContext } from "coreblow/plugin-sdk/reply-runtime";
...
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("coreblow/plugin-sdk/reply-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/coreblow-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
vi.mock("coreblow/plugin-sdk/web-media", () => ({
  loadWebMedia,
}));
vi.mock("coreblow/plugin-sdk/web-media.js", () => ({
  loadWebMedia,
}));
...
vi.doMock("coreblow/plugin-sdk/config-runtime", async (importOriginal) => {
...
vi.doMock("coreblow/plugin-sdk/conversation-runtime", async (importOriginal) => {
...
vi.doMock("coreblow/plugin-sdk/conversation-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("coreblow/plugin-sdk/conversation-runtime")>();
  return {
    ...actual,
    readChannelAllowFromStore,
...
function createModelsProviderDataFromConfig(cfg: CoreBlowConfig): {
...
vi.doMock("coreblow/plugin-sdk/command-auth", async (importOriginal) => {
...
vi.doMock("coreblow/plugin-sdk/command-auth.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("coreblow/plugin-sdk/command-auth")>();
  return {
    ...actual,
    listSkillCommandsForAgents: skillCommandListHoisted.listSkillCommandsForAgents,
...
vi.doMock("coreblow/plugin-sdk/reply-runtime", async (importOriginal) => {
...
vi.doMock("coreblow/plugin-sdk/reply-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("coreblow/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
    getReplyFromConfig: replySpyHoisted.replySpy,
...
vi.doMock("coreblow/plugin-sdk/channel-runtime", async (importOriginal) => {
...
const DEFAULT_TELEGRAM_TEST_CONFIG: CoreBlowConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: CoreBlowConfig) => {
    return createModelsProviderDataFromConfig(cfg);
  });
  middlewareUseSpy.mockReset();
  runnerHoisted.sequentializeMiddleware.mockReset();
  runnerHoisted.sequentializeMiddleware.mockImplementation(async (_ctx: any, next: any) => {
    if (typeof next === "function") {
      await next();
    }
  });
