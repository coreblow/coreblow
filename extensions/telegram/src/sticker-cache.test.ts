...
vi.mock("coreblow/plugin-sdk/agent-runtime", () => ({
...
vi.mock("coreblow/plugin-sdk/media-runtime", () => ({
  AUTO_IMAGE_KEY_PROVIDERS: ["openai"],
  DEFAULT_IMAGE_MODELS: { openai: "gpt-4.1-mini" },
  resolveAutoImageModel: vi.fn(async () => null),
}));

vi.mock("coreblow/plugin-sdk/media-understanding-runtime", () => ({
  describeImageFileWithModel: vi.fn(),
}));

const TEST_CACHE_DIR = "/tmp/coreblow-test-sticker-cache/telegram";
...
describe("sticker-cache", () => {
  beforeEach(async () => {
    process.env.COREBLOW_STATE_DIR = "/tmp/coreblow-test-sticker-cache";
    fs.rmSync("/tmp/coreblow-test-sticker-cache", { recursive: true, force: true });
