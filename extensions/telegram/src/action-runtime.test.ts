...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): CoreBlowConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as CoreBlowConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): CoreBlowConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as CoreBlowConfig;
    await expect(
      handleTelegramAction(
        {
          action: "sendSticker",
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: CoreBlowConfig, accountId = "media") {
    await handleTelegramAction(
