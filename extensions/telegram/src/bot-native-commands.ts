...
import type { ChannelGroupPolicy } from "coreblow/plugin-sdk/config-runtime";
import { getRuntimeConfigSnapshot } from "coreblow/plugin-sdk/config-runtime";
...
export const registerTelegramNativeCommands = ({
  ...
  const loadFreshRuntimeConfig = (): CoreBlowConfig => telegramDeps.loadConfig();
  const resolveFreshTelegramConfig = (runtimeCfg: CoreBlowConfig): TelegramAccountConfig => {
    try {
