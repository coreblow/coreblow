...
export class TalkVoiceRuntime {
  private extension: TalkVoiceExtension;
  private channels = new Map<string, TalkVoiceChannelImpl>();
  private running = false;

  constructor(extension: TalkVoiceExtension) {
  ...
  getChannel(id: string) {
    if (!this.channels.has(id)) {
      this.channels.set(id, new TalkVoiceChannelImpl(id));
    }
