...
import type { PendingPairingRequest } from "./notify.ts.js";
...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredCoreBlowTmpDir: vi.fn(() => path.join(os.tmpdir(), "coreblow-device-pair-tests")),
}));
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredCoreBlowTmpDir: pluginApiMocks.resolvePreferredCoreBlowTmpDir,
...
function createApi(params?: {
  runtime?: CoreBlowPluginApi["runtime"];
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as CoreBlowPluginApi["runtime"],
    registerCommand: params?.registerCommand,
  }) as CoreBlowPluginApi;
}
...
function registerPairCommand(params?: {
  ...
  let command: CoreBlowPluginCommandDefinition | undefined;
...
function createChannelRuntime(
  ...
  } as unknown as CoreBlowPluginApi["runtime"];
}
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredCoreBlowTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredCoreBlowTmpDir(), { recursive: true, force: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the CoreBlow iOS app:");
    expect(text).toContain("![CoreBlow pairing QR](data:image/png;base64,ZmFrZXBuZw==)");
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the CoreBlow iOS app:");
