...
export class DevicePairRuntime {
  private extension: DevicePairExtension;
  private channels = new Map<string, DevicePairChannelImpl>();
  private running = false;

  constructor(extension: DevicePairExtension) {
  ...
  getChannel(id: string) {
    if (!this.channels.has(id)) {
      this.channels.set(id, new DevicePairChannelImpl(id));
    }
