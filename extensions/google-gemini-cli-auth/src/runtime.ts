...
export class GoogleGeminiCliAuthRuntime {
  private extension: GoogleGeminiCliAuthExtension;
  private channels = new Map<string, GoogleGeminiCliAuthChannelImpl>();
  private running = false;

  constructor(extension: GoogleGeminiCliAuthExtension) {
  ...
  getChannel(id: string) {
    if (!this.channels.has(id)) {
      this.channels.set(id, new GoogleGeminiCliAuthChannelImpl(id));
    }
