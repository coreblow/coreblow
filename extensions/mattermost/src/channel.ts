 ...
 */
import type { MattermostMessage, MattermostChannel } from '../types';

export class MattermostChannelImpl {
  private channelId: string;
  private connected = false;
  private messageQueue: MattermostMessage[] = [];
  ...
  async sendMessage(content: string, options: Record<string, any> = {}) {
    if (!this.connected) throw new Error('Not connected to mattermost');
    return {
