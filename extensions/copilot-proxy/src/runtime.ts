...
export class CopilotProxyRuntime {
  private extension: CopilotProxyExtension;
  private channels = new Map<string, CopilotProxyChannelImpl>();
  private running = false;

  constructor(extension: CopilotProxyExtension) {
  ...
  getChannel(id: string) {
    if (!this.channels.has(id)) {
      this.channels.set(id, new CopilotProxyChannelImpl(id));
    }
