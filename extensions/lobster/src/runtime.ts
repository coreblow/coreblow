...
export class LobsterRuntime {
  [k: string]: any;
  private extension: LobsterExtension;
  private channels = new Map<string, LobsterChannelImpl>();
  private running = false;

  constructor(extension: LobsterExtension) {
  ...
  getChannel(id: string) {
    if (!this.channels.has(id)) {
      this.channels.set(id, new LobsterChannelImpl(id));
    }
