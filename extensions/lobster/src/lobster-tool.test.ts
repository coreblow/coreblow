...
import type { CoreBlowPluginApi, CoreBlowPluginToolContext } from "../runtime-api.js";
import {
...
function fakeApi(overrides: Partial<CoreBlowPluginApi> = {}): CoreBlowPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<CoreBlowPluginToolContext> = {}): CoreBlowPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: CoreBlowPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
...
describe("resolveWindowsLobsterSpawn", () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-lobster-win-spawn-"));
    setProcessPlatform("win32");
  });
