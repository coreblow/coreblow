...
import { createAccountStatusSink } from "coreblow/plugin-sdk/channel-lifecycle";
import {
  createLoggedPairingApprovalNotifier,
  createPairingPrefixStripper,
} from "coreblow/plugin-sdk/channel-pairing";
import { createAllowlistProviderRouteAllowlistWarningCollector } from "coreblow/plugin-sdk/channel-policy";
import { createChatChannelPlugin } from "coreblow/plugin-sdk/core";
import { runStoppablePassiveMonitor } from "coreblow/plugin-sdk/extension-shared";
import {
    ...
    base: {
      ...
      gateway: {
        ...
        logoutAccount: async ({ accountId, cfg }) => {
          const nextCfg = { ...cfg } as CoreBlowConfig;
          ...
          if (changed) {
            if (nextSection && Object.keys(nextSection).length > 0) {
              ...
              if (Object.keys(nextChannels).length > 0) {
                nextCfg.channels = nextChannels as CoreBlowConfig["channels"];
              } else {
...
/**
 * Channel implementation used by nextcloud-talk runtime for per-channel state.
 */
export class NextcloudTalkChannelImpl {
  readonly id: string;
