...
export class MinimaxPortalAuthRuntime {
  [k: string]: any;
  private extension: MinimaxPortalAuthExtension;
  private channels = new Map<string, MinimaxPortalAuthChannelImpl>();
  private running = false;

  constructor(extension: MinimaxPortalAuthExtension) {
  ...
  getChannel(id: string) {
    if (!this.channels.has(id)) {
      this.channels.set(id, new MinimaxPortalAuthChannelImpl(id));
    }
