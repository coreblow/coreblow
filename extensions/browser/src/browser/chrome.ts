...
export function resolveCoreBlowUserDataDir(profileName = DEFAULT_COREBLOW_BROWSER_PROFILE_NAME) {
...
export function buildCoreBlowChromeLaunchArgs(params: {
...
export async function launchCoreBlowChrome(
  resolved: ResolvedBrowserConfig,
  profile: ResolvedBrowserProfile,
  ...
  const userDataDir = resolveCoreBlowUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_COREBLOW_BROWSER_COLOR).toUpperCase(),
  );

  // First launch to create preference files if missing, then decorate and relaunch.
  const spawnOnce = () => {
    const args = buildCoreBlowChromeLaunchArgs({
      resolved,
      profile,
      userDataDir,
    });
  ...
  if (needsDecorate) {
    try {
      decorateCoreBlowProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 coreblow browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`coreblow browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`coreblow browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 coreblow browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopCoreBlowChrome(
