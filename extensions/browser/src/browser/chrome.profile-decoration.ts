...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".coreblow-profile-decorated");
}
 ...
 */
export function decorateCoreBlowProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_COREBLOW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_COREBLOW_BROWSER_COLOR).toUpperCase();
