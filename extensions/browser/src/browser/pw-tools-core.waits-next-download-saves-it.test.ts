...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredCoreBlowTmpDir: vi.fn(() => "/tmp/coreblow"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-coreblow-dir.js");
...
describe("pw-tools-core", () => {
  beforeAll(async () => {
    ...
    tmpDirModule = await import("../infra/tmp-coreblow-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredCoreBlowTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredCoreBlowTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredCoreBlowTmpDir.mockReturnValue("/tmp/coreblow");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".coreblow-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredCoreBlowTmpDir.mockReturnValue("/tmp/coreblow-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "coreblow-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "coreblow-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredCoreBlowTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "coreblow-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "coreblow-preferred", "downloads")}${path.sep}`),
    );
  });
