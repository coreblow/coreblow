...
vi.mock("./chrome.js", () => ({
  stopCoreBlowChrome: stopCoreBlowChromeMock,
}));
...
beforeEach(async () => {
  ...
  stopCoreBlowChromeMock.mockClear();
});
...
describe("stopKnownBrowserProfiles", () => {
  it("stops all known profiles and ignores per-profile failures", async () => {
    ...
    expect(stopMap.coreblow).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopCoreBlowChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("coreblow browser stop failed: Error: oops");
  });
});
