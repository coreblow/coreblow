...
function localCoreBlowProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "coreblow",
...
function createLocalCoreBlowResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localCoreBlowProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveCoreBlowUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "coreblow-reset-"));
    const profileDir = path.join(tempRoot, "coreblow");
    ...
    const ops = createLocalCoreBlowResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveCoreBlowUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "coreblow-reset-no-own-"));
    const profileDir = path.join(tempRoot, "coreblow");
    fs.mkdirSync(profileDir, { recursive: true });
