import type { BrowserConfig, BrowserProfileConfig, CoreBlowConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_COREBLOW_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_COREBLOW_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "coreblow" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_COREBLOW_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_COREBLOW_BROWSER_PROFILE_NAME] = {
...
export function resolveBrowserConfig(
  ...
  rootConfig?: CoreBlowConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_COREBLOW_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_COREBLOW_BROWSER_PROFILE_NAME]
        ? DEFAULT_COREBLOW_BROWSER_PROFILE_NAME
        : "user");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "existing-session" ? "existing-session" : "coreblow";
