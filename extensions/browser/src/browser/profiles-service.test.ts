...
import { resolveCoreBlowUserDataDir } from "./chrome.js";
...
vi.mock("./chrome.js", () => ({
  resolveCoreBlowUserDataDir: vi.fn(() => "/tmp/coreblow-test/coreblow/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "coreblow-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveCoreBlowUserDataDir).mockReturnValue(userDataDir);
