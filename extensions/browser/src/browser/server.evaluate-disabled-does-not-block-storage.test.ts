  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          coreblow: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  ...
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.COREBLOW_GATEWAY_PORT;
    process.env.COREBLOW_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.COREBLOW_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.COREBLOW_GATEWAY_PASSWORD;
