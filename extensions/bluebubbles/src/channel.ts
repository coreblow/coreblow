 ...
 */
import type { BluebubblesMessage, BluebubblesChannel } from '../types';

export class BluebubblesChannelImpl {
  private channelId: string;
  private connected = false;
  private messageQueue: BluebubblesMessage[] = [];
  ...
  async sendMessage(content: string, options: Record<string, any> = {}) {
    if (!this.connected) throw new Error('Not connected to bluebubbles');
    return {
