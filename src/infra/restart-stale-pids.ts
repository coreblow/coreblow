 ...
 * returning control to the caller (typically `triggerCoreBlowRestart` →
...
/**
 * Parse coreblow gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("coreblow")) {
 ...
 * Returns only PIDs that belong to coreblow gateway processes (not the current process).
 */
