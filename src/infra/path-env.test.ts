...
let ensureCoreBlowCliOnPath: typeof import("./path-env.js").ensureCoreBlowCliOnPath;
...
describe("ensureCoreBlowCliOnPath", () => {
  ...
  beforeAll(async () => {
    ({ ensureCoreBlowCliOnPath } = await import("./path-env.js"));
  });
  ...
  function setupAppCliRoot(name: string) {
    const tmp = abs(`/tmp/coreblow-path/${name}`);
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "coreblow");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  function bootstrapPath(params: {
    ...
    ensureCoreBlowCliOnPath(params);
  ...
  it("prepends the bundled app bin dir when a sibling coreblow exists", () => {
  ...
  it.each([
    ...
    ({ envValue, allowProjectLocalBin }) => {
      ...
      const localCli = path.join(localBinDir, "coreblow");
  ...
  it.each([
    ...
    },
    {
      ...
      setup: () => {
        const tmp = abs("/tmp/coreblow-path/case-linuxbrew");
