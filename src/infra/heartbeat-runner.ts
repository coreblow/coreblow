...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: CoreBlowConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: CoreBlowConfig) {
...
function resolveHeartbeatAgents(cfg: CoreBlowConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: CoreBlowConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: CoreBlowConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: CoreBlowConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
