...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred CoreBlow tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideCoreBlow = path.join(os.tmpdir(), "outside-coreblow", "test-media.png");
        await expect(
          runMessageAction({
            cfg: slackConfig,
            action: "send",
