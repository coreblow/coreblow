...
export const POSIX_COREBLOW_TMP_DIR = "/tmp/coreblow";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredCoreBlowTmpDirOptions = {
...
export function resolvePreferredCoreBlowTmpDir(
  options: ResolvePreferredCoreBlowTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "coreblow" : `coreblow-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[coreblow] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback CoreBlow temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback CoreBlow temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_COREBLOW_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_COREBLOW_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_COREBLOW_TMP_DIR)) {
      return POSIX_COREBLOW_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_COREBLOW_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_COREBLOW_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_COREBLOW_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_COREBLOW_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_COREBLOW_TMP_DIR;
