...
function makeTargetsCfg(targets: Array<{ channel: string; to: string }>): CoreBlowConfig {
  return {
    approvals: {
      exec: {
        enabled: true,
...
function makeSessionCfg(options: { discordExecApprovalsEnabled?: boolean } = {}): CoreBlowConfig {
  return {
