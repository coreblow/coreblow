...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/coreblow-state", "devices")).toEqual({
      dir: path.join("/tmp/coreblow-state", "devices"),
      pendingPath: path.join("/tmp/coreblow-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/coreblow-state", "devices", "paired.json"),
    });
  });
