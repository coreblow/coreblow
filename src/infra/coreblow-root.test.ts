...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__coreblow_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "coreblow-root");
...
function setPackageRoot(root: string, name = "coreblow") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./coreblow-root.js").resolveCoreBlowPackageRootSync,
  asyncResolver: typeof import("./coreblow-root.js").resolveCoreBlowPackageRoot,
  opts: Parameters<typeof import("./coreblow-root.js").resolveCoreBlowPackageRootSync>[0],
...
describe("resolveCoreBlowPackageRoot", () => {
  let resolveCoreBlowPackageRoot: typeof import("./coreblow-root.js").resolveCoreBlowPackageRoot;
  let resolveCoreBlowPackageRootSync: typeof import("./coreblow-root.js").resolveCoreBlowPackageRootSync;

  beforeEach(async () => {
    vi.resetModules();
    ({ resolveCoreBlowPackageRoot, resolveCoreBlowPackageRootSync } =
      await import("./coreblow-root.js"));
    state.entries.clear();
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "coreblow");
        const pkgRoot = path.join(project, "node_modules", "coreblow");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "coreblow");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "coreblow.mjs")));
    ...
    {
      name: "falls through from a non-coreblow moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-coreblow");
    ...
    {
      name: "returns null for non-coreblow package roots",
      setup: () => {
        const pkgRoot = fx("not-coreblow");
        setPackageRoot(pkgRoot, "not-coreblow");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "coreblow.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "coreblow");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
    ...
    await expectResolvedPackageRoot(
      resolveCoreBlowPackageRootSync,
      resolveCoreBlowPackageRoot,
      opts,
      expected,
    );
  });
