...
async function runExtractedArchiveFailureCase(configureArchive: () => void) {
  vi.spyOn(installSource, "withTempDir").mockImplementation(
    async (_prefix, fn) => await fn("/tmp/coreblow-install-flow"),
  );
  configureArchive();
...
describe("resolveExistingInstallPath", () => {
  ...
  beforeEach(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-install-flow-"));
  });
...
describe("withExtractedArchiveRoot", () => {
  ...
  it("extracts archive and passes root directory to callback", async () => {
    const tmpRoot = path.join(path.sep, "tmp", "coreblow-install-flow");
    ...
    expect(withTempDirSpy).toHaveBeenCalledWith("coreblow-plugin-", expect.any(Function));
