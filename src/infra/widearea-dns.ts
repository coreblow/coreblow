...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.COREBLOW_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*coreblow-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "coreblow";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "coreblow");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "coreblow-gw");
  ...
  records.push(`_coreblow-gw._tcp IN PTR ${instanceLabel}._coreblow-gw._tcp`);
  records.push(`${instanceLabel}._coreblow-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._coreblow-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; coreblow-content-hash: ${contentHash}\n${contentBody}`;
}
