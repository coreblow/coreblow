...
export async function getTailscaleBinary(): Promise<string> {
  const forcedBinary = process.env.COREBLOW_TEST_TAILSCALE_BINARY?.trim();
  if (forcedBinary) {
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for CoreBlow. You can keep running the web gateway without it: \`${formatCliCommand("coreblow gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
