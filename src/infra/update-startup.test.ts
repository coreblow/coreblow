...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["COREBLOW_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.COREBLOW_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveCoreBlowPackageRoot } = await import("./coreblow-root.js"));
    ...
    vi.mocked(resolveCoreBlowPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveCoreBlowPackageRoot).mockResolvedValue("/opt/coreblow");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/coreblow/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
