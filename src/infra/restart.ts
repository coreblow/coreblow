...
export function triggerCoreBlowRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.COREBLOW_SYSTEMD_UNIT,
      process.env.COREBLOW_PROFILE,
    );
  ...
  const label =
    process.env.COREBLOW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.COREBLOW_PROFILE);
