...
const DEFAULT_PACKAGE_NAME = "coreblow";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["coreblow", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "coreblow.mjs");
