...
const resolvePreferredCoreBlowTmpDirMock = vi.hoisted(() => vi.fn(() => os.tmpdir()));

vi.mock("node:child_process", () => ({
  spawn: (...args: unknown[]) => spawnMock(...args),
}));
vi.mock("./tmp-coreblow-dir.js", () => ({
  resolvePreferredCoreBlowTmpDir: () => resolvePreferredCoreBlowTmpDirMock(),
}));
...
afterEach(() => {
  envSnapshot.restore();
  spawnMock.mockReset();
  resolvePreferredCoreBlowTmpDirMock.mockReset();
  resolvePreferredCoreBlowTmpDirMock.mockReturnValue(os.tmpdir());
...
describe("relaunchGatewayScheduledTask", () => {
  ...
  it("writes a detached schtasks relaunch helper", () => {
    ...
    expect(result).toMatchObject({
      ok: true,
      method: "schtasks",
      tried: expect.arrayContaining(['schtasks /Run /TN "CoreBlow Gateway (work)"']),
    });
    ...
    expect(script).toContain('schtasks /Run /TN "CoreBlow Gateway (work)" >nul 2>&1');
  ...
  it("prefers COREBLOW_WINDOWS_TASK_NAME overrides", () => {
    ...
    const scriptPath = [...createdScriptPaths][0];
    const script = fs.readFileSync(scriptPath, "utf8");
    expect(script).toContain('schtasks /Run /TN "CoreBlow Gateway (custom)" >nul 2>&1');
  });
  ...
  it("quotes the cmd /c script path when temp paths contain metacharacters", () => {
    const unref = vi.fn();
    const metacharTmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "coreblow&(restart)-"));
