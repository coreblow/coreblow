...
function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `coreblow-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredCoreBlowTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredCoreBlowTmpDir", () => {
  it("prefers /tmp/coreblow when it already exists and is writable", () => {
  ...
  it("prefers /tmp/coreblow when it does not exist but /tmp is writable", () => {
  ...
  it.each([
    {
      name: "falls back to os.tmpdir()/coreblow when /tmp/coreblow is not a directory",
      lstatSync: vi.fn(() => makeDirStat({ isDirectory: false, mode: 0o100644 })),
    },
    {
      name: "falls back to os.tmpdir()/coreblow when /tmp is not writable",
      lstatSync: vi.fn(() => {
        throw nodeErrorWithCode("ENOENT");
      }),
    ...
    {
      name: "falls back when /tmp/coreblow is a symlink",
      lstatSync: symlinkTmpDirLstat(),
    },
    {
      name: "falls back when /tmp/coreblow is not owned by the current user",
      lstatSync: vi.fn(() => makeDirStat({ uid: 0 })),
    },
    {
      name: "falls back when /tmp/coreblow is group/other writable",
  ...
  it("repairs /tmp/coreblow after create when the initial mode stays too broad", () => {
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback CoreBlow temp dir/);
  });
  ...
  it("uses an unscoped fallback suffix when process uid is unavailable", () => {
    ...
    const fallbackPath = path.join(tmpdirPath, "coreblow");
  ...
  it("throws when the fallback directory cannot be created", () => {
    ...
    ).toThrow(/Unable to create fallback CoreBlow temp dir/);
  });
});
