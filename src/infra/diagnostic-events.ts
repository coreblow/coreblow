...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __coreblowDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__coreblowDiagnosticEventsState) {
    globalStore.__coreblowDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__coreblowDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: CoreBlowConfig): boolean {
