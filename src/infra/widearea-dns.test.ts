...
const baseZoneOpts: WideAreaGatewayZoneOpts = {
  domain: "coreblow.internal.",
  gatewayPort: 18789,
  displayName: "Mac Studio (CoreBlow)",
...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "coreblow.internal", expected: "coreblow.internal." },
    { value: "coreblow.internal.", expected: "coreblow.internal." },
    { value: "  coreblow.internal  ", expected: "coreblow.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    {
      ...
      params: {
        env: { COREBLOW_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { COREBLOW_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
  ...
  it("builds the default zone path from the normalized domain", () => {
    expect(getWideAreaZonePath("coreblow.internal.")).toBe(
      path.join(utils.CONFIG_DIR, "dns", "coreblow.internal.db"),
    );
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    const txt = renderZoneText({
      ...
      sshPort: 22,
      cliPath: "/opt/homebrew/bin/coreblow",
    });

    expectZoneRecords(txt, [
      `$ORIGIN coreblow.internal.`,
      `studio-london IN A 100.123.224.76`,
      `studio-london IN AAAA fd7a:115c:a1e0::8801:e04c`,
      `_coreblow-gw._tcp IN PTR studio-london._coreblow-gw._tcp`,
      `studio-london._coreblow-gw._tcp IN SRV 0 0 18789 studio-london`,
      `displayName=Mac Studio (CoreBlow)`,
      `gatewayPort=18789`,
      `sshPort=22`,
      `cliPath=/opt/homebrew/bin/coreblow`,
  ...
  it.each([
    ...
    },
    {
      ...
      records: [
        `$ORIGIN coreblow.internal.`,
        `studio-london IN A 100.123.224.76`,
        `studio-london._coreblow-gw._tcp IN TXT`,
        `displayName=Mac Studio (CoreBlow)`,
        `gatewayTls=1`,
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("coreblow.internal."),
      changed: false,
    });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("coreblow.internal."),
