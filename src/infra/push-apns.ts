...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.COREBLOW_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.COREBLOW_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
  ...
  const inlineKeyRaw =
    normalizeNonEmptyString(env.COREBLOW_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.COREBLOW_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.COREBLOW_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set COREBLOW_APNS_PRIVATE_KEY_P8 or COREBLOW_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading COREBLOW_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
