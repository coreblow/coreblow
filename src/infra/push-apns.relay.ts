...
const GATEWAY_DEVICE_ID_HEADER = "x-coreblow-gateway-device-id";
const GATEWAY_SIGNATURE_HEADER = "x-coreblow-gateway-signature";
const GATEWAY_SIGNED_AT_HEADER = "x-coreblow-gateway-signed-at-ms";
...
export function resolveApnsRelayConfigFromEnv(
  ...
  const envBaseUrl = normalizeNonEmptyString(env.COREBLOW_APNS_RELAY_BASE_URL);
  ...
  const baseUrlSource = envBaseUrl
    ? "COREBLOW_APNS_RELAY_BASE_URL"
    : "gateway.push.apns.relay.baseUrl";
  if (!baseUrl) {
    return {
      ok: false,
      error:
        "APNs relay config missing: set gateway.push.apns.relay.baseUrl or COREBLOW_APNS_RELAY_BASE_URL",
    };
  }
  ...
  try {
    ...
    if (parsed.protocol === "http:" && !readAllowHttp(env.COREBLOW_APNS_RELAY_ALLOW_HTTP)) {
      throw new Error(
        "http relay URLs require COREBLOW_APNS_RELAY_ALLOW_HTTP=true (development only)",
      );
    }
    ...
    return {
      ...
      value: {
        ...
        timeoutMs: normalizeTimeoutMs(
          env.COREBLOW_APNS_RELAY_TIMEOUT_MS ?? configuredRelay?.timeoutMs,
        ),
      },
    };
  } catch (err) {
...
const { getInstance: getPushApnsRelayService, __testing: __testing_pushApnsRelay } =
  createStandaloneSingleton({ create: () => new PushApnsRelayService(), defaultDeps: {} });

export { getPushApnsRelayService, __testing_pushApnsRelay };
