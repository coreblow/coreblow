...
export function buildPortHints(listeners: PortListener[], port: number): string[] {
  ...
  if (kinds.has("gateway")) {
    hints.push(
      `Gateway already running locally. Stop it (${formatCliCommand("coreblow gateway stop")}) or use a different port.`,
    );
  }
  if (kinds.has("ssh")) {
    hints.push(
...
const { getInstance: getPortsFormatService, __testing: __testing_portsFormat } =
  createStandaloneSingleton({ create: () => new PortsFormatService(), defaultDeps: {} });

export { getPortsFormatService, __testing_portsFormat };
