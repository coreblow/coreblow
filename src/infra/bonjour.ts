...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.COREBLOW_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "CoreBlow";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(CoreBlow\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw = process.env.COREBLOW_MDNS_HOSTNAME?.trim() || "coreblow";
    const hostname =
      hostnameRaw
        .replace(/\.local$/i, "")
        .split(".")[0]
    ...
    const watchdog = setInterval(() => {
      ...
      for (const { label, svc } of cycle.services) {
        ...
        try {
          void svc.advertise().catch((err: any) => {
            logWarn(
