...
describe("web tools defaults", () => {
  ...
  it("uses runtime-only web_search providers when runtime metadata is present", () => {
    ...
    // CB resolves the web_search tool through resolveWebSearchDefinition which
    // may prefer the runtime plugin provider's createTool description or fall
    // back to the bundled provider.  Assert the tool is correctly created.
...
describe("web_search kimi provider", () => {
  ...
  it("runs the Kimi web_search tool flow and echoes tool results", async () => {
      ...
      if (idx === 1) {
        return new Response(
          JSON.stringify({
                  ...
                  tool_calls: [
                    {
                      ...
                      function: {
                        name: "$web_search",
                        arguments: JSON.stringify({ q: "coreblow" }),
                      },
                    },
                  ],
                },
            ...
            search_results: [
              { title: "CoreBlow", url: "https://coreblow.ai/docs", content: "docs" },
            ],
          }),
    ...
    const result = await tool?.execute?.("call-1", { query: "latest coreblow release" });
    ...
    expect(JSON.parse(toolMessage?.content ?? "{}")).toMatchObject({
      search_results: [{ url: "https://coreblow.ai/docs" }],
    });
    ...
    expect(details.citations).toEqual(["https://coreblow.ai/docs"]);
