...
import { __testing, createCoreBlowCodingTools } from "./pi-tools.js";
import { createCoreBlowReadTool, createSandboxedReadTool } from "./pi-tools.read.js";
import { createHostSandboxFsBridge } from "./test-helpers/host-sandbox-fs-bridge.js";

const defaultTools = createCoreBlowCodingTools();
...
describe("createCoreBlowCodingTools", () => {
  ...
  it("exposes raw for gateway config.apply tool calls", () => {
    const gateway = createCoreBlowCodingTools({ senderIsOwner: true }).find(
  ...
  it("filters session tools for sub-agent sessions by default", () => {
    const tools = createCoreBlowCodingTools({
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-depth-policy-"));
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createCoreBlowCodingTools({
  ...
  it("applies xai model compat for direct Grok tool cleanup", () => {
    const xaiTools = createCoreBlowCodingTools({
      modelProvider: "xai",
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "coreblow-outside.txt");
  ...
  it("auto-pages read output across chunks when context window budget allows", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-read-autopage-"));
  ...
  it("adds capped continuation guidance when aggregated read output reaches budget", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-read-cap-"));
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createCoreBlowReadTool(
      baseRead as unknown as Parameters<typeof createCoreBlowReadTool>[0],
    );
