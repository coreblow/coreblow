...
function mockContextDeps(params: {
  ...
  const ensureCoreBlowModelsJson = vi.fn(async () => {});
  ...
  vi.doMock("./agent-paths.js", () => ({
    resolveCoreBlowAgentDir: () => "/tmp/coreblow-agent",
  }));
  ...
  return { ensureCoreBlowModelsJson };
}
...
describe("lookupContextTokens", () => {
  ...
  it("only warms eagerly for real coreblow startup commands that need model metadata", async () => {
    const argvSnapshot = process.argv;
    try {
      for (const scenario of [
        {
          argv: ["node", "coreblow", "chat"],
          expectedCalls: 1,
        },
        {
          argv: ["node", "coreblow", "--profile", "--", "config", "validate"],
          expectedCalls: 0,
        },
        {
          argv: ["node", "coreblow", "logs", "--limit", "5"],
          expectedCalls: 0,
        },
        {
        ...
        const { ensureCoreBlowModelsJson } = mockContextModuleDeps(loadConfigMock);
        process.argv = scenario.argv;
        await import("./context.js");
        await flushAsyncWarmup();
        expect(loadConfigMock).toHaveBeenCalledTimes(scenario.expectedCalls);
        expect(ensureCoreBlowModelsJson).toHaveBeenCalledTimes(scenario.expectedCalls);
      }
    } finally {
