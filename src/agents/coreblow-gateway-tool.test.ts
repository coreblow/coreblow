...
let createCoreBlowTools: typeof import("./coreblow-tools.js").createCoreBlowTools;

async function loadFreshCoreBlowToolsModuleForTest() {
  vi.resetModules();
  vi.doMock("./tools/gateway.js", () => createGatewayToolModuleMocks());
  ({ createCoreBlowTools } = await import("./coreblow-tools.js"));
}
...
describe("gateway tool", () => {
  beforeEach(async () => {
    await loadFreshCoreBlowToolsModuleForTest();
  });
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-test-"));

    try {
      await withEnvAsync(
        { COREBLOW_STATE_DIR: stateDir, COREBLOW_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: coreblow --profile isolated doctor --non-interactive",
          );
  ...
  it("rejects config.apply when protected exec settings are omitted", async () => {
    ...
    await expect(
      tool.execute("call-missing-protected", {
        action: "config.apply",
        raw: '{ agents: { defaults: { workspace: "~/coreblow" } } }',
      }),
    ).rejects.toThrow(
