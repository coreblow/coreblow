...
const CORE_IMPLICIT_PROVIDER_RESOLVERS = [
  {
    id: "anthropic-vertex",
    resolve: async (params: { config?: CoreBlowConfig; env: NodeJS.ProcessEnv }) =>
      resolveImplicitAnthropicVertexProvider({
        env: params.env,
      }),
  },
...
function resolveLiveProviderCatalogTimeoutMs(env: NodeJS.ProcessEnv): number | null {
  const live =
    env.COREBLOW_LIVE_TEST === "1" || env.COREBLOW_LIVE_GATEWAY === "1" || env.LIVE === "1";
  if (!live) {
    return null;
  }
  const raw = env.COREBLOW_LIVE_PROVIDER_DISCOVERY_TIMEOUT_MS?.trim();
  if (!raw) {
    return 15_000;
  }
...
function resolveLiveProviderDiscoveryFilter(env: NodeJS.ProcessEnv): string[] | undefined {
  ...
  const raw = env.COREBLOW_LIVE_PROVIDERS?.trim();
  if (!raw || raw === "all") {
    return undefined;
  }
  const ids = raw
...
function buildPluginCatalogConfig(ctx: ImplicitProviderContext): CoreBlowConfig {
...
): Promise<NonNullable<CoreBlowConfig["models"]>["providers"]> {
