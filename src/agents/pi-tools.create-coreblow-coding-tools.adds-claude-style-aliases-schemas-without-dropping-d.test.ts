...
describe("createCoreBlowCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-read-"));
    try {
      ...
      const contents = "Hello from coreblow read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "coreblow-sandbox");
    const sandbox = createPiToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "coreblow-workspace"),
    ...
    const tools = createCoreBlowCodingTools({ sandbox });
