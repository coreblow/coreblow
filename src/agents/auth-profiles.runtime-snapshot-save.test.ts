...
describe("auth profile runtime snapshot persistence", () => {
  it("does not write resolved plaintext keys during usage updates", async () => {
    const stateDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "coreblow-auth-runtime-save-"),
    );
    ...
    try {
      ...
      // Plaintext key resolved at runtime must NOT be persisted to disk
      expect(persisted.profiles["openai:default"]?.key).toBeUndefined();
      expect(persisted.profiles["openai:default"]?.keyRef).toMatchObject({
        source: "env",
        id: "OPENAI_API_KEY",
      });
    } finally {
