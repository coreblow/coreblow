...
test("exec sets COREBLOW_SHELL in pty mode", async () => {
  const tool = createExecTool({ allowBackground: false, security: "full", ask: "off" });
  const result = await tool.execute("toolcall-coreblow-shell", {
    command: "node -e \"process.stdout.write(process.env.COREBLOW_SHELL || '')\"",
    pty: true,
  });
