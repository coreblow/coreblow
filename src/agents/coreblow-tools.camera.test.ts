...
function getNodesTool(options?: { modelHasVision?: boolean; allowMediaInvokeCommands?: boolean }) {
  ...
  const tool = createCoreBlowTools(toolOptions).find((candidate) => candidate.name === "nodes");
  if (!tool) {
...
beforeEach(async () => {
  ...
  vi.unstubAllGlobals();
  await loadCoreBlowToolsForTest();
});
...
describe("nodes camera_snap", () => {
  ...
  it("omits inline base64 image blocks when model has no vision", async () => {
    ...
    expect(expectFirstMediaUrl(result)).toMatch(/coreblow-camera-snap-front-.*\.jpg$/);
  });
...
describe("nodes photos_latest", () => {
  ...
  it("returns MEDIA paths and no inline images when model has no vision", async () => {
    ...
    expect(expectFirstMediaUrl(result)).toMatch(/coreblow-camera-snap-.*\.jpg$/);
  });
...
async function loadCoreBlowToolsForTest(): Promise<void> {
  vi.resetModules();
