...
import { planCoreBlowModelsJson } from "./models-config.plan.js";
...
async function buildModelsJsonFingerprint(params: {
  ...
  sourceConfigForSecrets: CoreBlowConfig;
  agentDir: string;
}): Promise<string> {
...
function resolveModelsConfigInput(config?: CoreBlowConfig): {
  config: CoreBlowConfig;
  sourceConfigForSecrets: CoreBlowConfig;
} {
...
export async function ensureCoreBlowModelsJson(
  config?: CoreBlowConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveCoreBlowAgentDir();
  ...
  const pending = withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planCoreBlowModelsJson({
      cfg,
      sourceConfigForSecrets: resolved.sourceConfigForSecrets,
      agentDir,
      env,
