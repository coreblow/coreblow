...
import { resolveCoreBlowDocsPath } from "../../docs-path.js";
...
import { createCoreBlowCodingTools, resolveToolLoopDetectionConfig } from "../../pi-tools.js";
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    const workspaceNotes = hookAdjustedBootstrapFiles.some(
      (file: any) => file.name === DEFAULT_BOOTSTRAP_FILENAME && !file.missing,
    )
      ? ["Reminder: commit your changes in this workspace after edits."]
      : undefined;

    const agentDir = params.agentDir ?? resolveCoreBlowAgentDir();
    ...
    const docsPath = await resolveCoreBlowDocsPath({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      if (transcriptPolicy.dropThinkingBlocks) {
        ...
        activeSession.agent.streamFn = (model: any, context: any, options: any) => {
      ...
      try {
        ...
        // double-compaction. See: https://github.com/coreblow/coreblow/issues/9282
        ...
        if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
          try {
            sessionManager.appendCustomEntry("coreblow:prompt-error", {
              timestamp: Date.now(),
              runId: params.runId,
              sessionId: params.sessionId,
              provider: params.provider,
      ...
      // See: https://github.com/coreblow/coreblow/issues/8643
