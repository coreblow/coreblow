...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("coreblow-bundled-");
  const workspaceDir = await createTempDir("coreblow-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: CoreBlowConfig) {
  const { bundledPluginsDir, workspaceDir } = await setupBundledDiffsPlugin();
  process.env.COREBLOW_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
