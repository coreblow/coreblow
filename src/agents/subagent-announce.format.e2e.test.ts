...
const { subagentRegistryMock } = vi.hoisted(() => ({
  subagentRegistryMock: {
    ...
    listSubagentRunsForController: vi.fn(
      (_controllerSessionKey: string): MockSubagentRun[] => [],
    ),
...
let configOverride: CoreBlowConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: CoreBlowConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/coreblow/coreblow/issues/31298
    previousFastTestEnv = process.env.COREBLOW_TEST_FAST;
  ...
  afterAll(() => {
    ...
    process.env.COREBLOW_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // COREBLOW_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("CoreBlow runtime context (internal):");
