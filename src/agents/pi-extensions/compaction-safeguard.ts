...
export default function compactionSafeguardExtension(api: ExtensionAPI): void {
  api.on("session_before_compact", async (event: any, ctx: any) => {
    ...
    const hasRealSummarizable = preparation.messagesToSummarize.some((message: any, index: any, messages: any) =>
      isRealConversationMessage(message, messages, index),
    );
    const hasRealTurnPrefix = preparation.turnPrefixMessages.some((message: any, index: any, messages: any) =>
      isRealConversationMessage(message, messages, index),
    );
