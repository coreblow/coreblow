...
class CoreBlowModelRegistry extends PiModelRegistryClass {
  ...
  override getAll(): Array<Model<Api>> {
    return super.getAll().map((entry: any) => normalizeRegistryModel(entry, this.agentDir));
  }

  override getAvailable(): Array<Model<Api>> {
    return super.getAvailable().map((entry: any) => normalizeRegistryModel(entry, this.agentDir));
  }
...
export function discoverModels(authStorage: PiAuthStorage, agentDir: string): PiModelRegistry {
  return new CoreBlowModelRegistry(authStorage, path.join(agentDir, "models.json"), agentDir);
}
