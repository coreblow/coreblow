...
function createConfig(): CoreBlowConfig {
  return {
    agents: {
      defaults: {
        sandbox: {
...
function createSession() {
  return {
    command: "ssh",
    configPath: path.join(os.tmpdir(), "coreblow-test-ssh-config"),
    host: "coreblow-sandbox",
  };
}
...
function createBackendSandboxConfig(params?: { binds?: string[]; target?: string }): SandboxConfig {
  return {
    ...
    workspaceRoot: "~/.coreblow/sandboxes",
    docker: {
      image: "img",
...
describe("ssh sandbox backend", () => {
  ...
  it("describes runtimes via the configured ssh target", async () => {
    const result = await sshSandboxBackendManager.describeRuntime({
      entry: {
        containerName: "coreblow-ssh-worker-abcd1234",
        backendId: "ssh",
        runtimeLabel: "coreblow-ssh-worker-abcd1234",
    ...
    expect(sshMocks.runSshSandboxCommand).toHaveBeenCalledWith(
      expect.objectContaining({
        remoteCommand: expect.stringContaining("/remote/coreblow/coreblow-ssh-agent-worker"),
      }),
    );
  });
  ...
  it("creates a remote-canonical backend that seeds once and reuses ssh exec", async () => {
    ...
    const backend = await createSshSandboxBackend({
      ...
      cfg: {
        ...
        browser: {
          enabled: false,
          image: "coreblow-browser",
          containerPrefix: "coreblow-browser-",
          network: "bridge",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
    ...
    expect(execSpec.argv.at(-1)).toContain("/remote/coreblow/coreblow-ssh-agent-worker");
