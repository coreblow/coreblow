...
import { resetWorkspaceTemplateDirCache, resolveWorkspaceTemplateDir } from "./workspace-templates.js";

const tempDirs: string[] = [];

async function makeTempRoot(): Promise<string> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-templates-"));
  tempDirs.push(root);
  return root;
}
...
describe("resolveWorkspaceTemplateDir", () => {
  ...
  it("returns fallback path when templates directory is missing", async () => {
    const root = await makeTempRoot();
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "coreblow" }));
