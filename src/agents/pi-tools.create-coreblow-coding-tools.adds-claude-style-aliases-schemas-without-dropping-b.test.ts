...
const defaultTools = createCoreBlowCodingTools({ senderIsOwner: true });
...
describe("createCoreBlowCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const disabledOpenAiTools = createCoreBlowCodingTools({
    ...
    const anthropicTools = createCoreBlowCodingTools({
      config: disabledConfig,
    ...
    const allowModelsConfig: CoreBlowConfig = {
      tools: {
        exec: {
    ...
    const allowed = createCoreBlowCodingTools({
    ...
    const denied = createCoreBlowCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createCoreBlowCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
