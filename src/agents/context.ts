...
let configuredConfig: CoreBlowConfig | undefined;
...
function isLikelyCoreBlowCliProcess(argv: string[] = process.argv): boolean {
  const entryBasename = path
    .basename(argv[1] ?? "")
    .trim()
    .toLowerCase();
  return (
    entryBasename === "coreblow" ||
    entryBasename === "coreblow.mjs" ||
...
function shouldEagerWarmContextWindowCache(argv: string[] = process.argv): boolean {
  // Keep this gate tied to the real CoreBlow CLI entrypoints.
  //
  ...
  // built plugin-sdk can call ensureCoreBlowModelsJson(), which cascades into
  // plugin discovery and breaks dist/source singleton assumptions.
  if (!isLikelyCoreBlowCliProcess(argv)) {
    return false;
  }
...
function primeConfiguredContextWindows(): CoreBlowConfig | undefined {
  if (configuredConfig) {
...
function ensureContextWindowCacheLoaded(): Promise<void> {
  ...
  loadPromise = (async () => {
    try {
      await (await loadModelsConfigRuntime()).ensureCoreBlowModelsJson(cfg);
    } catch {
