...
function findTool(name: string, config: CoreBlowConfig) {
  const allTools = createCoreBlowTools({ config, sandboxed: true });
...
async function prepareAndActivate(params: { config: CoreBlowConfig; env?: NodeJS.ProcessEnv }) {
...
describe("coreblow tools runtime web metadata wiring", () => {
