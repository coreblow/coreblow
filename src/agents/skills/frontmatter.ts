...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseCoreBlowManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
  const spec = applyCoreBlowManifestInstallCommonFields<SkillInstallSpec>(
    {
...
export function resolveCoreBlowMetadata(
  frontmatter: ParsedSkillFrontmatter,
): CoreBlowSkillMetadata | undefined {
  const metadataObj = resolveCoreBlowManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveCoreBlowManifestRequires(metadataObj);
  const install = resolveCoreBlowManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveCoreBlowManifestOs(metadataObj);
  return {
