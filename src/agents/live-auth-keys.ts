...
const GOOGLE_LIVE_SINGLE_KEY = "COREBLOW_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "COREBLOW_LIVE_ANTHROPIC_KEY",
    listVar: "COREBLOW_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "COREBLOW_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `COREBLOW_LIVE_${base}_KEY`;
