...
function appendToolCallAndResult(sm: { appendMessage: (...args: unknown[]) => void; getEntries: () => unknown[] }) {
...
function getPersistedToolResult(sm: { getEntries: () => unknown[] }) {
  const messages = sm
    .getEntries()
...
describe("tool_result_persist hook", () => {
  ...
  it("loads tool_result_persist hooks without breaking persistence", async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "coreblow-toolpersist-"));
  ...
  api.on("tool_result_persist", (event) => {
    ...
    // Dynamic import to avoid transitive extension tree resolution
    const { loadCoreBlowPlugins } = await import("../plugins/loader.js");
...
describe("before_message_write hook", () => {
  it("continues persistence when a before_message_write hook throws", async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "coreblow-before-write-"));
    process.env.COREBLOW_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
