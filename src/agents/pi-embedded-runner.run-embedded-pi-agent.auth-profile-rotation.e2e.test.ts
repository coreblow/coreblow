...
const installRunEmbeddedMocks = () => {
  ...
  vi.doMock("./models-config.js", async (importOriginal) => {
    ...
    return {
      ...mod,
      ensureCoreBlowModelsJson: vi.fn(async () => ({ wrote: false })),
    };
  });
};
...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): CoreBlowConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeAgentOverrideOnlyFallbackConfig = (agentId: string): CoreBlowConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeCopilotConfig = (): CoreBlowConfig =>
  ({
    models: {
      providers: {
        "github-copilot": {
...
describe("runEmbeddedPiAgent auth profile rotation", () => {
  ...
  it("logs structured failover decision metadata for overloaded assistant rotation", async () => {
    ...
    setLoggerOverrideFn({
      level: "trace",
      consoleLevel: "silent",
      file: path.join(os.tmpdir(), `coreblow-auth-rotation-${Date.now()}.log`),
    });
