...
import { __testing as coreblowToolsTesting, createCoreBlowTools } from "./coreblow-tools.js";
...
describe("sessions tools", () => {
  ...
  beforeEach(() => {
    ...
    coreblowToolsTesting.setDepsForTest({
      callGateway: (opts: unknown) => callGatewayMock(opts),
      config: TEST_CONFIG,
    });
  ...
  it("sessions_list filters kinds and includes messages", async () => {
    ...
    const tool = createCoreBlowTools().find((candidate) => candidate.name === "sessions_list");
    expect(tool).toBeDefined();
    if (!tool) {
  ...
  it("sessions_history filters tool messages by default", async () => {
    ...
    const tool = createCoreBlowTools().find((candidate) => candidate.name === "sessions_history");
    expect(tool).toBeDefined();
    if (!tool) {
