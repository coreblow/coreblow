...
async function collectSkillInstallScanWarnings(entry: SkillEntry): Promise<string[]> {
  ...
  try {
    ...
    if (summary.critical > 0) {
      ...
      warnings.push(
        `Skill "${skillName}" has ${summary.warn} suspicious code pattern(s). Run "coreblow security audit --deep" for details.`,
      );
    }
  } catch (err) {
    warnings.push(
      `Skill "${skillName}" code safety scan failed (${String(err)}). Installation continues; run "coreblow security audit --deep" after install.`,
    );
  }
...
export async function installSkill(params: SkillInstallRequest): Promise<SkillInstallResult> {
  ...
  const trustedInstallSources = new Set(["coreblow-bundled", "coreblow-managed", "coreblow-extra"]);
