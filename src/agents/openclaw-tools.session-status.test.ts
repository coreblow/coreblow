...
const buildStatusMessageMock = vi.hoisted(() => vi.fn(() => "CoreBlow\n🧠 Model: GPT-5.4"));
...
async function loadFreshCoreBlowToolsForSessionStatusTest() {
  vi.resetModules();
...
describe("session_status tool", () => {
  beforeEach(async () => {
    buildStatusMessageMock.mockClear();
    await loadFreshCoreBlowToolsForSessionStatusTest();
  });
  ...
  it("returns a status card for the current session", async () => {
    ...
    expect(details.statusText).toContain("CoreBlow");
