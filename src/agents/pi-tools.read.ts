...
function withToolResultText(
  ...
  const content = Array.isArray(result.content) ? result.content : [];
  let replaced = false;
  const nextContent: ToolContentBlock[] = content.map((block: any) => {
    if (
      !replaced &&
      block &&
      typeof block === "object" &&
...
export function createSandboxedReadTool(params: SandboxToolParams) {
  ...
  return createCoreBlowReadTool(base, {
...
export function createCoreBlowReadTool(
  base: AnyAgentTool,
  options?: CoreBlowReadToolOptions,
): AnyAgentTool {
  const patched = patchToolSchemaForClaudeCompatibility(base);
  return {
    ...patched,
    execute: async (toolCallId: any, params: any, signal: any) => {
