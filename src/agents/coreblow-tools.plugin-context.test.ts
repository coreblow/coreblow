...
let createCoreBlowCodingTools: typeof import("./pi-tools.js").createCoreBlowCodingTools;

describe("createCoreBlowTools plugin context", () => {
  beforeEach(async () => {
    resolvePluginToolsMock.mockClear();
    vi.resetModules();
    ({ createCoreBlowTools } = await import("./coreblow-tools.js"));
    ({ createCoreBlowCodingTools } = await import("./pi-tools.js"));
  });
  ...
  it("does not inject ambient thread defaults into plugin tools", async () => {
    ...
    const first = createCoreBlowTools({
      config: {} as never,
      agentThreadId: "111.222",
    }).find((tool) => tool.name === "plugin-thread-default");
    const second = createCoreBlowTools({
      config: {} as never,
  ...
  it("does not inject messageThreadId defaults for missing params objects", async () => {
    ...
    const wrapped = createCoreBlowTools({
      config: {} as never,
      agentThreadId: "77",
