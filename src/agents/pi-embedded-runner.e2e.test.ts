...
beforeAll(async () => {
  ...
  e2eWorkspace = await createEmbeddedPiRunnerTestWorkspace("coreblow-embedded-agent-");
...
const runWithOrphanedSingleUserMessage = async (text: string, sessionKey: string) => {
  ...
  // Write a user message directly as a JSONL entry (CB stub lacks appendMessage)
  await fs.appendFile(
    sessionFile,
    JSON.stringify({
      type: "message",
...
describe("runEmbeddedPiAgent", () => {
  ...
  it(
    ...
    async () => {
      ...
      // Write seed messages directly as JSONL entries (CB stub lacks appendMessage)
      await fs.appendFile(
        sessionFile,
        JSON.stringify({
          type: "message",
