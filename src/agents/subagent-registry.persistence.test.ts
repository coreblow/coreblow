...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["COREBLOW_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-subagent-"));
  ...
  it("uses isolated temp state when COREBLOW_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "coreblow-test-state"));
  });
});
