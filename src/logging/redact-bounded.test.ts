/**
 * CoreBlow — Redact Bounded Tests
 *
 * Tests for replacePatternBounded chunked regex replacement.
 */
...
describe('replacePatternBounded', () => {
    it('replaces pattern in short text (below threshold)', () => {
        const result = replacePatternBounded('my-secret-key-123', /secret/g, '***');
        expect(result).toBe('my-***-key-123');
    });

    it('replaces all occurrences', () => {
        const result = replacePatternBounded('abc abc abc', /abc/g, 'x');
        expect(result).toBe('x x x');
    });

    it('handles text below chunk threshold normally', () => {
        const text = 'short text with secret';
        const result = replacePatternBounded(text, /secret/g, '[REDACTED]');
    ...
    it('chunks large text above threshold', () => {
        ...
        const result = replacePatternBounded(chunk, /token/g, '***', {
            chunkThreshold: 100,
            chunkSize: 50,
        });
    ...
    it('handles no matches', () => {
        const result = replacePatternBounded('clean text', /secret/g, '***');
        expect(result).toBe('clean text');
    });

    it('handles empty text', () => {
        expect(replacePatternBounded('', /x/g, 'y')).toBe('');
    });
});
