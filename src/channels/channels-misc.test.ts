...
describe("WA_WEB_AUTH_DIR", () => {
  ...
  it("resolves lazily and caches across the legacy and channels/web entrypoints", async () => {
    const resolveWaWebAuthDir = vi.fn(() => "/tmp/coreblow-whatsapp-auth");
    ...
    expect(String(channelWeb.WA_WEB_AUTH_DIR)).toBe("/tmp/coreblow-whatsapp-auth");
    expect(String(webEntry.WA_WEB_AUTH_DIR)).toBe("/tmp/coreblow-whatsapp-auth");
