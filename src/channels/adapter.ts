...
/**
 * Bridge a ChannelAdapter instance into a ChannelPlugin object.
 * This allows legacy CB adapters to participate in the plugin registry.
 */
export function bridgeAdapterToPlugin(
    ...
    const meta: ChannelMeta = {
        id,
        label: adapter.name,
        selectionLabel: adapter.name,
        docsPath: `channels/${id}`,
        blurb: `${adapter.name} channel adapter.`,
        ...overrides?.meta,
    };
        ...
        heartbeat: {
            checkReady: async () => {
                try {
                    ...
                    return { ok: h.connected, reason: h.connected ? 'connected' : (h.error ?? 'disconnected') };
                } catch {
...
// ============================================================
// Adapter Registry (with Plugin Bridge)
// ============================================================
 ...
 * Also bridges the adapter into the bundled ChannelPlugin registry
 * if no plugin is already registered for this channel.
 */
