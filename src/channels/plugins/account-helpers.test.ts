...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): CoreBlowConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as CoreBlowConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedAccountIdsCase(params: {
  resolve: (cfg: CoreBlowConfig) => string[];
  input: CoreBlowConfig;
  expected: string[];
}) {
  expect(params.resolve(params.input)).toEqual(params.expected);
}

function expectResolvedDefaultAccountCase(input: CoreBlowConfig, expected: string) {
