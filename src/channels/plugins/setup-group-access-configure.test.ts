...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: CoreBlowConfig, policy: ChannelAccessPolicy) => CoreBlowConfig;
  resolveAllowlist: (params: { cfg: CoreBlowConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: CoreBlowConfig; resolved: TResolved }) => CoreBlowConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: CoreBlowConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: CoreBlowConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: CoreBlowConfig, policy: ChannelAccessPolicy): CoreBlowConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: CoreBlowConfig, policy: ChannelAccessPolicy): CoreBlowConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: CoreBlowConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: CoreBlowConfig; resolved: string[] }) => {
      calls.push("apply");
