...
export function setTopLevelChannelDmPolicyWithAllowFrom(params: {
  ...
  channel: string;
  dmPolicy: DmPolicy;
  getAllowFrom?: (cfg: CoreBlowConfig) => Array<string | number> | undefined;
}): CoreBlowConfig {
  const channelConfig =
    (params.cfg.channels?.[params.channel] as Record<string, unknown> | undefined) ?? {};
...
export function createTopLevelChannelDmPolicy(params: {
  ...
  policyKey: string;
  allowFromKey: string;
  getCurrent: (cfg: CoreBlowConfig) => DmPolicy;
...
export function createTopLevelChannelDmPolicySetter(params: {
  channel: string;
  getAllowFrom?: (cfg: CoreBlowConfig) => Array<string | number> | undefined;
}): (cfg: CoreBlowConfig, dmPolicy: DmPolicy) => CoreBlowConfig {
  return (cfg, dmPolicy) =>
...
export function createTopLevelChannelAllowFromSetter(params: {
  channel: string;
  enabled?: boolean;
}): (cfg: CoreBlowConfig, allowFrom: string[]) => CoreBlowConfig {
...
export function createTopLevelChannelGroupPolicySetter(params: {
  channel: string;
  enabled?: boolean;
}): (cfg: CoreBlowConfig, groupPolicy: "open" | "allowlist" | "disabled") => CoreBlowConfig {
...
export async function runSingleChannelSecretStep(params: {
  ...
  applyUseEnv?: (cfg: CoreBlowConfig) => CoreBlowConfig | Promise<CoreBlowConfig>;
  applySet?: (
    cfg: CoreBlowConfig,
    value: SecretInput,
    resolvedValue: string,
  ) => CoreBlowConfig | Promise<CoreBlowConfig>;
}): Promise<{
  cfg: CoreBlowConfig;
...
export async function promptSingleChannelSecretInput(params: {
  ...
  const selectedMode = await resolveSecretInputModeForEnvSelection({
    ...
    copy: {
      ...
      plaintextHint: "Stores the credential directly in CoreBlow config",
  ...
  const resolved = await promptSecretRefForSetup({
    ...
    copy: {
      ...
      envVarPlaceholder: params.preferredEnvVar ?? "COREBLOW_SECRET",
      envVarFormatError:
        'Use an env var name like "COREBLOW_SECRET" (uppercase letters, numbers, underscores).',
      noProvidersMessage:
...
export async function promptParsedAllowFromForAccount<TConfig extends CoreBlowConfig>(params: {
  cfg: TConfig;
  accountId?: string;
  defaultAccountId: string;
...
export function createPromptParsedAllowFromForAccount<TConfig extends CoreBlowConfig>(params: {
...
export function createTopLevelChannelParsedAllowFromPrompt(params: {
  ...
  placeholder: string;
  parseEntries: (raw: string) => ParsedAllowFromResult;
  getExistingAllowFrom?: (cfg: CoreBlowConfig) => Array<string | number>;
  mergeEntries?: (params: { existing: Array<string | number>; parsed: string[] }) => string[];
}): NonNullable<ChannelSetupDmPolicy["promptAllowFrom"]> {
...
export async function promptLegacyChannelAllowFromForAccount<TAccount>(params: {
  ...
  accountId?: string;
  defaultAccountId: string;
  resolveAccount: (cfg: CoreBlowConfig, accountId: string) => TAccount;
  resolveExisting: (account: TAccount, cfg: CoreBlowConfig) => Array<string | number>;
