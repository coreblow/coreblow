...
type SetupContractEntry = {
  ...
  cases: Array<{
    ...
    beforeTest?: () => void;
    assertPatchedConfig?: (cfg: CoreBlowConfig) => void;
    assertResolvedAccount?: (account: unknown, cfg: CoreBlowConfig) => void;
  }>;
};
...
setBundledChannelRuntime("line", {
  channel: {
    line: {
      listLineAccountIds,
      resolveDefaultLineAccountId,
      resolveLineAccount: ({ cfg, accountId }: { cfg: CoreBlowConfig; accountId?: string }) =>
...
const matrixSessionBindingStateDir = fs.mkdtempSync(
  path.join(os.tmpdir(), "coreblow-matrix-session-binding-contract-"),
);
