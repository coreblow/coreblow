  ...
  it.each([
    {
      ...
      setup: () => {
        const dir = fs.mkdtempSync(path.join(os.tmpdir(), "coreblow-catalog-"));
        const catalogPath = path.join(dir, "catalog.json");
        writeCatalogFile(
          catalogPath,
          createCatalogEntry({
            packageName: "@coreblow/demo-channel",
    ...
    },
    {
      ...
      setup: () => {
        const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "coreblow-channel-catalog-state-"));
    ...
    },
    {
      ...
      setup: () => {
        const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "coreblow-catalog-state-"));
  ...
  it.each([
    {
      ...
      setup: () => {
        const home = fs.mkdtempSync(path.join(os.tmpdir(), "coreblow-catalog-home-"));
        const catalogPath = path.join(home, "catalog.json");
        writeCatalogFile(
          catalogPath,
          createCatalogEntry({
            packageName: "@coreblow/env-demo-channel",
        ...
        return {
          env: {
            ...process.env,
            COREBLOW_PLUGIN_CATALOG_PATHS: "~/catalog.json",
            COREBLOW_HOME: home,
            HOME: home,
          },
    ...
    },
    {
      ...
      setup: () => {
        ...
        writeCatalogFile(
          catalogPath,
          createCatalogEntry({
            packageName: "@coreblow/default-env-demo",
