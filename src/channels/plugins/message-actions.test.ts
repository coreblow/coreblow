...
describe("message action capability checks", () => {
  ...
  it("aggregates capabilities across plugins", () => {
    ...
    expect(listChannelMessageCapabilities({} as CoreBlowConfig).toSorted()).toEqual([
      "buttons",
      "cards",
      "interactive",
    ]);
    expect(channelSupportsMessageCapability({} as CoreBlowConfig, "interactive")).toBe(true);
    expect(channelSupportsMessageCapability({} as CoreBlowConfig, "buttons")).toBe(true);
    expect(channelSupportsMessageCapability({} as CoreBlowConfig, "cards")).toBe(true);
  });
  ...
  it("checks per-channel capabilities", () => {
    ...
    expect(
      channelSupportsMessageCapabilityForChannel(
        { cfg: {} as CoreBlowConfig, channel: "demo-buttons" },
        "interactive",
      ),
    ).toBe(true);
    expect(
      channelSupportsMessageCapabilityForChannel(
        { cfg: {} as CoreBlowConfig, channel: "demo-cards" },
        "interactive",
      ),
    ).toBe(false);
    expect(
    ...
    expect(channelSupportsMessageCapabilityForChannel({ cfg: {} as CoreBlowConfig }, "cards")).toBe(
      false,
    );
  });
  ...
  it("uses unified message tool discovery for actions, capabilities, and schema", () => {
    ...
    expect(listChannelMessageActions({} as CoreBlowConfig)).toEqual(["send", "broadcast", "react"]);
    expect(listChannelMessageCapabilities({} as CoreBlowConfig)).toEqual(["interactive"]);
    expect(
      resolveChannelMessageToolSchemaProperties({
        cfg: {} as CoreBlowConfig,
  ...
  it("skips crashing action/capability discovery paths and logs once", () => {
    ...
    expect(listChannelMessageActions({} as CoreBlowConfig)).toEqual(["send", "broadcast"]);
    expect(listChannelMessageCapabilities({} as CoreBlowConfig)).toEqual([]);
