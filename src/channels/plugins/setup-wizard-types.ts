...
export type ChannelOnboardingPostWriteHook = {
  channel: ChannelId;
  accountId: string;
  run: (ctx: { cfg: CoreBlowConfig; runtime: RuntimeEnv }) => Promise<void> | void;
};
...
export type ChannelSetupDmPolicy = {
  ...
  getCurrent: (cfg: CoreBlowConfig, accountId?: string) => DmPolicy;
  setPolicy: (cfg: CoreBlowConfig, policy: DmPolicy, accountId?: string) => CoreBlowConfig;
