...
function createOpenAIFileRuntimeConfig(secretFile: string): CoreBlowConfig {
  return asConfig({
    secrets: {
      providers: {
        default: {
...
describe("secrets runtime snapshot integration", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv([
      ...
      "COREBLOW_DISABLE_PLUGIN_DISCOVERY_CACHE",
      "COREBLOW_VERSION",
    ]);
    delete process.env.COREBLOW_BUNDLED_PLUGINS_DIR;
    process.env.COREBLOW_DISABLE_PLUGIN_DISCOVERY_CACHE = "1";
    delete process.env.COREBLOW_VERSION;
  });
    ...
    await withEnvAsync(
      ...
      async () => {
        const prepared = await prepareSecretsRuntimeSnapshot({
          ...
          agentDirs: ["/tmp/coreblow-agent-main"],
          loadAuthStore: () =>
        ...
        expect(
          ensureAuthProfileStore("/tmp/coreblow-agent-main").profiles["openai:default"],
        ).toMatchObject({
          type: "api_key",
          key: "sk-runtime",
        });
  ...
  it("keeps active secrets runtime snapshots resolved after config writes", async () => {
    ...
    await withTempHome("coreblow-secrets-runtime-write-", async (home) => {
  ...
  it("keeps last-known-good runtime snapshot active when refresh fails after a write", async () => {
    ...
    await withTempHome("coreblow-secrets-runtime-refresh-fail-", async (home) => {
      const { secretFile, agentDir } = await createOpenAIFileRuntimeFixture(home);
  ...
  it(
    ...
    async () => {
      await withTempHome("coreblow-secrets-runtime-web-reload-lkg-", async (home) => {
        const prepared = await prepareSecretsRuntimeSnapshot({
          config: asConfig({
            tools: {
              web: {
        ...
        const persistedConfig = JSON.parse(
          await fs.readFile(path.join(home, ".coreblow", "coreblow.json"), "utf8"),
        ) as CoreBlowConfig;
  ...
  it("recomputes config-derived agent dirs when refreshing active secrets runtime snapshots", async () => {
    await withTempHome("coreblow-secrets-runtime-agent-dirs-", async (home) => {
      const mainAgentDir = path.join(home, ".coreblow", "agents", "main", "agent");
      const opsAgentDir = path.join(home, ".coreblow", "agents", "ops", "agent");
