...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("coreblow doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with COREBLOW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with COREBLOW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.COREBLOW_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://coreblow-gateway.ai:18789",
      onConnectError,
    });
