...
import { ensureCoreBlowModelsJson } from "../agents/models-config.js";
import { isRateLimitErrorMessage } from "../agents/pi-embedded-helpers/errors.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { clearRuntimeConfigSnapshot, loadConfig } from "../config/config.js";
import type { ModelsConfig, CoreBlowConfig, ModelProviderConfig } from "../config/types.js";
...
const ZAI_FALLBACK = isTruthyEnvValue(process.env.COREBLOW_LIVE_GATEWAY_ZAI_FALLBACK);
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const PROVIDERS = parseFilter(process.env.COREBLOW_LIVE_GATEWAY_PROVIDERS);
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.COREBLOW_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
const GATEWAY_LIVE_HEARTBEAT_MS = Math.max(
  1_000,
  toInt(process.env.COREBLOW_LIVE_GATEWAY_HEARTBEAT_MS, 30_000),
);
...
const QUIET_LIVE_LOGS = process.env.COREBLOW_LIVE_TEST_QUIET !== "0";

const describeLive = isLiveTestEnabled(["COREBLOW_LIVE_GATEWAY"]) ? describe : describe.skip;
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayMax = toInt(process.env.COREBLOW_LIVE_GATEWAY_MAX_MODELS, -1);
  if (gatewayMax >= 0) {
    return gatewayMax;
  }
  // Reuse shared live-model cap when gateway-specific cap is not provided.
  return Math.max(0, toInt(process.env.COREBLOW_LIVE_MAX_MODELS, 0));
}
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<CoreBlowConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const previous = {
    configPath: process.env.COREBLOW_CONFIG_PATH,
    token: process.env.COREBLOW_GATEWAY_TOKEN,
    skipChannels: process.env.COREBLOW_SKIP_CHANNELS,
    skipGmail: process.env.COREBLOW_SKIP_GMAIL_WATCHER,
    skipCron: process.env.COREBLOW_SKIP_CRON,
    skipCanvas: process.env.COREBLOW_SKIP_CANVAS_HOST,
    disableBonjour: process.env.COREBLOW_DISABLE_BONJOUR,
    logLevel: process.env.COREBLOW_LOG_LEVEL,
    agentDir: process.env.COREBLOW_AGENT_DIR,
  ...
  if (QUIET_LIVE_LOGS) {
    process.env.COREBLOW_DISABLE_BONJOUR = "1";
    process.env.COREBLOW_LOG_LEVEL = "silent";
  }

  const token = `test-${randomUUID()}`;
  process.env.COREBLOW_GATEWAY_TOKEN = token;
  const agentId = "dev";

  const hostAgentDir = resolveCoreBlowAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-live-state-"));
  process.env.COREBLOW_STATE_DIR = tempStateDir;
  ...
  process.env.COREBLOW_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.coreblow-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveCoreBlowAgentDir();
  const sanitizedCfg: CoreBlowConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-live-"));
  const tempConfigPath = path.join(tempDir, "coreblow.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.COREBLOW_CONFIG_PATH = tempConfigPath;
  ...
  try {
        ...
        try {
          ...
          for (
            ...
            toolText = await requestGatewayAgentText({
              ...
              modelKey,
              message: strictReply
                ? "CoreBlow live tool probe (local, safe): " +
    ...
    process.env.COREBLOW_CONFIG_PATH = previous.configPath;
    process.env.COREBLOW_GATEWAY_TOKEN = previous.token;
    process.env.COREBLOW_SKIP_CHANNELS = previous.skipChannels;
    process.env.COREBLOW_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.COREBLOW_SKIP_CRON = previous.skipCron;
    process.env.COREBLOW_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.COREBLOW_DISABLE_BONJOUR = previous.disableBonjour;
    process.env.COREBLOW_LOG_LEVEL = previous.logLevel;
    process.env.COREBLOW_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.COREBLOW_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      const rawModels = process.env.COREBLOW_LIVE_GATEWAY_MODELS?.trim();
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via COREBLOW_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
        );
      }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.coreblow-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
