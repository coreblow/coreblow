...
const { createCoreBlowTools } = await import("../agents/coreblow-tools.js");
...
type SessionSendTool = ReturnType<typeof createCoreBlowTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createCoreBlowTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["COREBLOW_GATEWAY_PORT", "COREBLOW_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.COREBLOW_GATEWAY_PORT = String(gatewayPort);
  process.env.COREBLOW_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("COREBLOW_CONFIG_PATH missing in gateway test environment");
      }
      ...
      const tool = createCoreBlowTools({
        config: {
          tools: {
            sessions: {
              visibility: "all",
