...
let lastCreateCoreBlowToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/coreblow-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateCoreBlowToolsContext?.agentTo,
          agentThreadId: lastCreateCoreBlowToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createCoreBlowTools: (ctx: Record<string, unknown>) => {
      lastCreateCoreBlowToolsContext = ctx;
      return tools;
    },
  };
});
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateCoreBlowToolsContext?.allowMediaInvokeCommands).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateCoreBlowToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-coreblow-message-to": "channel:24514",
        "x-coreblow-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
