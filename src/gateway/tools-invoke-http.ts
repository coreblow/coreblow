...
import { createCoreBlowTools } from "../agents/coreblow-tools.js";
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-coreblow-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-coreblow-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-coreblow-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-coreblow-thread-id")?.trim() || undefined;
  ...
  const allTools = createCoreBlowTools({
