...
export function attachCoreBlowTranscriptMeta(
  message: unknown,
  ...
  const existing =
    record.__coreblow && typeof record.__coreblow === "object" && !Array.isArray(record.__coreblow)
      ? (record.__coreblow as Record<string, unknown>)
      : {};
  return {
    ...record,
    __coreblow: {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      if (parsed?.message) {
        messageSeq += 1;
        messages.push(
          attachCoreBlowTranscriptMeta(parsed.message, {
      ...
      const model =
        typeof message.model === "string"
          ...
          : typeof parsed.model === "string"
            ? parsed.model.trim()
            : undefined;
      const isDeliveryMirror = modelProvider === "coreblow" && model === "delivery-mirror";
