...
describe("gateway pre-auth hardening", () => {
  ...
  it("rejects excess simultaneous unauthenticated sockets from the same client ip", async () => {
    ...
    process.env.COREBLOW_TEST_MAX_PREAUTH_CONNECTIONS_PER_IP = "1";
    cleanupEnv.push(() => {
      if (previous === undefined) {
        delete process.env.COREBLOW_TEST_MAX_PREAUTH_CONNECTIONS_PER_IP;
      } else {
        process.env.COREBLOW_TEST_MAX_PREAUTH_CONNECTIONS_PER_IP = previous;
      }
    });
