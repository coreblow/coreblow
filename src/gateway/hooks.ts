...
export function resolveHooksConfig(cfg: CoreBlowConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: CoreBlowConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-coreblow-token"] === "string"
      ? req.headers["x-coreblow-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-coreblow-idempotency-key"]) ||
