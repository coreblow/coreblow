...
type GatewaySecretDefaults = NonNullable<CoreBlowConfig["secrets"]>["defaults"];
 ...
 * This prevents literal placeholder strings like `${COREBLOW_GATEWAY_TOKEN}` from being
...
export function readGatewayTokenEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  return trimToUndefined(env.COREBLOW_GATEWAY_TOKEN);
}

export function readGatewayPasswordEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  return trimToUndefined(env.COREBLOW_GATEWAY_PASSWORD);
}
