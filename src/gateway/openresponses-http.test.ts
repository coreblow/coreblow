...
async function writeGatewayConfig(config: Record<string, unknown>) {
  ...
  if (!configPath) {
    throw new Error("COREBLOW_CONFIG_PATH is required for gateway config tests");
  }
  ...
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    try {
      ...
      const resMissingAuth = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        ...
        body: JSON.stringify({ model: "coreblow", input: "hi" }),
      });
      ...
      expect(invalidModelJson.error?.message).toBe(
        "Invalid `model`. Use `coreblow` or `coreblow/<agentId>`.",
      );
      ...
      const resHeader = await postResponses(
        port,
        { model: "coreblow", input: "hi" },
        { "x-coreblow-agent-id": "beta" },
      );
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resModel = await postResponses(port, { model: "coreblow/beta", input: "hi" });
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resDefaultAlias = await postResponses(port, { model: "coreblow/default", input: "hi" });
      ...
      const resChannelHeader = await postResponses(
        port,
        { model: "coreblow", input: "hi" },
        { "x-coreblow-message-channel": "custom-client-channel" },
      );
      ...
      const resModelOverride = await postResponses(
        ...
        { "x-coreblow-model": "openai/gpt-5.4" },
      );
      ...
      expect(invalidOverrideJson.error?.message).toBe("Invalid `x-coreblow-model`.");
      expect(agentCommand).toHaveBeenCalledTimes(0);
