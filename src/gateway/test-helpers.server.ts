...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".coreblow-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".coreblow-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".coreblow-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>coreblow-test-control-ui</title>\n",
    "utf-8",
  );
...
const CONNECT_CHALLENGE_NONCE_KEY = "__coreblowTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__coreblowTestConnectChallengeTracked";
