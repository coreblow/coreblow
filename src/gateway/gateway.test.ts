...
async function writeWorkspacePlugin(params: {
  ...
  const pluginDir = path.join(params.workspaceDir, ".coreblow", "extensions", params.id);
...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "coreblow");
      await fs.mkdir(workspaceDir, { recursive: true });

      const configDir = path.join(tempHome, ".coreblow");
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-gw-http-tools-home-"));
      process.env.HOME = tempHome;
      process.env.COREBLOW_STATE_DIR = path.join(tempHome, ".coreblow");
      delete process.env.COREBLOW_CONFIG_PATH;
      process.env.COREBLOW_SKIP_CHANNELS = "1";
...
`.trimStart(),
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-wizard-home-"));
      process.env.HOME = tempHome;
