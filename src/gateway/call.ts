...
function loadGatewayConfig(): CoreBlowConfig {
  const loadConfigFn =
...
function resolveGatewayPortValue(config?: CoreBlowConfig, env?: NodeJS.ProcessEnv): number {
...
function gatewaySecretInputPathCanWin(params: {
  ...
  const sentinel = `__COREBLOW_GATEWAY_SECRET_REF_PROBE_${params.path.replaceAll(".", "_")}__`;
