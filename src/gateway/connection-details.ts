...
type GatewayConnectionDetailResolvers = {
  ...
  resolveGatewayPort?: (cfg?: CoreBlowConfig, env?: NodeJS.ProcessEnv) => number;
};
...
export function buildGatewayConnectionDetailsWithResolvers(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : trimToUndefined(process.env.COREBLOW_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.COREBLOW_ALLOW_INSECURE_PRIVATE_WS === "1";
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set COREBLOW_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: coreblow doctor --fix",
