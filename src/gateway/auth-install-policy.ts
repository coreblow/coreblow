...
type GatewayInstallAuthMode = NonNullable<NonNullable<CoreBlowConfig["gateway"]>["auth"]>["mode"];
...
function hasConfiguredGatewayPasswordForInstall(cfg: CoreBlowConfig): boolean {
...
function hasDurableGatewayPasswordEnvForInstall(
  ...
  return Boolean(durableServiceEnv.COREBLOW_GATEWAY_PASSWORD?.trim());
}
