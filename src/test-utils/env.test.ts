...
describe("env test utils", () => {
  it("captureEnv restores mutated keys", () => {
    const keyA = "COREBLOW_ENV_TEST_A";
    const keyB = "COREBLOW_ENV_TEST_B";
  ...
  it("captureFullEnv restores added keys and baseline values", () => {
    const key = "COREBLOW_ENV_TEST_ADDED";
  ...
  it("withEnv applies values only inside callback", () => {
    const key = "COREBLOW_ENV_TEST_SYNC";
  ...
  it("withEnv restores values when callback throws", () => {
    const key = "COREBLOW_ENV_TEST_SYNC_THROW";
    const prev = process.env[key];
  ...
  it("withEnv can delete a key only inside callback", () => {
    const key = "COREBLOW_ENV_TEST_SYNC_DELETE";
    const prev = process.env[key];
  ...
  it("withEnvAsync restores values when callback throws", async () => {
    const key = "COREBLOW_ENV_TEST_ASYNC";
    const prev = process.env[key];
  ...
  it("withEnvAsync applies values only inside async callback", async () => {
    const key = "COREBLOW_ENV_TEST_ASYNC_OK";
    const prev = process.env[key];
  ...
  it("withEnvAsync can delete a key only inside callback", async () => {
    const key = "COREBLOW_ENV_TEST_ASYNC_DELETE";
    const prev = process.env[key];
    process.env[key] = "outer";
  ...
  it("createPathResolutionEnv clears leaked path overrides before applying explicit ones", () => {
    const homeDir = path.join(path.sep, "tmp", "coreblow-home");
    const resolvedHomeDir = path.resolve(homeDir);
    const previousCoreBlowHome = process.env.COREBLOW_HOME;
    const previousStateDir = process.env.COREBLOW_STATE_DIR;
    const previousBundledDir = process.env.COREBLOW_BUNDLED_PLUGINS_DIR;
    process.env.COREBLOW_HOME = "/srv/coreblow-home";
    process.env.COREBLOW_STATE_DIR = "/srv/coreblow-state";
    process.env.COREBLOW_BUNDLED_PLUGINS_DIR = "/srv/coreblow-bundled";
    ...
    try {
      ...
      expect(env.COREBLOW_HOME).toBeUndefined();
      expect(env.COREBLOW_BUNDLED_PLUGINS_DIR).toBeUndefined();
      expect(env.COREBLOW_STATE_DIR).toBe("~/state");
    } finally {
      restoreEnvKey("COREBLOW_HOME", previousCoreBlowHome);
      restoreEnvKey("COREBLOW_STATE_DIR", previousStateDir);
      restoreEnvKey("COREBLOW_BUNDLED_PLUGINS_DIR", previousBundledDir);
    }
  });
  ...
  it("withPathResolutionEnv only applies the explicit path env inside the callback", () => {
    ...
    try {
      const seen = withPathResolutionEnv(
        homeDir,
        { COREBLOW_BUNDLED_PLUGINS_DIR: "~/bundled" },
        (env) => ({
          processHome: process.env.HOME,
          processCoreBlowHome: process.env.COREBLOW_HOME,
          processBundledDir: process.env.COREBLOW_BUNDLED_PLUGINS_DIR,
          envBundledDir: env.COREBLOW_BUNDLED_PLUGINS_DIR,
        }),
      );

      expect(seen).toEqual({
        processHome: resolvedHomeDir,
        processCoreBlowHome: undefined,
        processBundledDir: "~/bundled",
        envBundledDir: "~/bundled",
      });
      expect(process.env.COREBLOW_HOME).toBe("/srv/coreblow-home");
    } finally {
      restoreEnvKey("COREBLOW_HOME", previousCoreBlowHome);
    }
  });
