...
describe("env test utils", () => {
  ...
  it("createPathResolutionEnv clears leaked path overrides before applying explicit ones", () => {
    ...
    process.env.OPENCLAW_HOME = "/srv/coreblow-home";
    process.env.OPENCLAW_STATE_DIR = "/srv/coreblow-state";
    process.env.OPENCLAW_BUNDLED_PLUGINS_DIR = "/srv/coreblow-bundled";
    ...
    try {
      ...
      restoreEnvKey("OPENCLAW_HOME", previousCoreBlowHome);
    ...
    try {
      const seen = withPathResolutionEnv(
        ...
        (env) => ({
          ...
          processCoreBlowHome: process.env.OPENCLAW_HOME,
      ...
      expect(process.env.OPENCLAW_HOME).toBe("/srv/coreblow-home");
    } finally {
      restoreEnvKey("OPENCLAW_HOME", previousCoreBlowHome);
    }
  });
