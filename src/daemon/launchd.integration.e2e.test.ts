...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `coreblow-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      COREBLOW_LAUNCHD_LABEL: `ai.coreblow.launchd-int-${testId}`,
      COREBLOW_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
