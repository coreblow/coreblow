...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  const markerKeys = ["coreblow_service_marker"];
  const kindKeys = ["coreblow_service_kind"];
...
function isCoreBlowGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  return label.startsWith("ai.coreblow.");
}
...
function isCoreBlowGatewayTaskName(name: string): boolean {
  ...
  return normalized === defaultName || normalized.startsWith("coreblow gateway");
}
...
async function scanLaunchdDir(params: {
  ...
  for (const { name: labelFromName, fullPath, contents } of candidates) {
    ...
    results.push({
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "coreblow" || isLegacyLabel(label),
    });
  }
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (isCoreBlowGatewayTaskName(name)) {
        continue;
      }
