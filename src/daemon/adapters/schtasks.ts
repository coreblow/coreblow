/**
 * src/daemon/adapters/schtasks.ts
 * Adapter for managing Windows Task Scheduler.
 * Ported from CoreBlow daemon/schtasks.ts.
 */
...
// Basic tree kill fallback until kill-tree is ported specifically
...
function resolveTaskName(env: GatewayServiceEnv): string {
    const override = env.COREBLOW_WINDOWS_TASK_NAME?.trim() ?? env.COREBLOW_WINDOWS_TASK_NAME?.trim();
...
export function resolveTaskScriptPath(env: GatewayServiceEnv): string {
    const override = env.COREBLOW_TASK_SCRIPT?.trim() ?? env.COREBLOW_TASK_SCRIPT?.trim();
    if (override) return override;

    const scriptName = env.COREBLOW_TASK_SCRIPT_NAME?.trim() ?? env.COREBLOW_TASK_SCRIPT_NAME?.trim() ?? "gateway.cmd";
...
async function assertSchtasksAvailable() {
    ...
    throw new Error(`schtasks unavailable: ${(res.stderr || res.stdout).trim() || "unknown error"}`);
}
...
function resolveConfiguredGatewayPort(env: GatewayServiceEnv): number | null {
    const raw = env.COREBLOW_GATEWAY_PORT?.trim() ?? env.COREBLOW_GATEWAY_PORT?.trim();
    if (!raw) return null;
...
async function waitForGatewayPortRelease(port: number, timeoutMs = 5_000): Promise<boolean> {
    ...
    while (Date.now() < deadline) {
        const diagnostics = await inspectPortUsage(port).catch(() => null);
        if (diagnostics?.status === "free") return true;
        await sleep(250);
    }
    return false;
}
...
async function activateScheduledTask(params: {
    ...
    const taskDescription = params.description ?? "CoreBlow Gateway";
...
export async function readScheduledTaskRuntime(
    ...
    if (res.code !== 0) {
        ...
        const isMissing = /specified task name (.*) could not be found/i.test(detail);
        return {
            status: isMissing ? "stopped" : "unknown",
            detail: detail || undefined,
            missingUnit: isMissing,
        };
    }
    const parsedLine = parseSchtasksQuery(res.stdout);
    const { status, detail } = deriveScheduledTaskRuntimeStatus(parsedLine);
    return {
        status,
        detail,
        lastRunTime: parsedLine.lastRunTime,
        lastRunResult: parsedLine.lastRunResult,
    };
}
