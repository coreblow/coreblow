...
describe("runCapability video provider wiring", () => {
  it("merges video baseUrl and headers with entry precedence", async () => {
    ...
    await withVideoFixture("coreblow-video-merge", async ({ ctx, media, cache }) => {
      const cfg = {
        models: {
          providers: {
            moonshot: {
  ...
  it("auto-selects moonshot for video when google is unavailable", async () => {
    const isolatedAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-video-agent-"));
    try {
      await withEnvAsync(
        {
        ...
        async () => {
          await withVideoFixture("coreblow-video-auto-moonshot", async ({ ctx, media, cache }) => {
            const cfg = {
              models: {
                providers: {
                  moonshot: {
