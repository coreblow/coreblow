...
const TEMP_MEDIA_PREFIX = "coreblow-media-";
...
function createGroqAudioConfig(): CoreBlowConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): CoreBlowConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): CoreBlowConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: CoreBlowConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredCoreBlowTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        COREBLOW_AGENT_DIR: isolatedAgentDir,
