...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set COREBLOW_GATEWAY_TOKEN/COREBLOW_GATEWAY_PASSWORD, pass --token/--password,",
...
export class GatewayChatClient {
  ...
  constructor(connection: ResolvedGatewayConnection) {
    ...
    this.client = new GatewayClient({
      ...
      clientDisplayName: "coreblow-tui",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.COREBLOW_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.COREBLOW_GATEWAY_PASSWORD);
