...
describe("resolveInitialTuiAgentId", () => {
  const cfg: CoreBlowConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/coreblow" },
        { id: "ops", workspace: "/tmp/coreblow/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/coreblow/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run coreblow devices list");
    expect(state.pairingHint).toContain("coreblow devices list");
  });
