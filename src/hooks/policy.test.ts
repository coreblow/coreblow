...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "coreblow-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "coreblow-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "coreblow-bundled");
      const plugin = makeHookEntry("shared", "coreblow-plugin");
      const managed = makeHookEntry("shared", "coreblow-managed");
    ...
    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "coreblow-managed");
      const workspace = makeHookEntry("shared", "coreblow-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "coreblow-workspace");
      const second = makeHookEntry("shared", "coreblow-workspace");
      second.hook.handlerPath = "/tmp/coreblow-workspace/shared/handler-2.js";
