...
`;
    ...
    expect(parsed.coreblow.emoji).toBe("💾");
    expect(parsed.coreblow.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.coreblow.emoji).toBe("📝");
    expect(parsed.coreblow.events).toEqual(["command"]);
    expect(parsed.coreblow.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.coreblow.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"coreblow": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"coreblow": {"events": ["test"]}}');
  });
...
describe("resolveCoreBlowMetadata", () => {
  it("extracts coreblow metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveCoreBlowMetadata(frontmatter);
  ...
  it("returns undefined when coreblow key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        coreblow: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with CoreBlow" },
            { id: "npm", kind: "npm", package: "@coreblow/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@coreblow/hook");
  });
    ...
    "coreblow":
      {
        ...
        "events": ["command:new", "command:reset"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with CoreBlow" }],
      },
  }
---
...
`;
    ...
    const coreblow = resolveCoreBlowMetadata(frontmatter);
    expect(coreblow).toBeDefined();
    expect(coreblow?.emoji).toBe("💾");
    expect(coreblow?.events).toEqual(["command:new", "command:reset"]);
    expect(coreblow?.requires?.config).toEqual(["workspace.dir"]);
    expect(coreblow?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const coreblow = resolveCoreBlowMetadata(frontmatter);
    expect(coreblow?.emoji).toBe("disk");
    expect(coreblow?.events).toEqual(["command:new"]);
  });
});
