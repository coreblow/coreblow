...
function listAgents(cfg: CoreBlowConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: CoreBlowConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<CoreBlowConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: CoreBlowConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: CoreBlowConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: CoreBlowConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<CoreBlowConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  CoreBlowConfig,
  {
    bindingsRef: CoreBlowConfig["bindings"];
    agentsRef: CoreBlowConfig["agents"];
    sessionRef: CoreBlowConfig["session"];
...
function resolveRouteCacheForConfig(cfg: CoreBlowConfig): Map<string, ResolvedAgentRoute> {
