...
export interface MediaUnderstandingConfig {
    visionProvider: 'openai' | 'gemini' | 'ollama';
...
export class MediaUnderstanding {
    private config: MediaUnderstandingConfig;

    constructor(config: Partial<MediaUnderstandingConfig> = {}) {
        this.config = {
     ...
     */
    async analyze(url: string, mimeType?: string): Promise<MediaAnalysis> {
     ...
     */
    async analyzeImage(url: string): Promise<MediaAnalysis> {
    ...
    private async analyzeImageGemini(url: string): Promise<MediaAnalysis> {
     ...
     */
    async transcribeAudio(url: string): Promise<MediaAnalysis> {
    ...
    async parseDocument(url: string): Promise<MediaAnalysis> {
        ...
        try {
            ...
            if (ext === 'pdf') {
                ...
                if (!text.trim()) text = '[PDF contains images only — use OCR for text extraction]';
     ...
     */
    private detectType(url: string, mimeType?: string): MediaAnalysis['type'] {
        if (mimeType) {
        ...
        if (['pdf', 'docx', 'xlsx', 'txt', 'md', 'csv'].includes(ext)) return 'document';
