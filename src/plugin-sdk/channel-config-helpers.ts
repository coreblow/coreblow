...
function formatPairingApproveHint(channelId: string): string {
  const listCmd = formatCliCommand(`coreblow pairing list ${channelId}`);
  const approveCmd = formatCliCommand(`coreblow pairing approve ${channelId} <code>`);
...
type ChannelConfigAccessorParams<Config extends CoreBlowConfig = CoreBlowConfig> = {
  cfg: Config;
  accountId?: string | null;
};
...
type MultiAccountChannelConfigAdapterParams<
  ...
  Config extends CoreBlowConfig = CoreBlowConfig,
> = {
  sectionKey: string;
...
export function adaptScopedAccountAccessor<Result, Config extends CoreBlowConfig = CoreBlowConfig>(
...
> {
  const base = {
    resolveAllowFrom({ cfg, accountId }: { cfg: CoreBlowConfig; accountId?: string | null }) {
...
function setTopLevelChannelEnabledInConfigSection<Config extends CoreBlowConfig>(params: {
  cfg: Config;
  sectionKey: string;
  enabled: boolean;
}): Config {
...
function removeTopLevelChannelConfigSection<Config extends CoreBlowConfig>(params: {
  cfg: Config;
  sectionKey: string;
}): Config {
...
function clearTopLevelChannelConfigFields<Config extends CoreBlowConfig>(params: {
  cfg: Config;
  sectionKey: string;
  clearBaseFields: string[];
}): Config {
