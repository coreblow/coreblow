...
function printSessionUpdate(notification: SessionNotification): void {
  ...
  switch (update.sessionUpdate) {
    ...
    case "available_commands_update": {
      const names = update.availableCommands?.map((cmd: any) => `/${cmd.name}`).join(" ");
      if (names) {
        console.log(`\n[commands] ${names}`);
      }
      return;
...
export async function createAcpClient(opts: AcpClientOptions = {}): Promise<AcpClientHandle> {
  ...
  const defaultServerCommand = entryPath ? process.execPath : "coreblow";
  ...
  await client.initialize({
    ...
    clientInfo: { name: "coreblow-acp-client", version: "1.0.0" },
  });
...
export async function runAcpClientInteractive(opts: AcpClientOptions = {}): Promise<void> {
  ...
  console.log("CoreBlow ACP client");
