...
const createTempDir = () => tempDirs.make("coreblow-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets COREBLOW_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "coreblow",
    });

    expect(env.COREBLOW_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("coreblow");
  });

  it("overrides pre-existing COREBLOW_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      COREBLOW_SHELL: "wrong",
    });
    expect(env.COREBLOW_SHELL).toBe("acp-client");
  });
  ...
  it("preserves COREBLOW_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        COREBLOW_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["COREBLOW_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default CoreBlow bridge", () => {
    ...
    expect(env.COREBLOW_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.COREBLOW_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "coreblow",
        serverArgs: ["acp"],
        defaultServerCommand: "coreblow",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("COREBLOW_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "coreblow", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "coreblow", "dist", "entry.js");
    const shimPath = path.join(dir, "coreblow.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\coreblow\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/coreblow-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/coreblow-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/coreblow-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
