...
export function buildGatewayRuntimeHints(
  ...
  if (runtime.cachedLabel && platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(env.COREBLOW_PROFILE);
    hints.push(
  ...
  if (runtime.status === "stopped") {
    ...
    hints.push(
      ...buildPlatformRuntimeLogHints({
        platform,
        env,
        systemdServiceName: resolveGatewaySystemdServiceName(env.COREBLOW_PROFILE),
        windowsTaskName: resolveGatewayWindowsTaskName(env.COREBLOW_PROFILE),
      }),
    );
  }
  return hints;
