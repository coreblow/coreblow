...
function manifest(id: string): PluginManifestRecord {
  return {
    ...
    rootDir: `/plugins/${id}`,
    source: `/plugins/${id}`,
    manifestPath: `/plugins/${id}/coreblow.plugin.json`,
  };
}
...
describe("doctor preview warnings", () => {
  ...
  it("includes stale plugin config warnings", () => {
    ...
    expect(warnings[0]).toContain('Run "coreblow doctor --fix"');
  ...
  it("warns but skips auto-removal when plugin discovery has errors", () => {
    ...
    expect(warnings[0]).toContain('rerun "coreblow doctor --fix"');
  });
});
