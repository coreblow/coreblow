...
describe("doctor matrix provider helpers", () => {
  ...
  it("warns on stale custom Matrix plugin paths", async () => {
    const missingPath = path.join(tmpdir(), "coreblow-matrix-missing-provider-test");
    ...
    expect(warnings[1]).toContain("coreblow plugins install @coreblow/matrix");
    expect(warnings[2]).toContain("coreblow plugins install ./extensions/matrix");
  });
  ...
  it("removes stale Matrix plugin config when install path is missing", async () => {
    const missingPath = path.join(tmpdir(), "coreblow-matrix-stale-cleanup-test-" + Date.now());
    await fs.rm(missingPath, { recursive: true, force: true });
