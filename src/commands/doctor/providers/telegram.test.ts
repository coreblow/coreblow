...
describe("doctor telegram provider warnings", () => {
  beforeEach(() => {
    ...
    listTelegramAccountIdsMock.mockReset().mockImplementation((cfg: CoreBlowConfig) => {
    ...
    inspectTelegramAccountMock
      .mockReset()
      .mockImplementation((_params: { cfg: CoreBlowConfig; accountId: string }) => ({
        enabled: true,
        tokenStatus: "configured",
      }));
    resolveTelegramAccountMock
      .mockReset()
      .mockImplementation((params: { cfg: CoreBlowConfig; accountId?: string | null }) => {
  ...
  it("formats allowFrom username warnings", () => {
    const warnings = collectTelegramAllowFromUsernameWarnings({
      ...
      doctorFixCommand: "coreblow doctor --fix",
    });

    expect(warnings).toEqual([
      expect.stringContaining("Telegram allowFrom contains 1 non-numeric entries"),
      expect.stringContaining('Run "coreblow doctor --fix"'),
    ]);
  });
