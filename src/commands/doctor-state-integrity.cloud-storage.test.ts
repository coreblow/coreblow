...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "CoreBlow", ".coreblow");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/coreblow-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".coreblow");
  ...
  it("anchors cloud detection to OS homedir when COREBLOW_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".coreblow");
    const originalCoreBlowHome = process.env.COREBLOW_HOME;
    process.env.COREBLOW_HOME = "/tmp/coreblow-home-override";
    ...
    try {
      ...
      if (originalCoreBlowHome === undefined) {
