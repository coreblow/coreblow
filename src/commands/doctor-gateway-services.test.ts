...
const originalUpdateInProgress = process.env.COREBLOW_UPDATE_IN_PROGRESS;
...
async function runRepair(cfg: CoreBlowConfig) {
...
async function runNonInteractiveRepair(params: {
  ...
  if (params.updateInProgress) {
    process.env.COREBLOW_UPDATE_IN_PROGRESS = "1";
  } else {
    delete process.env.COREBLOW_UPDATE_IN_PROGRESS;
  }
...
function setupGatewayTokenRepairScenario() {
  mocks.readCommand.mockResolvedValue({
    ...
    environment: {
      COREBLOW_GATEWAY_TOKEN: "stale-token",
    },
  });
  mocks.auditGatewayServiceConfig.mockResolvedValue({
    ok: false,
    issues: [
      {
        ...
        message: "Gateway service COREBLOW_GATEWAY_TOKEN does not match gateway.auth.token",
        level: "recommended",
      },
    ],
  });
...
describe("maybeRepairGatewayServiceConfig", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    fsMocks.realpath.mockImplementation(async (value: string) => value);
    mocks.resolveGatewayAuthTokenForService.mockImplementation(async (cfg: CoreBlowConfig, env) => {
      const configToken =
        typeof cfg.gateway?.auth?.token === "string" ? cfg.gateway.auth.token.trim() : undefined;
      const envToken = env.COREBLOW_GATEWAY_TOKEN?.trim() || undefined;
  ...
  afterEach(() => {
    ...
    if (originalUpdateInProgress === undefined) {
      ...
      process.env.COREBLOW_UPDATE_IN_PROGRESS = originalUpdateInProgress;
    }
  });
  ...
  it("uses COREBLOW_GATEWAY_TOKEN when config token is missing", async () => {
    await withEnvAsync({ COREBLOW_GATEWAY_TOKEN: "env-token" }, async () => {
      setupGatewayTokenRepairScenario();
  ...
  it("does not flag entrypoint mismatch when symlink and realpath match", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint: "/Users/test/Library/pnpm/global/5/node_modules/coreblow/dist/index.js",
      installEntrypoint:
        "/Users/test/Library/pnpm/global/5/node_modules/.pnpm/coreblow@2026.3.12/node_modules/coreblow/dist/index.js",
      realpath: async (value: string) => {
        if (value.includes("/global/5/node_modules/coreblow/")) {
          return value.replace(
            "/global/5/node_modules/coreblow/",
            "/global/5/node_modules/.pnpm/coreblow@2026.3.12/node_modules/coreblow/",
          );
        }
        return value;
      },
  ...
  it("does not flag entrypoint mismatch when realpath fails but normalized absolute paths match", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint: "/opt/coreblow/../coreblow/dist/index.js",
      installEntrypoint: "/opt/coreblow/dist/index.js",
  ...
  it("still flags entrypoint mismatch when canonicalized paths differ", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint:
        "/Users/test/.nvm/versions/node/v22.0.0/lib/node_modules/coreblow/dist/index.js",
      installEntrypoint: "/Users/test/Library/pnpm/global/5/node_modules/coreblow/dist/index.js",
    });
  ...
  it("repairs entrypoint mismatch in non-interactive fix mode", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint: "/Users/test/Library/npm/node_modules/coreblow/dist/entry.js",
      installEntrypoint: "/Users/test/Library/npm/node_modules/coreblow/dist/index.js",
    ...
    await withEnvAsync(
      ...
      async () => {
        mocks.readCommand.mockResolvedValue({
          programArguments: gatewayProgramArguments,
          environment: {
            COREBLOW_GATEWAY_TOKEN: "env-file-token",
          },
...
describe("maybeScanExtraGatewayServices", () => {
  ...
  it("removes legacy Linux user systemd services", async () => {
    mocks.findExtraGatewayServices.mockResolvedValue([
      {
        platform: "linux",
        label: "coreblow-gateway.service",
        detail: "unit: /home/test/.config/systemd/user/coreblow-gateway.service",
        scope: "user",
        legacy: true,
      },
    ]);
    mocks.uninstallLegacySystemdUnits.mockResolvedValue([
      {
        ...
        unitPath: "/home/test/.config/systemd/user/coreblow-gateway.service",
        enabled: true,
        exists: true,
      },
    ]);
    ...
    expect(mocks.note).toHaveBeenCalledWith(
      expect.stringContaining("coreblow-gateway.service"),
      "Legacy gateway removed",
    );
    expect(runtime.log).toHaveBeenCalledWith(
      "Legacy gateway services removed. Installing CoreBlow gateway next.",
    );
  });
});
