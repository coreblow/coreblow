...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveCoreBlowAgentDir: vi.fn().mockReturnValue("/tmp/coreblow-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/coreblow-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/coreblow-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/coreblow-dev/coreblow.json",
    }),
...
async function loadFreshModelsStatusCommandModuleForTest() {
  ...
  vi.doMock("../../agents/agent-paths.js", () => ({
    resolveCoreBlowAgentDir: mocks.resolveCoreBlowAgentDir,
  }));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/coreblow-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  ...
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveCoreBlowAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.configPath).toBe("/tmp/coreblow-dev/coreblow.json");
    expect(payload.auth.storePath).toBe("/tmp/coreblow-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/coreblow-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/coreblow-agent-custom");
