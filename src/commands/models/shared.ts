...
export async function loadValidConfigOrThrow(): Promise<CoreBlowConfig> {
...
export async function updateConfig(
  mutator: (cfg: CoreBlowConfig) => CoreBlowConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: CoreBlowConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: CoreBlowConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("coreblow agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
