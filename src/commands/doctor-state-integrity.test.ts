...
function setupSessionState(cfg: CoreBlowConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: CoreBlowConfig) {
...
async function runStateIntegrityText(cfg: CoreBlowConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  it("prints coreblow-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: CoreBlowConfig = {};
    ...
    expect(text).toMatch(/coreblow sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/coreblow sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /coreblow sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
