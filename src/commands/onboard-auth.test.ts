...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under COREBLOW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("coreblow-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-oauth-sync-"));
    ...
    process.env.COREBLOW_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-oauth-nosync-"));
    process.env.COREBLOW_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside COREBLOW_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-oauth-external-"));
    process.env.COREBLOW_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* COREBLOW_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to COREBLOW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("coreblow-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
