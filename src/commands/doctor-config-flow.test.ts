...
describe("doctor config flow", () => {
  ...
  it("creates a Matrix migration snapshot before doctor repair mutates Matrix state", async () => {
    await withTempHome(async (home) => {
      ...
      const snapshotDir = path.join(home, "Backups", "coreblow-migrations");
      ...
      expect(marker.archivePath).toContain(path.join("Backups", "coreblow-migrations"));
    });
  });
  ...
  it("warns when Matrix is installed from a stale custom path", async () => {
    ...
    expect(
      doctorWarnings.some(
        (line) => line.includes("custom path") && line.includes("/tmp/coreblow-matrix-missing"),
      ),
    ).toBe(true);
  });
