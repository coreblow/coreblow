...
const TEST_ARCHIVE_ROOT = "2026-03-09T00-00-00.000Z-coreblow-backup";
...
describe("backupVerifyCommand", () => {
  ...
  beforeAll(async () => {
    tempHome = await createTempHomeEnv("coreblow-backup-verify-test-");
  });
  ...
  it("fails when the archive does not contain a manifest", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-backup-no-manifest-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
  ...
  it("fails when the manifest references a missing asset payload", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-backup-missing-asset-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
      const rootName = "2026-03-09T00-00-00.000Z-coreblow-backup";
        ...
        assets: [
          {
            kind: "state",
            sourcePath: "/tmp/.coreblow",
            archivePath: `${rootName}/payload/posix/tmp/.coreblow`,
          },
        ],
      };
      await fs.writeFile(
  ...
  it("fails when archive paths contain traversal segments", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "coreblow-backup-traversal-",
  ...
  it("fails when archive paths contain backslashes", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "coreblow-backup-backslash-",
  ...
  it("ignores payload manifest.json files when locating the backup manifest", async () => {
    const stateDir = path.join(tempHome.home, ".coreblow");
    const externalWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-workspace-"));
  ...
  it("fails when the archive contains duplicate root manifest entries", async () => {
    const payloadArchivePath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.coreblow/payload.txt`;
    await withBrokenArchiveFixture(
      {
        tempPrefix: "coreblow-backup-duplicate-manifest-",
  ...
  it("fails when the archive contains duplicate payload entries", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "coreblow-backup-duplicate-payload-",
        manifestAssetArchivePath: payloadArchivePath,
        payloads: [
