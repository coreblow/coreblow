...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: CoreBlowConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: CoreBlowConfig) => { changed: boolean; next: CoreBlowConfig };
  defaultModel: string;
  overrideConfig: CoreBlowConfig;
  alreadyDefaultConfig: CoreBlowConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: CoreBlowConfig) => config,
      applyDefaultConfig: (config: CoreBlowConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: CoreBlowConfig = { agents: { defaults: {} } };
