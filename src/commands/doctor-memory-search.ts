 ...
 * Runs as part of `coreblow doctor` — config-only, no network calls.
 */
  ...
  if (resolved.provider !== "auto") {
        ...
        if (opts?.gatewayMemoryProbe?.checked && !opts.gatewayMemoryProbe.ready) {
          ...
          note(
            [
              ...
              "",
              `Verify: ${formatCliCommand("coreblow memory status --deep")}`,
            ]
              .filter(Boolean)
              .join("\n"),
            "Memory search",
      ...
      note(
        [
          ...
          suggestedRemoteProvider
            ? `- Switch to a remote provider: ${formatCliCommand(`coreblow config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("coreblow configure --section model")}`,
        `- To disable: ${formatCliCommand("coreblow config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("coreblow memory status --deep")}`,
      ].join("\n"),
      "Memory search",
