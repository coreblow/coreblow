...
function createTelegramCfg(botToken: string, enabled?: boolean): CoreBlowConfig {
  return {
    channels: {
      telegram: {
        botToken,
...
function patchTelegramAdapter(overrides: Parameters<typeof patchChannelSetupWizardAdapter>[1]) {
  return patchChannelSetupWizardAdapter("telegram", {
    ...
    getStatus:
      overrides.getStatus ??
      vi.fn(async ({ cfg }: { cfg: CoreBlowConfig }) => ({
        channel: "telegram",
...
async function expectQuickstartPickerSkipsWithoutRuntime() {
  ...
  await expect(
    runSetupChannels({} as CoreBlowConfig, prompter, {
      quickstartDefaults: true,
    }),
  ).resolves.toEqual({} as CoreBlowConfig);
...
async function runQuickstartTelegramSetupWithInteractive(params: {
  ...
  try {
    const cfg = await runSetupChannels({} as CoreBlowConfig, prompter, {
      quickstartDefaults: true,
      onSelection: selection,
      onAccountId,
    });
...
vi.mock("./channel-setup/plugin-install.js", async (importOriginal) => {
  ...
  return {
    ...
    ensureChannelSetupPluginInstalled: vi.fn(async ({ cfg }: { cfg: CoreBlowConfig }) => ({
      cfg,
      installed: true,
    })),
...
describe("setupChannels", () => {
  ...
  it("QuickStart uses single-select (no multiselect) and doesn't prompt for Telegram token when WhatsApp is chosen", async () => {
    ...
    await runSetupChannels({} as CoreBlowConfig, prompter, {
      quickstartDefaults: true,
    ...
    await runSetupChannels(
      {
        ...
        plugins: {
          entries: {
            "@coreblow/msteams-plugin": { enabled: true },
          },
        },
      } as CoreBlowConfig,
      prompter,
          ...
          registry.channels.push({
            ...
            plugin: {
              ...
              config: {
                ...
                resolveAccount: (cfg: CoreBlowConfig, accountId: string) =>
                  (
                    cfg.channels?.msteams as
                      | {
                          accounts?: Record<string, Record<string, unknown>>;
              ...
              setupWizard: {
                ...
                status: {
                  ...
                  resolveConfigured: ({ cfg }: { cfg: CoreBlowConfig }) =>
  ...
  it("applies configureInteractive result cfg/account updates", async () => {
    const configureInteractive = vi.fn(async ({ cfg }: { cfg: CoreBlowConfig }) => ({
      cfg: {
        ...cfg,
        channels: {
          ...cfg.channels,
  ...
  it("uses configureWhenConfigured when channel is already configured", async () => {
    const configureWhenConfigured = vi.fn(async ({ cfg }: { cfg: CoreBlowConfig }) => ({
      cfg: {
        ...cfg,
        channels: {
          ...cfg.channels,
