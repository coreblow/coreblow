...
function resolveAgentName(cfg: CoreBlowConfig, agentId: string) {
...
function resolveAgentModel(cfg: CoreBlowConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: CoreBlowConfig): AgentSummary[] {
