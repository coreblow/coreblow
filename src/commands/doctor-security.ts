...
import type { CoreBlowConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: CoreBlowConfig): string[] {
...
export async function noteSecurityWarnings(cfg: CoreBlowConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("coreblow security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.coreblow/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("coreblow approvals get --gateway")}`,
    );
  }
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("coreblow configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("coreblow config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("coreblow doctor --fix")} to generate a token`,
              `  Or set token directly: ${formatCliCommand(
                "coreblow config set gateway.auth.mode token",
              )}`,
            ];
      warnings.push(
        `- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`,
        `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`,
        `  Fix: ${formatCliCommand("coreblow config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('coreblow config set session.dmScope "per-channel-peer"') +
