...
describe("noteStartupOptimizationHints", () => {
  it("does not warn when compile cache and no-respawn are configured", () => {
    ...
    noteStartupOptimizationHints(
      {
        NODE_COMPILE_CACHE: "/var/tmp/coreblow-compile-cache",
        COREBLOW_NO_RESPAWN: "1",
      },
  ...
  it("warns when compile cache is under /tmp and no-respawn is not set", () => {
    ...
    noteStartupOptimizationHints(
      {
        NODE_COMPILE_CACHE: "/tmp/coreblow-compile-cache",
      },
      { platform: "linux", arch: "arm64", totalMemBytes: 4 * 1024 ** 3, noteFn },
    );
    ...
    expect(message).toContain("COREBLOW_NO_RESPAWN is not set to 1");
    expect(message).toContain("export NODE_COMPILE_CACHE=/var/tmp/coreblow-compile-cache");
    expect(message).toContain("export COREBLOW_NO_RESPAWN=1");
  });
