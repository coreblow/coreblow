...
import { readConfigFileSnapshot, writeConfigFile, type CoreBlowConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("coreblow config set gateway.auth.mode token")} or ${formatCliCommand("coreblow config set gateway.auth.mode password")}.`,
  ].join(" ");
}
