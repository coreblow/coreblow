...
beforeAll(() => {
  envSnapshot = captureEnv(["COREBLOW_PROFILE"]);
  process.env.COREBLOW_PROFILE = "isolated";
});
...
vi.mock("../infra/coreblow-root.js", () => ({
  resolveCoreBlowPackageRoot: vi.fn().mockResolvedValue("/tmp/coreblow"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/coreblow/pnpm-lock.yaml",
      markerPath: "/tmp/coreblow/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.coreblow.gateway.plist",
    }),
  }),
...
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.coreblow.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("coreblow status --all") ||
          line.includes("coreblow --profile isolated status --all"),
      ),
    ).toBe(true);
  });
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("COREBLOW_GATEWAY_TOKEN", "abcd1234", async () => {
