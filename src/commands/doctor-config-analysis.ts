...
import { CoreBlowSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: CoreBlowConfig): {
  config: CoreBlowConfig;
  removed: string[];
} {
  const parsed = CoreBlowSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: CoreBlowConfig): void {
