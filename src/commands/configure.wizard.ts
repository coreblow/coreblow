    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from coreblow.json",
        },
      ],
...
export async function runConfigureWizard(
  ...
  try {
    intro(opts.command === "update" ? "CoreBlow update wizard" : "CoreBlow configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: CoreBlowConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("coreblow doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
