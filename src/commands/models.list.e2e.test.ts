...
const ensureCoreBlowModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveCoreBlowAgentDir = vi.fn().mockReturnValue("/tmp/coreblow-agent");
...
beforeEach(() => {
  ...
  ensureCoreBlowModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureCoreBlowModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the write snapshot config when provided", async () => {
    ...
    expect(ensureCoreBlowModelsJson).toHaveBeenCalled();
    expect(ensureCoreBlowModelsJson.mock.calls[0]?.[0]).toEqual(resolvedConfig);
  });
