...
describe("promptRemoteGatewayConfig", () => {
  ...
  it("falls back to manual URL entry when discovery trust is declined", async () => {
    ...
    const next = await promptRemoteGatewayConfig({} as CoreBlowConfig, prompter);
  ...
  it("allows ws:// hostname remote URLs when COREBLOW_ALLOW_INSECURE_PRIVATE_WS=1", async () => {
    ...
    const text: WizardPrompter["text"] = vi.fn(async (params) => {
      if (params.message === "Gateway WebSocket URL") {
        ...
        return "ws://coreblow-gateway.ai:18789";
      }
      return "";
    }) as WizardPrompter["text"];
    ...
    expect(next.gateway?.mode).toBe("remote");
    expect(next.gateway?.remote?.url).toBe("ws://coreblow-gateway.ai:18789");
  });
