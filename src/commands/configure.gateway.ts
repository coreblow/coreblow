...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "COREBLOW_GATEWAY_TOKEN",
          placeholder: "COREBLOW_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like COREBLOW_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. CoreBlow will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: CoreBlow trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.coreblow.com/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
