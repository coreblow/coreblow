...
export function parseCoreBlowVersion(raw: string | null | undefined): CoreBlowVersion | null {
  if (!raw) {
    return null;
  }
...
export function normalizeCoreBlowVersionBase(raw: string | null | undefined): string | null {
  const parsed = parseCoreBlowVersion(raw);
  if (!parsed) {
    return null;
  }
...
export function isSameCoreBlowStableFamily(
  a: string | null | undefined,
  b: string | null | undefined,
): boolean {
  const parsedA = parseCoreBlowVersion(a);
  const parsedB = parseCoreBlowVersion(b);
  if (!parsedA || !parsedB) {
    return false;
  }
...
export function compareCoreBlowVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseCoreBlowVersion(a);
...
export function shouldWarnOnTouchedVersion(
  ...
  const parsedCurrent = parseCoreBlowVersion(current);
  const parsedTouched = parseCoreBlowVersion(touched);
  if (
    parsedCurrent &&
    parsedTouched &&
  ...
  if (isSameCoreBlowStableFamily(current, touched)) {
    return false;
  }
  const cmp = compareCoreBlowVersions(current, touched);
  return cmp !== null && cmp < 0;
}

function releaseRank(version: CoreBlowVersion): number {
