...
export const GENERATED_BASE_CONFIG_SCHEMA = {
  ...
  uiHints: {
    ...
    update: {
      ...
      group: "Update",
      order: 25,
      help: "Update-channel and startup-check behavior for keeping CoreBlow runtime versions current. Use conservative channels in production and more experimental channels only in controlled environments.",
      tags: ["advanced"],
    },
    cli: {
      label: "CLI",
    ...
    hooks: {
      ...
      group: "Hooks",
      order: 110,
      help: "Inbound webhook automation surface for mapping external events into wake or agent actions in CoreBlow. Keep this locked down with explicit token/session/agent controls before exposing it beyond trusted networks.",
      tags: ["advanced"],
    },
    ui: {
      label: "UI",
    ...
    meta: {
      label: "Metadata",
      help: "Metadata fields automatically maintained by CoreBlow to record write/version history for this config file. Keep these values system-managed and avoid manual edits unless debugging migration history.",
      tags: ["advanced"],
    },
    "meta.lastTouchedVersion": {
      label: "Config Last Touched Version",
      help: "Auto-set when CoreBlow writes the config.",
      tags: ["media"],
    },
    "meta.lastTouchedAt": {
    ...
    "env.vars": {
      ...
      help: "Explicit key/value environment variable overrides merged into runtime process environment for CoreBlow. Use this for deterministic env configuration instead of relying only on shell profile side effects.",
      tags: ["advanced"],
    },
    "wizard.lastRunAt": {
    ...
    "wizard.lastRunVersion": {
      ...
      help: "CoreBlow version recorded at the time of the most recent wizard run on this config. Use this when diagnosing behavior differences across version-to-version setup changes.",
      tags: ["advanced"],
    },
    ...
    "diagnostics.cacheTrace.filePath": {
      ...
      help: "JSONL output path for cache trace logs (default: $COREBLOW_STATE_DIR/logs/cache-trace.jsonl).",
      tags: ["observability", "storage"],
    },
    ...
    "agents.list[].runtime": {
      label: "Agent Runtime",
      help: "Optional runtime descriptor for this agent. Use embedded for default CoreBlow execution or acp for external ACP harness defaults.",
      tags: ["advanced"],
    },
    "agents.list[].runtime.type": {
      label: "Agent Runtime Type",
      help: 'Runtime type for this agent: "embedded" (default CoreBlow runtime) or "acp" (ACP harness defaults).',
      tags: ["advanced"],
    },
    ...
    "agents.list[].runtime.acp.agent": {
      ...
      help: "Optional ACP harness agent id to use for this CoreBlow agent (for example codex, claude, cursor, gemini, coreblow).",
      tags: ["advanced"],
    },
    ...
    "gateway.controlUi.basePath": {
      ...
      help: "Optional URL prefix where the Control UI is served (e.g. /coreblow).",
      placeholder: "/coreblow",
      tags: ["network", "storage"],
    },
    ...
    "gateway.nodes.allowCommands": {
      ...
      help: "Extra node.invoke commands to allow beyond the gateway defaults (array of command strings). Enabling dangerous commands here is a security-sensitive override and is flagged by `coreblow security audit`.",
      tags: ["access", "network"],
    },
    ...
    "nodeHost.browserProxy.allowProfiles": {
      ...
      help: "Optional allowlist of browser profile names exposed through node proxy routing. Leave empty to preserve the default full profile surface, including profile create/delete routes. When set, CoreBlow enforces least-privilege profile access and blocks persistent profile create/delete through the proxy.",
    ...
    "agents.defaults.memorySearch.store.path": {
      ...
      help: "Sets where the SQLite memory index is stored on disk for each agent. Keep the default `~/.coreblow/memory/{agentId}.sqlite` unless you need custom storage placement or backup policy alignment.",
      tags: ["storage"],
    },
    ...
    "memory.backend": {
      ...
      help: 'Selects the global memory engine: "builtin" uses CoreBlow memory internals, while "qmd" uses the QMD sidecar pipeline. Keep "builtin" unless you intentionally operate QMD.',
      tags: ["storage"],
    },
    "memory.citations": {
    ...
    "models.providers.*.injectNumCtxForOpenAICompat": {
      ...
      help: "Controls whether CoreBlow injects `options.num_ctx` for Ollama providers configured with the OpenAI-compatible adapter (`openai-completions`). Default is true. Set false only if your proxy/upstream rejects unknown `options` payload fields.",
      tags: ["models"],
    },
    ...
    "agents.defaults.embeddedPi": {
      label: "Embedded Pi",
      help: "Embedded Pi runner hardening controls for how workspace-local Pi settings are trusted and applied in CoreBlow sessions.",
      tags: ["advanced"],
    },
    ...
    "agents.defaults.sandbox.browser.network": {
      ...
      help: "Docker network for sandbox browser containers (default: coreblow-sandbox-browser). Avoid bridge if you need stricter isolation.",
      tags: ["storage"],
    },
    ...
    "commands.mcp": {
      label: "Allow /mcp",
      help: "Allow /mcp chat command to manage CoreBlow MCP server config under mcp.servers (default: false).",
      tags: ["advanced"],
    },
    "commands.plugins": {
      label: "Allow /plugins",
    ...
    mcp: {
      label: "MCP",
      help: "Global MCP server definitions managed by CoreBlow. Embedded Pi and other runtime adapters can consume these servers without storing them inside Pi-owned project settings.",
      tags: ["advanced"],
    },
    "mcp.servers": {
      label: "MCP Servers",
      help: "Named MCP server definitions. CoreBlow stores them in its own config and runtime adapters decide which transports are supported at execution time.",
      tags: ["advanced"],
    },
    "ui.seamColor": {
      label: "Accent Color",
    ...
    "session.parentForkMaxTokens": {
      ...
      help: "Maximum parent-session token count allowed for thread/session inheritance forking. If the parent exceeds this, CoreBlow starts a fresh thread session instead of forking; set 0 to disable this protection.",
    ...
    "discovery.wideArea.domain": {
      ...
      help: "Optional unicast DNS-SD domain for wide-area discovery, such as coreblow.internal. Use this when you intentionally publish gateway discovery beyond local mDNS scopes.",
      tags: ["network"],
    },
    "discovery.mdns": {
    ...
    "agents.list[].identity.avatar": {
      ...
      placeholder: "avatars/coreblow.png",
      tags: ["advanced"],
    },
    ...
    "plugins.installs": {
      ...
      help: "CLI-managed install metadata (used by `coreblow plugins update` to locate install sources).",
      tags: ["advanced"],
    },
    ...
    "plugins.installs.*.installPath": {
      ...
      help: "Resolved install directory (usually ~/.coreblow/extensions/<id>).",
      tags: ["storage"],
    },
