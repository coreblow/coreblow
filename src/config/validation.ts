...
import type { CoreBlowConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: CoreBlowConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: CoreBlowConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: CoreBlowConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = CoreBlowSchema.safeParse(normalizedRaw);
  ...
  const validatedConfig = validated.data as CoreBlowConfig;
...
function validateConfigObjectWithPluginsBase(
  ...
  const warnings: ConfigValidationIssue[] = listLegacyWebSearchConfigPaths(raw).map((path) => ({
    ...
    message:
      ...
      "Move it under plugins.entries.<plugin>.config.webSearch.*; CoreBlow mapped it automatically for compatibility.",
  }));
  ...
  let compatConfig: CoreBlowConfig | null | undefined;

  const ensureCompatConfig = (): CoreBlowConfig => {
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native CoreBlow config schema.
