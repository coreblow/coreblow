...
function createCustomRootCfg(customRoot: string, defaultAgentId = "ops"): CoreBlowConfig {
  return {
    session: {
...
const discoveryResolvers = [
  {
    label: "async",
    resolve: async (cfg: CoreBlowConfig, env: NodeJS.ProcessEnv) =>
  ...
  it("resolves all configured agent stores", async () => {
    await withTempHome(async () => {
      const cfg: CoreBlowConfig = {
        session: {
          store: "~/.coreblow/agents/{agentId}/sessions/sessions.json",
        },
        agents: {
...
describe("resolveAllAgentSessionStoreTargets", () => {
  ...
  it("respects the caller env when resolving configured and discovered store roots", async () => {
    await withTempHome(async (home) => {
      ...
      const env = {
        ...process.env,
        COREBLOW_STATE_DIR: envStateDir,
      };
      const cfg: CoreBlowConfig = {};
