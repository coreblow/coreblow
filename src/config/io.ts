...
import type { CoreBlowConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  coreblow config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  coreblow config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: CoreBlowConfig,
  pathSegments: string[],
): { changed: boolean; next: CoreBlowConfig } {
...
function coerceConfig(value: unknown): CoreBlowConfig {
...
function stampConfigVersion(cfg: CoreBlowConfig): CoreBlowConfig {
  const now = new Date().toISOString();
  return {
    ...cfg,
    meta: {
...
function warnIfConfigFromFuture(cfg: CoreBlowConfig, logger: Pick<typeof console, "warn">): void {
  ...
  if (shouldWarnOnTouchedVersion(VERSION, touched)) {
    logger.warn(
      `Config was last written by a newer CoreBlow (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as CoreBlowConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): CoreBlowConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(effectiveConfigRaw as CoreBlowConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: CoreBlowConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as CoreBlowConfig)
        : cfgToWrite;
...
let runtimeConfigSnapshot: CoreBlowConfig | null = null;
let runtimeConfigSourceSnapshot: CoreBlowConfig | null = null;
...
export function getRuntimeConfigSnapshot(): CoreBlowConfig | null {
  return runtimeConfigSnapshot;
}

export function getRuntimeConfigSourceSnapshot(): CoreBlowConfig | null {
  return runtimeConfigSourceSnapshot;
}

function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: CoreBlowConfig;
  candidate: CoreBlowConfig;
}): boolean {
...
export function projectConfigOntoRuntimeSourceSnapshot(config: CoreBlowConfig): CoreBlowConfig {
...
export async function readBestEffortConfig(): Promise<CoreBlowConfig> {
...
export async function writeConfigFile(
  ...
  // edits to coreblow.json remain visible (no stale snapshot).
}
