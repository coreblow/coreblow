...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ COREBLOW_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-coreblow",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-coreblow#general");
  });
  ...
  it("derives session transcripts dir from COREBLOW_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { COREBLOW_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when COREBLOW_STATE_DIR differs from stored paths", () => {
