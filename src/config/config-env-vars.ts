...
function collectConfigEnvVarsByTarget(cfg?: CoreBlowConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: CoreBlowConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: CoreBlowConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: CoreBlowConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.COREBLOW_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
