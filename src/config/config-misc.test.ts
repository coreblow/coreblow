...
import { buildWebSearchProviderConfig, withTempHome, writeCoreBlowConfig } from "./test-helpers.js";
import { CoreBlowSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = CoreBlowSchema.safeParse({
      $schema: "https://coreblow.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://coreblow.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = CoreBlowSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = CoreBlowSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("rejects removed legacy config entries without auto-migrating", async () => {
    await withTempHome(async (home) => {
      await writeCoreBlowConfig(home, {
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeCoreBlowConfig(home, {
        gateway: { bind: "${COREBLOW_BIND}" },
      });

      const prev = process.env.COREBLOW_BIND;
      process.env.COREBLOW_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        ...
        if (prev === undefined) {
          delete process.env.COREBLOW_BIND;
        } else {
          process.env.COREBLOW_BIND = prev;
        }
      }
    });
  });
