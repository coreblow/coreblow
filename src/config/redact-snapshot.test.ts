...
import type { ConfigFileSnapshot } from "./types.coreblow.js";
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/coreblow/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/coreblow/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/coreblow/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/coreblow/nickserv-password.txt");
