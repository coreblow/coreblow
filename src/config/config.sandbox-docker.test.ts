...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("coreblow-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "coreblow-sandbox-browser-global" },
      agentBrowser: { network: "coreblow-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("coreblow-sandbox-browser-agent");
  });
