...
export type PromptDefaultModelResult = { model?: string; config?: CoreBlowConfig };
...
function resolveConfiguredModelRaw(cfg: CoreBlowConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: CoreBlowConfig): string[] {
...
export function applyModelAllowlist(cfg: CoreBlowConfig, models: string[]): CoreBlowConfig {
