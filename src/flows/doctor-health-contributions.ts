...
export type DoctorHealthFlowContext = {
  ...
  cfg: CoreBlowConfig;
  cfgForPersistence: CoreBlowConfig;
  sourceConfigValid: boolean;
  configPath: string;
...
export function resolveDoctorMode(cfg: CoreBlowConfig): DoctorFlowMode {
...
async function runGatewayConfigHealth(ctx: DoctorHealthFlowContext): Promise<void> {
  if (!ctx.cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("coreblow configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("coreblow config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(ctx.configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("coreblow setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveDoctorMode(ctx.cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(ctx.cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("coreblow config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("coreblow config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
...
async function runWriteConfigHealth(ctx: DoctorHealthFlowContext): Promise<void> {
  ...
  if (!ctx.prompter.shouldRepair) {
    ctx.runtime.log(`Run "${formatCliCommand("coreblow doctor --fix")}" to apply changes.`);
  }
}
