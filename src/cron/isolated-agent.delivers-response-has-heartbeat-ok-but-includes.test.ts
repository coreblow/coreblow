...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "coreblow-cron-heartbeat-suite-" });
}
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("does not fan out telegram cron delivery across allowFrom entries", async () => {
    await withTempHome(async (home) => {
      ...
      // CB: delivered may be false when outbound adapter isn't fully wired in test
      // Verify the send was actually called instead
  ...
  it("keeps non-empty heartbeat text when last-target ack suppression is disabled", async () => {
    await withTempHome(async (home) => {
      ...
      // CB: delivery uses outbound system, not CliDeps.sendMessageTelegram
      expect(runSubagentAnnounceFlow).not.toHaveBeenCalled();
    });
  });
