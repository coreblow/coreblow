...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHelper(fn, { prefix: "coreblow-cron-submodel-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".coreblow", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<CoreBlowConfig> = {},
): CoreBlowConfig {
  const base: CoreBlowConfig = {
    agents: {
      defaults: {
...
async function runSubagentModelCase(params: {
  home: string;
  cfgOverrides?: Partial<CoreBlowConfig>;
  ...
  it.each([
    {
      ...
      } satisfies Partial<CoreBlowConfig>,
