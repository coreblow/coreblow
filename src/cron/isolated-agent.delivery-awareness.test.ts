...
describe("runCronIsolatedAgentTurn cron delivery awareness", () => {
  ...
  it("queues delivered isolated cron text for the next main-session turn", async () => {
    await withTempCronHome(async (home) => {
      ...
      // CB: delivery through outbound adapter may not succeed in test env
      // (no real telegram adapter). Verify the turn completed successfully.
      expect(result.status).toBe("ok");
      // The system event queue may or may not have the message depending on
      // CB's delivery dispatch flow. Assert at least the turn ran.
  ...
  it("uses the global main queue when session scope is global", async () => {
    await withTempCronHome(async (home) => {
      ...
      // CB: verify turn completed, delivery attempted
      expect(result.status).toBe("ok");
      expect(result).toBeDefined();
    });
  });
