...
const { makeStorePath } = createCronStoreHarness({ prefix: "coreblow-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses COREBLOW_HOME for tilde expansion", () => {
    vi.stubEnv("COREBLOW_HOME", "/srv/coreblow-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/coreblow-home", "cron", "jobs.json"));
  });
});
