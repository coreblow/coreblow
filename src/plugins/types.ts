 ...
 */
export type CoreBlowPluginConfigSchema = {
...
export type CoreBlowPluginToolContext = {
...
export type CoreBlowPluginToolFactory = (
  ctx: CoreBlowPluginToolContext,
) => AnyAgentTool | AnyAgentTool[] | null | undefined;

export type CoreBlowPluginToolOptions = {
  name?: string;
  names?: string[];
  optional?: boolean;
};

export type CoreBlowPluginHookOptions = {
  entry?: HookEntry;
  name?: string;
  description?: string;
  register?: boolean;
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<CoreBlowConfig>;
  defaultModel?: string;
  notes?: string[];
};
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). CoreBlow uses this to expose
 ...
 *
 * Runs after CoreBlow resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common CoreBlow auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and CoreBlow wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before CoreBlow builds its generic
...
/**
 * Provider-owned stream wrapper hook after CoreBlow applies its generic
 ...
 *
 * Return `true` or `false` to override CoreBlow's built-in provider cache TTL
 ...
 *
 * Runs only after CoreBlow exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that CoreBlow should append to the generic `Unknown model`
 * error.
 */
 ...
 *
 * Runs after CoreBlow loads the discovered model catalog and merges configured
...
export type ProviderOAuthProfileIdRepair = {
   ...
   *
   * When omitted, CoreBlow falls back to `<provider>:default`.
   */
...
export type ProviderPlugin = {
   ...
   *
   * CoreBlow calls this only from async model resolution paths. After it
  ...
  /**
   * Provider-owned stream wrapper applied after generic CoreBlow wrappers.
   *
   ...
   *
   * Called after CoreBlow resolves the raw configured credential but before the
   ...
   * hint than CoreBlow's generic auth-store guidance.
   */
   ...
   * CoreBlow's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when CoreBlow should surface a provider-specific hint for
   ...
   * merging. CoreBlow deduplicates by `provider/id`, so plugins only need to
   ...
   *
   * CoreBlow uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `coreblow doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `coreblow doctor` should offer.
   *
   ...
   *
   * CoreBlow calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep CoreBlow's generic error text.
   */
  buildAuthDoctorHint?: (
...
export type WebSearchProviderPlugin = {
   ...
   * when CoreBlow has no config-aware runtime context yet.
   *
  ...
  getConfiguredCredentialValue?: (config?: CoreBlowConfig) => unknown;
  setConfiguredCredentialValue?: (configTarget: CoreBlowConfig, value: unknown) => void;
  applySelectionConfig?: (config: CoreBlowConfig) => CoreBlowConfig;
...
export type CoreBlowPluginGatewayMethod = {
  method: string;
...
export type PluginCommandContext = {
  ...
  commandBody: string;
  /** Current CoreBlow configuration */
  config: CoreBlowConfig;
 ...
 */
export type CoreBlowPluginCommandDefinition = {
...
export type CoreBlowPluginHttpRouteAuth = "gateway" | "plugin";
export type CoreBlowPluginHttpRouteMatch = "exact" | "prefix";

export type CoreBlowPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type CoreBlowPluginHttpRouteParams = {
  path: string;
  handler: CoreBlowPluginHttpRouteHandler;
  auth: CoreBlowPluginHttpRouteAuth;
  match?: CoreBlowPluginHttpRouteMatch;
  replaceExisting?: boolean;
};

export type CoreBlowPluginCliContext = {
  program: Command;
  config: CoreBlowConfig;
  workspaceDir?: string;
  logger: PluginLogger;
...
export type CoreBlowPluginCliRegistrar = (ctx: CoreBlowPluginCliContext) => void | Promise<void>;

export type CoreBlowPluginCliCommandDescriptor = {
  name: string;
  description: string;
...
export type CoreBlowPluginServiceContext = {
  config: CoreBlowConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type CoreBlowPluginService = {
  id: string;
  start: (ctx: CoreBlowPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: CoreBlowPluginServiceContext) => void | Promise<void>;
};
...
export type CliBackendPlugin = {
  ...
  /**
   * Whether CoreBlow should inject bundle MCP config for this backend.
   *
...
export type CoreBlowPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type CoreBlowPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  ...
  register?: (api: CoreBlowPluginApi) => void | Promise<void>;
  activate?: (api: CoreBlowPluginApi) => void | Promise<void>;
};

export type CoreBlowPluginModule =
  | CoreBlowPluginDefinition
  | ((api: CoreBlowPluginApi) => void | Promise<void>);

export type PluginRegistrationMode = "full" | "setup-only" | "setup-runtime";

/** Main registration API injected into native plugin entry files. */
export type CoreBlowPluginApi = {
  id: string;
  name: string;
  version?: string;
  description?: string;
  ...
  logger: PluginLogger;
  registerTool: (
    tool: AnyAgentTool | CoreBlowPluginToolFactory,
    opts?: CoreBlowPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: CoreBlowPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: CoreBlowPluginHttpRouteParams) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: CoreBlowPluginChannelRegistration | ChannelPlugin) => void;
  ...
  registerCli: (
    registrar: CoreBlowPluginCliRegistrar,
    opts?: {
      commands?: string[];
      descriptors?: CoreBlowPluginCliCommandDescriptor[];
    },
  ) => void;
  registerService: (service: CoreBlowPluginService) => void;
   ...
   */
  registerCommand: (command: CoreBlowPluginCommandDefinition) => void;
...
export type PluginFormat = "coreblow" | "bundle";
...
export type PluginHookBeforeToolCallResult = {
  ...
  requireApproval?: {
     ...
     * CoreBlow does not await this callback before allowing or denying the tool call.
     */
