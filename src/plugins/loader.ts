...
export function resolveRuntimePluginRegistry(
  ...
  return getCompatibleActivePluginRegistry(options) ?? loadCoreBlowPlugins(options);
}
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: CoreBlowPluginDefinition;
  register?: CoreBlowPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as CoreBlowPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as CoreBlowPluginDefinition;
...
function resolveCandidateDuplicateRank(params: {
  candidate: ReturnType<typeof discoverCoreBlowPlugins>["candidates"][number];
...
function compareDuplicateCandidateOrder(params: {
  left: ReturnType<typeof discoverCoreBlowPlugins>["candidates"][number];
  right: ReturnType<typeof discoverCoreBlowPlugins>["candidates"][number];
  manifestByRoot: Map<string, ReturnType<typeof loadPluginManifestRegistry>["plugins"][number]>;
  provenance: PluginProvenanceIndex;
  env: NodeJS.ProcessEnv;
}): number {
...
export function loadCoreBlowPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  if (options.activate === false && options.cache !== false) {
    throw new Error(
      "loadCoreBlowPlugins: activate:false requires cache:false to prevent command registry divergence",
    );
  }
  const {
    env,
  ...
  const getJiti = (modulePath: string) => {
    ...
    // Pass loader's moduleUrl so the coreblow root can always be resolved even when
    // loading external plugins from outside the installation directory (e.g. ~/.coreblow/extensions/).
  ...
  const discovery = discoverCoreBlowPlugins({
    workspaceDir: options.workspaceDir,
  ...
  for (const candidate of orderedCandidates) {
    ...
    if (record.format === "bundle") {
      ...
      for (const capability of unsupportedCapabilities) {
        registry.diagnostics.push({
          ...
          pluginId: record.id,
          source: record.source,
          message: `bundle capability detected but not wired into CoreBlow yet: ${capability}`,
        });
      }
      if (
        enableState.enabled &&
    ...
    let mod: CoreBlowPluginModule | null = null;
    try {
      mod = getJiti(safeSource)(safeSource) as CoreBlowPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
