...
export function resolveBundledPluginsDir(env: NodeJS.ProcessEnv = process.env): string | undefined {
  const override = env.COREBLOW_BUNDLED_PLUGINS_DIR?.trim();
  if (override) {
    ...
    try {
      const argvPackageRoot = resolveCoreBlowPackageRootSync({ argv1: process.argv[1] });
  ...
  try {
    const packageRoots = [
      resolveCoreBlowPackageRootSync({ argv1: process.argv[1] }),
