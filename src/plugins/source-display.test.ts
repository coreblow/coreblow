...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "coreblow", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".coreblow", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".coreblow", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("resolves source roots from an explicit env override", () => {
    const homeDir = path.resolve(path.sep, "tmp", "coreblow-home");
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".coreblow", "extensions"),
      },
    });
  });
});
