...
import { resolveCoreBlowPackageRootSync } from "../infra/coreblow-root.js";
...
function hasTrustedCoreBlowRootIndicator(params: {
  packageRoot: string;
  ...
  const hasCoreBlowBin =
    (typeof params.packageJson.bin === "string" &&
      params.packageJson.bin.toLowerCase().includes("coreblow")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.coreblow === "string");
  const hasCoreBlowEntrypoint = fs.existsSync(path.join(params.packageRoot, "coreblow.mjs"));
  return hasCliEntryExport || hasCoreBlowBin || hasCoreBlowEntrypoint;
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedCoreBlowRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveTrustedCoreBlowRootFromArgvHint(params: {
  argv1?: string;
  cwd: string;
}): string | null {
  if (!params.argv1) {
  ...
  const packageRoot = resolveCoreBlowPackageRootSync({
    cwd: params.cwd,
    argv1: params.argv1,
  });
  if (!packageRoot) {
  ...
  return hasTrustedCoreBlowRootIndicator({ packageRoot, packageJson }) ? packageRoot : null;
}
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveCoreBlowPackageRootSync({ cwd });
  const fromExplicitHints =
    resolveTrustedCoreBlowRootFromArgvHint({ cwd, argv1: params.argv1 }) ??
    (params.moduleUrl
...
export function resolvePluginSdkScopedAliasMap(
  ...
  for (const subpath of listPluginSdkExportedSubpaths({
    ...
    for (const kind of orderedKinds) {
      ...
      if (fs.existsSync(candidate)) {
        aliasMap[`coreblow/plugin-sdk/${subpath}`] = candidate;
        break;
      }
    }
  }
...
export function buildPluginLoaderAliasMap(
  ...
  return {
    ...(extensionApiAlias ? { "coreblow/extension-api": extensionApiAlias } : {}),
    ...(pluginSdkAlias ? { "coreblow/plugin-sdk": pluginSdkAlias } : {}),
