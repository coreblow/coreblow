...
const APPROVALS_PATH = "~/.coreblow/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "coreblow-app-server:thread:",
  "coreblow-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("coreblow.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("coreblow.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to CoreBlow instead.${buildDetachHintSuffix(binding.detachHint)}`;
}
