...
const fixtureRoot = makeTrackedTempDir("coreblow-sdk-alias-root", fixtureTempDirs);
let tempDirIndex = 0;
...
function createPluginSdkAliasFixture(params?: {
  ...
  const packageJson: Record<string, unknown> = {
    name: params?.packageName ?? "coreblow",
    type: "module",
  };
  if (trustedRootIndicatorMode === "bin+marker") {
    packageJson.bin = {
      coreblow: "coreblow.mjs",
    };
  }
  ...
  if (trustedRootIndicatorMode === "bin+marker") {
    fs.writeFileSync(path.join(root, "coreblow.mjs"), "export {};\n", "utf-8");
  }
...
function createExtensionApiAliasFixture(params?: { srcBody?: string; distBody?: string }) {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: "coreblow", type: "module" }, null, 2),
    "utf-8",
  );
  fs.writeFileSync(path.join(root, "coreblow.mjs"), "export {};\n", "utf-8");
  fs.writeFileSync(srcFile, params?.srcBody ?? "export {};\n", "utf-8");
...
function createUserInstalledPluginSdkAliasFixture() {
  ...
  const externalPluginRoot = path.join(makeTempDir(), ".coreblow", "extensions", "demo");
...
function expectPluginSdkAliasTargets(
  ...
  expect(fs.realpathSync(aliases["coreblow/plugin-sdk"] ?? "")).toBe(
    fs.realpathSync(params.rootAliasPath),
  );
  if (params.channelRuntimePath) {
    expect(fs.realpathSync(aliases["coreblow/plugin-sdk/channel-runtime"] ?? "")).toBe(
...
function expectCwdFallbackPluginSdkAliasResolution(params: {
  ...
  const resolved = withCwd(params.fixture.root, () =>
    resolvePluginSdkAlias({
      srcFile: "channel-runtime.ts",
      distFile: "channel-runtime.js",
      modulePath: "/tmp/tsx-cache/coreblow-loader.js",
...
describe("plugin sdk alias helpers", () => {
  it.each([
    ...
    },
    {
      ...
      modulePath: () => "/tmp/tsx-cache/coreblow-loader.js",
      argv1: (root: string) => path.join(root, "coreblow.mjs"),
      srcFile: "index.ts",
      distFile: "index.js",
      env: { NODE_ENV: undefined },
      expected: "src" as const,
  ...
  it("resolves plugin-sdk aliases for user-installed plugins via the running coreblow argv hint", () => {
    ...
    const aliases = withCwd(externalPluginRoot, () =>
      withEnv({ NODE_ENV: undefined }, () =>
        buildPluginLoaderAliasMap(externalPluginEntry, path.join(fixture.root, "coreblow.mjs")),
      ),
    );
  ...
  it("resolves plugin-sdk aliases for user-installed plugins via moduleUrl hint", () => {
    ...
    // This covers installations where argv1 does not resolve to the coreblow root
    // (e.g. single-binary distributions or custom process launchers).
    // Use coreblow.mjs which is created by createPluginSdkAliasFixture (bin+marker mode).
    // Use fixture.root as cwd so process.cwd() fallback also resolves to fixture, not the
    // real coreblow repo root in the test runner environment.
    const loaderModuleUrl = pathToFileURL(path.join(fixture.root, "coreblow.mjs")).href;
    ...
    // runner binary, inside the coreblow repo), which resolves before moduleUrl is checked.
    // An empty string is falsy so resolveTrustedCoreBlowRootFromArgvHint returns null,
  ...
  it.each([
    ...
    {
      name: "does not resolve plugin-sdk alias files from cwd fallback when package root is not an CoreBlow root",
      fixture: () =>
        createPluginSdkAliasFixture({
          srcFile: "channel-runtime.ts",
          distFile: "channel-runtime.js",
  ...
  it("loads source runtime shims through the non-native Jiti boundary", async () => {
    ...
    fs.writeFileSync(
      ...
      `import { resolveOutboundSendDep } from "coreblow/plugin-sdk/infra-runtime";
...
`,
    ...
    const withAlias = createJiti(jitiBaseUrl, {
      ...buildPluginLoaderJitiOptions({
        "coreblow/plugin-sdk/infra-runtime": copiedChannelRuntimeShim,
      }),
      tryNative: false,
    });
