...
vi.mock("./loader.js", () => ({
  loadCoreBlowPlugins: (...args: unknown[]) => loadCoreBlowPluginsMock(...args),
}));
...
function getLastLoadPluginsCall(): Record<string, unknown> {
  const call = loadCoreBlowPluginsMock.mock.calls.at(-1)?.[0];
...
describe("resolvePluginProviders", () => {
  beforeEach(async () => {
    vi.resetModules();
    loadCoreBlowPluginsMock.mockReset();
    loadCoreBlowPluginsMock.mockReturnValue({
