...
function ensureCoreBlowExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
...
async function installBundleFromSourceDir(
  ...
  try {
    ...
    logger.warn?.(
      `Bundle "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "coreblow security audit --deep" after install.`,
    );
  }
...
async function detectNativePackageInstallSource(packageDir: string): Promise<boolean> {
  ...
  try {
    ...
    return ensureCoreBlowExtensions({ manifest }).ok;
  } catch {
    return false;
  }
}
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureCoreBlowExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from coreblow.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-coreblow"), the plugin registry
  ...
  if (!minHostVersionCheck.ok) {
    if (minHostVersionCheck.kind === "invalid") {
      return {
        ok: false,
        error: `invalid package.json coreblow.install.minHostVersion: ${minHostVersionCheck.error}`,
    ...
    return {
      ok: false,
      error: `plugin "${pluginId}" requires CoreBlow >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}. Upgrade CoreBlow and retry.`,
  ...
  try {
    ...
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "coreblow security audit --deep" after install.`,
    );
  }
...
export async function installPluginFromNpmSpec(params: {
  ...
  const flowResult = await runtime.installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "coreblow-npm-pack-",
    spec,
    timeoutMs,
