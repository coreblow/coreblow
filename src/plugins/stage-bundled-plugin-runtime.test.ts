...
describe("stageBundledPluginRuntime", () => {
  it("stages bundled dist plugins as runtime wrappers and links staged dist node_modules", () => {
    const repoRoot = makeRepoRoot("coreblow-stage-bundled-runtime-");
  ...
  it("writes wrappers that forward plugin entry imports into canonical dist files", async () => {
    const repoRoot = makeRepoRoot("coreblow-stage-bundled-runtime-chunks-");
  ...
  it("stages root runtime sidecars that bundled plugin boundaries resolve directly", () => {
    const repoRoot = makeRepoRoot("coreblow-stage-bundled-runtime-sidecars-");
  ...
  it("keeps plugin command registration on the canonical dist graph when loaded from dist-runtime", async () => {
    const repoRoot = makeRepoRoot("coreblow-stage-bundled-runtime-commands-");
    ...
    fs.writeFileSync(
      ...
      [
        "const registry = globalThis.__coreblowTestPluginCommands ??= new Map();",
  ...
  it("copies package metadata files but symlinks other non-js plugin artifacts into the runtime overlay", () => {
    const repoRoot = makeRepoRoot("coreblow-stage-bundled-runtime-assets-");
    createDistPluginDir(repoRoot, "diffs");
    setupRepoFiles(repoRoot, {
      "dist/extensions/diffs/package.json": JSON.stringify(
        { name: "@coreblow/diffs", coreblow: { extensions: ["./index.js"] } },
        null,
        2,
      ),
      "dist/extensions/diffs/coreblow.plugin.json": "{}\n",
    ...
    expectRuntimeArtifactText({
      repoRoot,
      pluginId: "diffs",
      relativePath: "coreblow.plugin.json",
      expectedText: "{}\n",
      symbolicLink: false,
    });
    expectRuntimeArtifactText({
  ...
  it("preserves package metadata needed for bundled plugin discovery from dist-runtime", () => {
    const repoRoot = makeRepoRoot("coreblow-stage-bundled-runtime-discovery-");
    ...
    setupRepoFiles(repoRoot, {
      ...
      "dist/extensions/demo/coreblow.plugin.json": JSON.stringify(
        {
          id: "demo",
          channels: ["demo"],
    ...
    const env = {
      ...process.env,
      COREBLOW_BUNDLED_PLUGINS_DIR: runtimeExtensionsDir,
    };
  ...
  it("removes stale runtime plugin directories that are no longer in dist", () => {
    const repoRoot = makeRepoRoot("coreblow-stage-bundled-runtime-stale-");
  ...
  it("removes dist-runtime when the built bundled plugin tree is absent", () => {
    const repoRoot = makeRepoRoot("coreblow-stage-bundled-runtime-missing-");
  ...
  it("tolerates EEXIST when an identical runtime symlink is materialized concurrently", () => {
    const repoRoot = makeRepoRoot("coreblow-stage-bundled-runtime-eexist-");
