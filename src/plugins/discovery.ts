...
function resolveDiscoveryCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.COREBLOW_PLUGIN_DISCOVERY_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseDiscoveryCache(env: NodeJS.ProcessEnv): boolean {
  const disabled = env.COREBLOW_DISABLE_PLUGIN_DISCOVERY_CACHE?.trim();
  if (disabled) {
    return false;
  }
...
function deriveIdHint(params: {
  ...
  // package is scoped (example: @coreblow/voice-call -> voice-call).
    ...
    addCandidate({
      ...
      manifest: {
        ...
        ...(!packageManifest?.description && entry.packageDescription
          ...
          : {}),
        ...(!packageManifest?.coreblow && entry.packageManifest
          ? { coreblow: entry.packageManifest }
          : {}),
      },
      packageDir: rootDir,
      bundledManifest: entry.manifest,
      bundledManifestPath: path.join(rootDir, "coreblow.plugin.json"),
    });
  }
...
export function discoverCoreBlowPlugins(params: {
  workspaceDir?: string;
  extraPaths?: string[];
  ownershipUid?: number | null;
  cache?: boolean;
