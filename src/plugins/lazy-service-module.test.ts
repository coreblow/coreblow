...
describe("startLazyPluginServiceModule", () => {
  afterEach(() => {
    delete process.env.COREBLOW_LAZY_SERVICE_SKIP;
    delete process.env.COREBLOW_LAZY_SERVICE_OVERRIDE;
  });
  ...
  it("uses the override module when configured", async () => {
    process.env.COREBLOW_LAZY_SERVICE_OVERRIDE = "virtual:service";
    const start = createAsyncHookMock();
    const loadOverrideModule = vi.fn(async () => ({ startOverride: start }));

    await expectLifecycleStarted({
      overrideEnvVar: "COREBLOW_LAZY_SERVICE_OVERRIDE",
