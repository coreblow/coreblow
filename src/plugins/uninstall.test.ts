...
type PluginConfig = NonNullable<CoreBlowConfig["plugins"]>;
...
function createPluginConfig(params: {
  ...
  loadPaths?: string[];
  channels?: CoreBlowConfig["channels"];
}): CoreBlowConfig {
...
function createSinglePluginWithEmptySlotsConfig(): CoreBlowConfig {
...
function createSingleNpmInstallConfig(installPath: string): CoreBlowConfig {
  return createPluginConfig({
    entries: createSinglePluginEntries(),
    installs: {
    ...
    },
    {
      ...
      config: createPluginConfig({
        ...
        channels: {
          ...
          timbot: { sdkAppId: "123" },
        } as unknown as CoreBlowConfig["channels"],
      }),
      pluginId: "timbot",
      expectedChannels: {
        defaults: { groupPolicy: "opt-in" },
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "coreblow-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        ...
        installPath: "/tmp/not-coreblow-extensions/my-plugin",
      },
      extensionsDir,
    });
