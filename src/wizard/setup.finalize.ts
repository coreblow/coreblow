...
export async function finalizeSetupWizard(
    ...
    if (gatewayProbe.ok) {
      ...
      await prompter.note(
        [
          ...
          `Start now: ${formatCliCommand("coreblow gateway run")}`,
          `Or rerun with: ${formatCliCommand("coreblow onboard --install-daemon")}`,
          `Or skip this probe next time: ${formatCliCommand("coreblow onboard --skip-health")}`,
        ].join("\n"),
        "Gateway",
      );
    }
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        `Open the dashboard anytime: ${formatCliCommand("coreblow dashboard --no-open")}`,
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ? "Opened in your browser. Keep that tab to control CoreBlow."
            : "Copy/paste this URL in a browser on this machine to control CoreBlow.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("coreblow dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened; keep that tab to control CoreBlow."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the dashboard link above."
        : "Onboarding complete. Use the dashboard link above to control CoreBlow.",
  );
