...
import type { ConfigFileSnapshot, CoreBlowConfig } from "../config/config.js";
...
function isFeishuDocToolEnabled(cfg: CoreBlowConfig): boolean {
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your CoreBlow state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your CoreBlow state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectGatewayConfigFindings(
  ...
  const envTokenConfigured = hasNonEmptyString(env.COREBLOW_GATEWAY_TOKEN);
  const envPasswordConfigured = hasNonEmptyString(env.COREBLOW_GATEWAY_PASSWORD);
...
function collectBrowserControlFindings(
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("coreblow security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(env.COREBLOW_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(env.COREBLOW_GATEWAY_PASSWORD) ||
...
function collectLoggingFindings(cfg: CoreBlowConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: CoreBlowConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectExecRuntimeFindings(cfg: CoreBlowConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectOpenExecSurfacePaths(cfg: CoreBlowConfig): string[] {
  const channels = asRecord(cfg.channels);
  if (!channels) {
    return [];
  }
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deep?.gateway?.attempted && !deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("coreblow status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("coreblow security audit --deep")}".`,
    });
  }
  if (deepProbeResult?.authWarning) {
    findings.push({
      ...
      remediation: `Set COREBLOW_GATEWAY_TOKEN/COREBLOW_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("coreblow security audit --deep")}".`,
    });
  }
