...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: CoreBlowConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: CoreBlowConfig, accountId: string | null | undefined) => unknown;
  listAccountIds?: (cfg: CoreBlowConfig) => string[];
  isConfigured?: (account: unknown, cfg: CoreBlowConfig) => boolean;
  isEnabled?: (account: unknown, cfg: CoreBlowConfig) => boolean;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
async function runConfigAuditCases<T extends { cfg: CoreBlowConfig }>(
  cases: readonly T[],
...
async function runInstallMetadataAudit(
  ...
  return runSecurityAudit({
    ...
    includeChannelSecurity: false,
    stateDir,
    configPath: path.join(stateDir, "coreblow.json"),
...
describe("security audit", () => {
  ...
  const createFilesystemAuditFixture = async (label: string) => {
    ...
    const configPath = path.join(stateDir, "coreblow.json");
  ...
  const runSharedExtensionsAudit = async (config: CoreBlowConfig) => {
    return runSecurityAudit({
      config,
      includeFilesystem: true,
      includeChannelSecurity: false,
      stateDir: sharedExtensionsStateDir,
      configPath: path.join(sharedExtensionsStateDir, "coreblow.json"),
      execDockerRawFn: execDockerRawUnavailable,
    });
  };
  ...
  const createSharedCodeSafetyFixture = async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        coreblow: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  it("evaluates gateway auth presence and rate-limit guardrails", async () => {
    const cases = [
      ...
      },
      {
        ...
        run: async () => {
          const sourceConfig: CoreBlowConfig = {
            gateway: {
              bind: "lan",
              auth: {
                token: {
          ...
          const resolvedConfig: CoreBlowConfig = {
            gateway: {
              bind: "lan",
              auth: {},
            },
  ...
  it("evaluates safeBinTrustedDirs risk findings", async () => {
    const riskyGlobalTrustedDirs =
      process.platform === "win32"
        ...
        : ["/usr/local/bin", "/tmp/coreblow-safe-bins"];
    const cases = [
      {
    ...
    const cases = [
      {
          ...
          return runSecurityAudit({
            ...
            execDockerRawFn: (async (args: string[]) => {
              if (args[0] === "ps") {
                return {
                  stdout: Buffer.from(
                    "coreblow-sbx-browser-old\ncoreblow-sbx-browser-missing-hash\n",
                  ),
                  stderr: Buffer.alloc(0),
                  code: 0,
                };
              }
              if (args[0] === "inspect" && args.at(-1) === "coreblow-sbx-browser-old") {
                return {
              ...
              if (args[0] === "inspect" && args.at(-1) === "coreblow-sbx-browser-missing-hash") {
                return {
        ...
        assert: (res: SecurityAuditReport) => {
          ...
          expect(staleEpoch?.detail).toContain("coreblow-sbx-browser-old");
        },
      },
      {
        ...
        run: async () => {
          ...
          return runSecurityAudit({
            ...
            execDockerRawFn: (async (args: string[]) => {
              if (args[0] === "ps") {
                return {
                  stdout: Buffer.from("coreblow-sbx-browser-exposed\n"),
                  stderr: Buffer.alloc(0),
                  code: 0,
                };
              }
              if (args[0] === "inspect" && args.at(-1) === "coreblow-sbx-browser-exposed") {
                return {
              ...
              if (args[0] === "port" && args.at(-1) === "coreblow-sbx-browser-exposed") {
                return {
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-coreblow.json");
      ...
      cases
        ...
        .map((testCase) => ({
          run: async () => {
            ...
            const configPath = path.join(fixture.stateDir, "coreblow.json");
            await fs.writeFile(configPath, "{}\n", "utf-8");
            if (!isWindows) {
              await fs.chmod(configPath, 0o600);
            }
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): CoreBlowConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("keeps source-configured channel security findings when resolved inspection is incomplete", async () => {
    const cases = [
      ...
      },
      {
        ...
        plugin: (sourceConfig: CoreBlowConfig) =>
          stubChannelPlugin({
            id: "slack",
            label: "Slack",
            inspectAccount: (cfg) => {
  ...
  it.each([
    {
      ...
      detailIncludes: [
        ...
        "~/.coreblow/credentials/discord-allowFrom.json:team.owner",
      ],
  ...
  it("adds probe_failed warnings for deep probe failure modes", async () => {
    const cfg: CoreBlowConfig = { gateway: { mode: "local" } };
    const cases: Array<{
      name: string;
  ...
  it("evaluates hooks ingress auth and routing findings", async () => {
    ...
    } satisfies NonNullable<CoreBlowConfig["hooks"]>;
    ...
    const cases = [
      ...
      },
      {
        ...
        cfg: { hooks: unrestrictedBaseHooks } satisfies CoreBlowConfig,
        expectedFinding: "hooks.allowed_agent_ids_unrestricted",
        expectedSeverity: "warn" as const,
      },
      {
        name: "scores unrestricted hooks.allowedAgentIds by remote exposure",
        cfg: { gateway: { bind: "lan" }, hooks: unrestrictedBaseHooks } satisfies CoreBlowConfig,
        expectedFinding: "hooks.allowed_agent_ids_unrestricted",
        expectedSeverity: "critical" as const,
      },
      {
        name: "scores hooks request sessionKey override by local exposure",
        cfg: { hooks: requestSessionKeyHooks } satisfies CoreBlowConfig,
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await audit(cfg, {
      stateDir: "/Users/test/Dropbox/.coreblow",
      configPath: "/Users/test/Dropbox/.coreblow/coreblow.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    const cfg: CoreBlowConfig = { logging: { redactSensitive: "off" } };
    ...
    const cases = [
          ...
          runInstallMetadataAudit(
            {
              plugins: {
                installs: {
                  "voice-call": {
                    source: "npm",
                    spec: "@coreblow/voice-call@1.2.3",
              ...
              hooks: {
                internal: {
                  installs: {
                    "test-hooks": {
                      source: "npm",
                      spec: "@coreblow/test-hooks@1.2.3",
      ...
      },
      {
        ...
        run: async () => {
          ...
          await fs.writeFile(
            path.join(pluginDir, "package.json"),
            JSON.stringify({ name: "@coreblow/voice-call", version: "9.9.9" }),
            "utf-8",
          );
          await fs.writeFile(
            path.join(hookDir, "package.json"),
            JSON.stringify({ name: "@coreblow/test-hooks", version: "8.8.8" }),
            "utf-8",
          );
      ...
      },
      {
        ...
        run: async () => {
          ...
          await fs.writeFile(
            ...
            JSON.stringify({
              name: "escape-plugin",
              coreblow: { extensions: ["../outside.js"] },
            }),
          );
