...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        COREBLOW_BASE_ENV: "base",
        COREBLOW_TO_REMOVE: undefined,
      },
      env: {
        COREBLOW_TEST_ENV: "ok",
      },
    });

    expect(resolved.COREBLOW_BASE_ENV).toBe("base");
    expect(resolved.COREBLOW_TEST_ENV).toBe("ok");
    expect(resolved.COREBLOW_TO_REMOVE).toBeUndefined();
    expect(resolved.COREBLOW_CLI).toBe(COREBLOW_CLI_ENV_VALUE);
  });
