...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.coreblow${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.COREBLOW_PROFILE = profile;

  const stateDir = env.COREBLOW_STATE_DIR?.trim() || resolveProfileStateDir(profile, env, homedir);
  if (!env.COREBLOW_STATE_DIR?.trim()) {
    env.COREBLOW_STATE_DIR = stateDir;
  }

  if (!env.COREBLOW_CONFIG_PATH?.trim()) {
    env.COREBLOW_CONFIG_PATH = path.join(stateDir, "coreblow.json");
  }

  if (profile === "dev" && !env.COREBLOW_GATEWAY_PORT?.trim()) {
    env.COREBLOW_GATEWAY_PORT = "19001";
  }
}
