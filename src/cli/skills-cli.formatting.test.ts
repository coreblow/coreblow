...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "coreblow-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "coreblow-bundled-skills-test-"));
    process.env.COREBLOW_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
