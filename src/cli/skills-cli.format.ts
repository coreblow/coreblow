...
function appendCoreHubHint(output: string, json?: boolean): string {
  if (json) {
    return output;
  }
  return `${output}\n\nTip: use \`coreblow skills search\`, \`coreblow skills install\`, and \`coreblow skills update\` for CoreHub-backed skills.`;
}
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("coreblow skills list")}\` to see all skills.`
      : "No skills found.";
    return appendCoreHubHint(message, opts.json);
  }
  ...
  return appendCoreHubHint(lines.join("\n"), opts.json);
}
...
export function formatSkillInfo(
  ...
  if (skill.primaryEnv && skill.missing.env.includes(skill.primaryEnv)) {
    ...
    lines.push(
      `  Save via CLI: ${formatCliCommand(`coreblow config set skills.entries.${safeSkillKey}.apiKey YOUR_KEY`)}`,
    );
    lines.push(
      `  Stored in: ${theme.muted("~/.coreblow/coreblow.json")} ${theme.muted(`(skills.entries.${safeSkillKey}.apiKey)`)}`,
    );
  }
