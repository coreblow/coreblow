...
function warnInlinePasswordFlag() {
  defaultRuntime.error(
    "Warning: --password can be exposed via process listings. Prefer --password-file or COREBLOW_GATEWAY_PASSWORD.",
  );
}
...
async function runGatewayCommand(opts: GatewayRunOpts) {
  const isDevProfile = process.env.COREBLOW_PROFILE?.trim().toLowerCase() === "dev";
  ...
  if (opts.cliBackendLogs || opts.claudeCliLogs) {
    ...
    process.env.COREBLOW_CLI_BACKEND_LOG_OUTPUT = "1";
  }
  ...
  if (opts.rawStream) {
    process.env.COREBLOW_RAW_STREAM = "1";
  }
  const rawStreamPath = toOptionString(opts.rawStreamPath);
  if (rawStreamPath) {
    process.env.COREBLOW_RAW_STREAM_PATH = rawStreamPath;
  }
  ...
  if (process.env.COREBLOW_SERVICE_MARKER?.trim()) {
  ...
  if (!opts.allowUnconfigured && mode !== "local") {
    if (!configExists) {
      defaultRuntime.error(
        `Missing config. Run \`${formatCliCommand("coreblow setup")}\` or set gateway.mode=local (or pass --allow-unconfigured).`,
      );
    } else {
      defaultRuntime.error(
  ...
  if (resolvedAuthMode === "password" && !passwordConfigured) {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.password (or COREBLOW_GATEWAY_PASSWORD), or pass --password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
  ...
  if (
    ...
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.token/password (or COREBLOW_GATEWAY_TOKEN/COREBLOW_GATEWAY_PASSWORD) or pass --token/--password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
  ...
  try {
    ...
    if (
      ...
      defaultRuntime.error(
        `Gateway failed to start: ${errMessage}\nIf the gateway is supervised, stop it with: ${formatCliCommand("coreblow gateway stop")}`,
      );
      try {
...
export function addGatewayRunCommand(cmd: Command): Command {
  return cmd
    .option("--port <port>", t("cli_help.gateway.run.options.port"))
    .option(
      "--bind <mode>",
      t("cli_help.gateway.run.options.bind"),
    )
    .option(
      "--token <token>",
      t("cli_help.gateway.run.options.shared_credential"),
    )
    .option(
      "--auth <mode>",
      t("cli_help.gateway.run.options.auth", { modes: formatModeChoices(GATEWAY_AUTH_MODES) }),
    )
    .option("--password <password>", t("cli_help.gateway.run.options.passphrase"))
    .option("--password-file <path>", t("cli_help.gateway.run.options.passphrase_file"))
    .option(
      "--tailscale <mode>",
      t("cli_help.gateway.run.options.tailscale", {
        modes: formatModeChoices(GATEWAY_TAILSCALE_MODES),
      }),
    )
    .option(
      "--tailscale-reset-on-exit",
      t("cli_help.gateway.run.options.tailscale_reset_on_exit"),
      false,
    )
    .option(
      "--allow-unconfigured",
      t("cli_help.gateway.run.options.allow_unconfigured"),
      false,
    )
    .option("--dev", t("cli_help.gateway.run.options.dev"), false)
    .option(
      "--reset",
      t("cli_help.gateway.run.options.reset"),
      false,
    )
    .option("--force", t("cli_help.gateway.run.options.force"), false)
    .option("--verbose", t("cli_help.gateway.run.options.verbose"), false)
    .option(
      "--cli-backend-logs",
      t("cli_help.gateway.run.options.cli_backend_logs"),
      false,
    )
    .option("--claude-cli-logs", t("cli_help.gateway.run.options.claude_cli_logs"), false)
    .option("--ws-log <style>", t("cli_help.gateway.run.options.ws_log"), "auto")
    .option("--compact", t("cli_help.gateway.run.options.compact"), false)
    .option("--raw-stream", t("cli_help.gateway.run.options.raw_stream"), false)
    .option("--raw-stream-path <path>", t("cli_help.gateway.run.options.raw_stream_path"))
