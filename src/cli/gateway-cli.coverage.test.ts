...
const gatewayStatusCommand = vi.fn(async (..._args: any[]) => {});
...
describe("gateway-cli coverage", () => {
  ...
  it("registers gateway discover and prints json output", async () => {
    ...
    discoverGatewayBeacons.mockResolvedValueOnce([
      {
        instanceName: "Studio (CoreBlow)",
        displayName: "Studio",
        domain: "coreblow.internal.",
        host: "studio.coreblow.internal",
        port: 18789,
  ...
  it("uses env/config port when --port is omitted", async () => {
    await withEnvOverride({ COREBLOW_GATEWAY_PORT: "19001" }, async () => {
      resetRuntimeCapture();
