...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    ...
    .addHelpText("after", () => {
      const examples = [
        ["coreblow update", "Update a source checkout (git)"],
        ["coreblow update --channel beta", "Switch to beta channel (git + npm)"],
        ["coreblow update --channel dev", "Switch to dev channel (git + npm)"],
        ["coreblow update --tag beta", "One-off update to a dist-tag or version"],
        ["coreblow update --tag main", "One-off package install from GitHub main"],
        ["coreblow update --dry-run", "Preview actions without changing anything"],
        ["coreblow update --no-restart", "Update without restarting the service"],
        ["coreblow update --json", "Output result as JSON"],
        ["coreblow update --yes", "Non-interactive (accept downgrade prompts)"],
        ["coreblow update wizard", "Interactive update wizard"],
        ["coreblow --update", "Shorthand for coreblow update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run coreblow update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.coreblow.com/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["coreblow update status", "Show channel + version status."],
          ["coreblow update status --json", "JSON output."],
          ["coreblow update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.coreblow.com/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
