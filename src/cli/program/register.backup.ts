...
export function registerBackupCommand(program: Command) {
  const backup = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/backup", "docs.coreblow.com/cli/backup")}\n`,
    );
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["coreblow backup create", "Create a timestamped backup in the current directory."],
          [
            "coreblow backup create --output ~/Backups",
            "Write the archive into an existing backup directory.",
          ],
          [
            "coreblow backup create --dry-run --json",
            "Preview the archive plan without writing any files.",
          ],
          [
            "coreblow backup create --verify",
            "Create the archive and immediately validate its manifest and payload layout.",
          ],
          [
            "coreblow backup create --no-include-workspace",
            "Back up state/config without agent workspace files.",
          ],
          ["coreblow backup create --only-config", "Back up only the active JSON config file."],
        ])}`,
    )
    .action(async (opts) => {
  ...
  backup
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "coreblow backup verify ./2026-03-09T00-00-00.000Z-coreblow-backup.tar.gz",
            "Check that the archive structure and manifest are intact.",
          ],
          [
            "coreblow backup verify ~/Backups/latest.tar.gz --json",
