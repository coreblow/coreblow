...
export function registerOnboardCommand(program: Command) {
  const command = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/onboard", "docs.coreblow.com/cli/onboard")}\n`,
    )
    .option("--workspace <dir>", "Agent workspace directory (default: ~/.coreblow/workspace)")
    .option(
      "--reset",
  ...
  command
    ...
    .option(
      ...
      "Gateway token SecretRef env var name (token auth; e.g. COREBLOW_GATEWAY_TOKEN)",
    )
