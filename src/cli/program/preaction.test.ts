...
const { setVerboseMock, emitCliBannerMock, ensureConfigReadyMock, ensurePluginRegistryLoadedMock, routeLogsToStderrMock } = vi.hoisted(() => ({
  setVerboseMock: vi.fn(),
  emitCliBannerMock: vi.fn(),
  ensureConfigReadyMock: vi.fn(async () => {}),
  ensurePluginRegistryLoadedMock: vi.fn(),
  routeLogsToStderrMock: vi.fn(),
}));
...
beforeEach(() => {
  ...
  originalHideBanner = process.env.COREBLOW_HIDE_BANNER;
  ...
  delete process.env.COREBLOW_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.COREBLOW_HIDE_BANNER = originalHideBanner;
  }
});
...
describe("registerPreActionHooks", () => {
  ...
  function buildProgram() {
    const program = new Command().name("coreblow");
    program
      .command("status")
      .option("--json")
      .action(() => {});
  ...
  it("handles debug mode and plugin-required command preaction", async () => {
    ...
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "coreblow", "status", "--debug"],
    });
    ...
    expect(processTitleSetSpy).toHaveBeenCalledWith("coreblow-status");

    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["message", "send"],
      processArgv: ["node", "coreblow", "message", "send"],
    });
  ...
  it("keeps setup alias and channels add manifest-first", async () => {
    await runPreAction({
      parseArgv: ["onboard"],
      processArgv: ["node", "coreblow", "onboard"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "coreblow", "channels", "add"],
    });
  ...
  it("only allows invalid config for explicit Matrix reinstall requests", async () => {
    await runPreAction({
      parseArgv: ["plugins", "install", "@coreblow/matrix"],
      processArgv: ["node", "coreblow", "plugins", "install", "@coreblow/matrix"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "coreblow", "plugins", "install", "alpha"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "coreblow", "plugins", "install", "./extensions/matrix"],
    });
    ...
    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["plugins", "install", "@coreblow/matrix", "--marketplace", "local/repo"],
      processArgv: [
        "node",
        "coreblow",
        "plugins",
  ...
  it("skips help/version preaction and respects banner opt-out", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "coreblow", "--version"],
    });
    ...
    vi.clearAllMocks();
    process.env.COREBLOW_HIDE_BANNER = "1";

    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "coreblow", "status"],
    });
  ...
  it("applies --json stdout suppression only for explicit JSON output commands", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "coreblow", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "coreblow", "update", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "coreblow", "config", "set", "gateway.auth.mode", "{bad", "--json"],
    });
  ...
  it("routes logs to stderr in --json mode so stdout stays clean", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "coreblow", "agents", "list", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "coreblow", "config", "set", "gateway.auth.mode", "local", "--json"],
    });
    ...
    await runPreAction({
      parseArgv: ["agents", "list"],
      processArgv: ["node", "coreblow", "agents", "list"],
    });
  ...
  it("bypasses config guard for config validate", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "coreblow", "config", "validate"],
    });
  ...
  it("bypasses config guard for config validate when root option values are present", async () => {
    await runPreAction({
      parseArgv: ["config", "validate"],
      processArgv: ["node", "coreblow", "--profile", "work", "config", "validate"],
    });
  ...
  it("bypasses config guard for config schema", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "coreblow", "config", "schema"],
    });
  ...
  it("bypasses config guard for backup create", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "coreblow", "backup", "create", "--json"],
    });
