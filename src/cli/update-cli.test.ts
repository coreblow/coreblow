...
import type { CoreBlowConfig, ConfigFileSnapshot } from "../config/types.coreblow.js";
...
vi.mock("../infra/coreblow-root.js", () => ({
  resolveCoreBlowPackageRoot: vi.fn(),
}));
...
const { resolveCoreBlowPackageRoot } = await import("../infra/coreblow-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/coreblow-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as CoreBlowConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/coreblow-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveCoreBlowPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("coreblow-update");
    setTty(false);
  ...
  const setupUpdatedRootRefresh = (params?: {
    ...
    const root = createCaseDir("coreblow-updated-root");
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveCoreBlowPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("coreblow");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/coreblow-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "coreblow-gateway" }],
      hints: [],
    });
  ...
  it.each([
    {
      ...
      assert: () => {
        ...
        expect(logs.join("\n")).toContain("CoreBlow update status");
      },
    },
    {
      name: "json output",
  ...
  it("parses update status --json as the subcommand option", async () => {
    ...
    await program.parseAsync(["node", "coreblow", "update", "status", "--json"]);
  ...
  it.each([
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as CoreBlowConfig,
        });
      },
    ...
    async ({ mode, options, prepare, expectedChannel, expectedTag, expectedPersistedChannel }) => {
      ...
      if (expectedChannel !== undefined) {
        ...
        expect(runGatewayUpdate).not.toHaveBeenCalled();
        expect(runCommandWithTimeout).toHaveBeenCalledWith(
          ["npm", "i", "-g", "coreblow@latest", "--no-fund", "--no-audit", "--loglevel=error"],
          expect.any(Object),
        );
      }
  ...
  it.each([
    ...
    },
    {
      ...
      run: async () => {
        mockPackageInstallStatus(createCaseDir("coreblow-update"));
        await updateCommand({ yes: true, tag: "github:coreblow/coreblow#main" });
      },
      expectedSpec: "github:coreblow/coreblow#main",
    },
    {
      ...
      expectedSpec: "http://10.211.55.2:8138/coreblow-next.tgz",
    },
  ] as const)(
  ...
  it("fails package updates when the installed correction version does not match the requested target", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "coreblow", version: "2026.3.23" }),
      "utf-8",
    );
  ...
  it("prepends portable Git PATH for package updates on Windows", async () => {
    ...
    const tempDir = createCaseDir("coreblow-update");
    const localAppData = createCaseDir("coreblow-localappdata");
