...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("coreblow-daemon-install-int-");
    configPath = path.join(tempHome, "coreblow.json");
    process.env.HOME = tempHome;
    process.env.COREBLOW_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.COREBLOW_GATEWAY_TOKEN).toBeUndefined();
  });
});
