...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# CoreBlow Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(coreblow completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# CoreBlow Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(coreblow completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/completion", "docs.coreblow.com/cli/completion")}\n`,
    )
    .addOption(
    ...
    .option(
      "--write-state",
      "Write completion scripts to $COREBLOW_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "coreblow") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
