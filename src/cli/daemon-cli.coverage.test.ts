...
const buildGatewayInstallPlan = vi.fn(
  async (params: { port: number; token?: string; env?: NodeJS.ProcessEnv }) => ({
    ...
    environment: {
      COREBLOW_GATEWAY_PORT: String(params.port),
      ...(params.token ? { COREBLOW_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    process.env.COREBLOW_STATE_DIR = "/tmp/coreblow-cli-state";
    process.env.COREBLOW_CONFIG_PATH = "/tmp/coreblow-cli-state/coreblow.json";
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        COREBLOW_PROFILE: "dev",
        COREBLOW_STATE_DIR: "/tmp/coreblow-daemon-state",
        COREBLOW_CONFIG_PATH: "/tmp/coreblow-daemon-state/coreblow.json",
        COREBLOW_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.coreblow.gateway.plist",
    });
