...
const piEmbeddedMocks = getSharedMocks("coreblow.trigger-handling.pi-embedded-mocks", () => ({
...
const modelCatalogMocks = getSharedMocks("coreblow.trigger-handling.model-catalog-mocks", () => ({
...
const modelFallbackMocks = getSharedMocks("coreblow.trigger-handling.model-fallback-mocks", () => ({
...
const webSessionMocks = getSharedMocks("coreblow.trigger-handling.web-session-mocks", () => ({
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "coreblow-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".coreblow", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): CoreBlowConfig {
  return {
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-5" },
        workspace: join(home, "coreblow"),
