...
import { scheduleGatewaySigusr1Restart, triggerCoreBlowRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting CoreBlow in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerCoreBlowRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting CoreBlow via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
