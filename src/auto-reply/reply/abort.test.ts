...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop coreblow!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@coreblow_bot", { botUsername: "coreblow_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@coreblow_bot", { botUsername: "coreblow_bot" })).toBe(true);
