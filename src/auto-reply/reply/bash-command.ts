...
function resolveForegroundMs(cfg: CoreBlowConfig): number {
...
export async function handleBashChatCommand(params: {
  ...
  if (!isCommandFlagEnabled(params.cfg, "bash")) {
    return buildDisabledCommandReply({
      label: "bash",
      configKey: "bash",
      docsUrl: "https://docs.coreblow.com/tools/slash-commands#config",
    });
  }
  ...
  try {
    ...
    const output =
      result.details?.status === "completed"
        ...
        : result.content.map((chunk: any) => (chunk.type === "text" ? chunk.text : "")).join("\n");
    return {
      text: [
        `⚙️ bash: ${commandText}`,
        `Exit: ${exitCode}`,
