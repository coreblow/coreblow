/**
 * auto-reply/reply/provider-dispatcher-class.ts
 * Route reply to the correct AI provider based on model selection.
 * Restored from gateway/src/ (lost in consolidation commit 67f492c72).
...
export class ProviderDispatcher {
    ...
    private recordError(providerId: string, _error: Error): void {
        const h = this.health.get(providerId);
        if (!h) return;
        h.errorCount++;
