...
This document describes how the CoreBlow ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`coreblow acp` exposes an ACP agent over stdio and forwards prompts to a running
CoreBlow Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
`coreblow acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It is designed to route IDE prompts into an existing CoreBlow Gateway
...
| Session modes                                                         | Partial     | `session/set_mode` is supported and the bridge exposes initial Gateway-backed session controls for thought level, tool verbosity, reasoning, usage detail, and elevated actions. |
| Session info and usage updates                                        | Partial     | The bridge emits `session_info_update` and best-effort `usage_update` notifications from cached Gateway session snapshots. |
| Tool streaming                                                        | Partial     | `tool_call` / `tool_call_update` events include raw I/O, text content, and best-effort file locations. |
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the CoreBlow gateway or agent instead.                                                                                                                                     |
...
| Client terminal methods (`terminal/*`)                                | Unsupported | The bridge does not create ACP client terminals.                                                                                                                                                       |
...
- If multiple ACP clients share the same Gateway session key, event and cancel
  routing are best-effort rather than strictly isolated per client.
...
- `session_info_update` and `usage_update` are derived from Gateway session
  snapshots, not live ACP-native runtime accounting.

## How can I use this

Use ACP when an IDE or tooling speaks Agent Client Protocol and you want it to
drive a CoreBlow Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `coreblow acp` over stdio.

Example config:

```bash
coreblow config set gateway.remote.url wss://gateway-host:18789
coreblow config set gateway.remote.token <token>
```

Example run:

```bash
coreblow acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
coreblow acp --session agent:main:main
coreblow acp --session agent:design:main
coreblow acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select "CoreBlow ACP" to start a thread.

## Execution Model

- ACP client spawns `coreblow acp` and speaks ACP messages over stdio.
...
```bash
coreblow acp --session agent:main:main
coreblow acp --session-label "support inbox"
coreblow acp --reset-session
```
...
`coreblow acp` resolves the Gateway URL and auth from CLI flags or config:
