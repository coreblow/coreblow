...
private const val NOTIFICATION_CHANNEL_BASE_ID = "coreblow.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "CoreBlow Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "CoreBlow Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "CoreBlow Active")
      }
