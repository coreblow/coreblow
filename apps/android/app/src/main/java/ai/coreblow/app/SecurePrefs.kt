...
/**
 * Encrypted preference store for gateway tokens, device identity,
 * and node configuration. Uses AndroidX Security for AES-256 encryption.
 */
class SecurePrefs(
    context: Context,
    ...
    companion object {
        val defaultWakeWords: List<String> = listOf("coreblow", "claude")
        private const val displayNameKey = "node.displayName"
        private const val locationModeKey = "location.enabledMode"
        private const val voiceWakeModeKey = "voiceWake.mode"
        private const val plainPrefsName = "coreblow.node"
        private const val securePrefsName = "coreblow.node.secure"
    }
    ...
    private val securePrefs: SharedPreferences by lazy {
        securePrefsOverride ?: createSecurePrefs(appContext, securePrefsName)
    }
    ...
    private val _displayName = MutableStateFlow(loadOrMigrateDisplayName(context = context))
    ...
    private val _lastDiscoveredStableId =
        MutableStateFlow(plainPrefs.getString("gateway.lastDiscoveredStableID", "") ?: "")
    ...
    fun loadGatewayToken(): String? {
        val manual = _gatewayToken.value.trim().ifEmpty {
    ...
    fun loadGatewayBootstrapToken(): String? {
        ...
        val stored = _gatewayBootstrapToken.value.trim().ifEmpty {
            val persisted = securePrefs.getString(key, null)?.trim().orEmpty()
            if (persisted.isNotEmpty()) _gatewayBootstrapToken.value = persisted
            persisted
        }
        return stored.takeIf { it.isNotEmpty() }
    }
