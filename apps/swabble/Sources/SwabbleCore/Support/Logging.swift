...
public enum LogLevel: String, Comparable, CaseIterable, Sendable {
    ...
    var rank: Int { switch self { case .trace: 0; case .debug: 1; case .info: 2; case .warn: 3; case .error: 4 } }
...
public struct Logger: Sendable {
    ...
    public func log(_ lvl: LogLevel, _ message: String) {
        guard lvl >= level else { return }
        let ts = ISO8601DateFormatter().string(from: Date())
        print("[\(lvl.rawValue.uppercased())] \(ts) | \(message)")
    }
...
extension LogLevel { public init?(configValue: String) { self.init(rawValue: configValue.lowercased()) } }
