...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__COREBLOW_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<CoreBlowTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.coreblow", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = CoreBlowTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> CoreBlowTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return CoreBlowTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> CoreBlowTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> CoreBlowTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return CoreBlowTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> CoreBlowTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = CoreBlowTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: CoreBlowTalkPTTStopPayload) {
