...
final class RemindersService: RemindersServicing {
    func list(params: CoreBlowRemindersListParams) async throws -> CoreBlowRemindersListPayload {
        ...
        let payload: [CoreBlowReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return CoreBlowReminderPayload(
        ...
        return CoreBlowRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: CoreBlowRemindersAddParams) async throws -> CoreBlowRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = CoreBlowReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return CoreBlowRemindersAddPayload(reminder: payload)
    }
