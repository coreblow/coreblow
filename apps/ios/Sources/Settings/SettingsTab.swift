...
struct SettingsTab: View {
    ...
    @AppStorage("location.enabledMode") private var locationEnabledModeRaw: String = CoreBlowLocationMode.off.rawValue
    ...
    @State private var lastLocationModeRaw: String = CoreBlowLocationMode.off.rawValue
    ...
    private let gatewayLogger = Logger(subsystem: "ai.coreblow.ios", category: "GatewaySettings")
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section("Gateway") {
                    ...
                    if self.isGatewayConnected {
                        self.gatewayAgentPickerView
                        Button("Disconnect", role: .destructive) {
                            self.appModel.disconnectGateway()
                        }
                    } else {
                        self.gatewayPairingSetupView
                    }
                }

                Section("Device") {
                    AnyView(Toggle("Voice Wake", isOn: self.$voiceWakeEnabled))
                    AnyView(Toggle("Talk Mode", isOn: self.$talkEnabled))
                    AnyView(Toggle("Show Talk Control", isOn: self.$talkButtonEnabled))
                    AnyView(Toggle("Background Listening", isOn: self.$talkBackgroundEnabled))
                    AnyView(Toggle("Allow Camera", isOn: self.$cameraEnabled))
                    AnyView(Toggle("Prevent Sleep", isOn: self.$preventSleep))
                }
                ...
                Section("Advanced") {
                    ...
                    TextField("Default Share Instruction", text: self.$defaultShareInstruction, axis: .vertical)
                        .lineLimit(2 ... 6)
                    Button("Connect Manual") {
                        Task { await self.connectManual() }
                    }
                    .disabled(self.manualConnectDisabled)
            ...
            .onAppear {
                self.settingsDidAppear()
            }
            .onChange(of: self.selectedAgentPickerId) { _, newValue in
                self.selectedAgentPickerDidChange(newValue)
            }
            .onChange(of: self.appModel.selectedAgentId ?? "") { _, newValue in
                self.appSelectedAgentDidChange(newValue)
            }
            .onChange(of: self.preferredGatewayStableID) { _, newValue in
                self.preferredGatewayStableIDDidChange(newValue)
            }
            .onChange(of: self.gatewayToken) { _, newValue in
                self.gatewayTokenDidChange(newValue)
            }
            .onChange(of: self.gatewayPassword) { _, newValue in
                self.gatewayPasswordDidChange(newValue)
            }
            ...
            .onChange(of: self.appModel.gatewayServerName) { _, newValue in
                self.gatewayServerNameDidChange(newValue)
            }
            .onChange(of: self.appModel.gatewayStatusText) { _, newValue in
                self.gatewayStatusTextDidChange(newValue)
            }
            .onChange(of: self.locationEnabledModeRaw) { _, newValue in
                self.locationModeDidChange(newValue)
            }
        }
        .gatewayTrustPromptAlert()
    }

    private func settingsDidAppear() {
    ...
    private func selectedAgentPickerDidChange(_ value: String) {
        let trimmed = value.trimmingCharacters(in: .whitespacesAndNewlines)
        self.appModel.setSelectedAgentId(trimmed.isEmpty ? nil : trimmed)
    }

    private func appSelectedAgentDidChange(_ value: String) {
        if value != self.selectedAgentPickerId {
            self.selectedAgentPickerId = value
        }
    }

    private func preferredGatewayStableIDDidChange(_ value: String) {
        let trimmed = value.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else { return }
        GatewaySettingsStore.savePreferredGatewayStableID(trimmed)
    }

    private func gatewayTokenDidChange(_ value: String) {
    ...
    private func gatewayPasswordDidChange(_ value: String) {
        guard !self.suppressCredentialPersist else { return }
        let trimmed = value.trimmingCharacters(in: .whitespacesAndNewlines)
        let instanceId = self.instanceId.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !instanceId.isEmpty else { return }
    ...
    private func gatewayServerNameDidChange(_ value: String?) {
        if value != nil {
            self.setupCode = ""
    ...
    private func gatewayStatusTextDidChange(_ value: String) {
    ...
    private func locationModeDidChange(_ value: String) {
        let previous = self.lastLocationModeRaw
        self.lastLocationModeRaw = value
        guard let mode = CoreBlowLocationMode(rawValue: value) else { return }
        Task {
    ...
    @ViewBuilder
    private var gatewayDisclosureView: some View {
        DisclosureGroup(isExpanded: self.$gatewayExpanded) {
            ...
            self.gatewayStatusRowsView

            if let serverName = self.appModel.gatewayServerName {
                self.gatewayConnectedDetailsView(serverName: serverName)
            } else {
                self.gatewayList(showing: .all)
            }

            DisclosureGroup("Advanced") {
                self.gatewayAdvancedSettingsView
            }
        } label: {
            self.gatewayDisclosureLabel
        }
    }

    @ViewBuilder
    private var gatewayDisclosureLabel: some View {
        HStack(spacing: 10) {
            Circle()
    ...
    @ViewBuilder
    private var gatewayAgentPickerView: some View {
        Picker("Bot", selection: self.$selectedAgentPickerId) {
            Text("Default").tag("")
            ForEach(self.gatewayAgentPickerItems, id: \.id) { agent in
    ...
    private var gatewayAgentPickerItems: [(id: String, name: String)] {
        ...
        return self.appModel.gatewayAgents
            ...
            .map { agent in
                ...
                return (id: agent.id, name: name.isEmpty ? agent.id : name)
            }
    }

    @ViewBuilder
    private var gatewayStatusRowsView: some View {
        if self.appModel.gatewayServerName == nil {
            LabeledContent("Discovery", value: self.gatewayController.discoveryStatusText)
        }
        LabeledContent("Status", value: self.appModel.gatewayStatusText)
    ...
    @ViewBuilder
    private func gatewayConnectedDetailsView(serverName: String) -> some View {
        LabeledContent("Server", value: serverName)
    ...
    @ViewBuilder
    private var deviceFeaturesView: some View {
        DisclosureGroup("Features") {
            ...
            self.featureToggle(
                ...
                help: "Enables voice conversation mode with your connected CoreBlow agent.") { newValue in
            ...
            self.featureToggle(
                ...
                help: "Allows the gateway to request photos or short video clips while CoreBlow is foregrounded.")
            ...
            HStack(spacing: 8) {
                ...
                Button {
                    self.activeFeatureHelp = FeatureHelp(
                        ...
                        message: "Controls location permissions for CoreBlow. Off disables location tools, While Using enables foreground location, and Always enables background location.")
                } label: {
            ...
            Picker("Location Access", selection: self.$locationEnabledModeRaw) {
                Text("Off").tag(CoreBlowLocationMode.off.rawValue)
                Text("While Using").tag(CoreBlowLocationMode.whileUsing.rawValue)
                Text("Always").tag(CoreBlowLocationMode.always.rawValue)
            }
            .labelsHidden()
            ...
            self.featureToggle(
                ...
                help: "Keeps the screen awake while CoreBlow is open.")

            DisclosureGroup("Advanced") {
                self.deviceAdvancedSettingsView
            }
        }
    }
    ...
    @ViewBuilder
    private var gatewayAdvancedSettingsView: some View {
        Toggle("Use Manual Gateway", isOn: self.$manualGatewayEnabled)
        ...
        Text("Use this when mDNS/Bonjour discovery is blocked. Leave port empty for 443 on tailnet DNS (TLS) or 18789 otherwise.")
            .font(.footnote)
            .foregroundStyle(.secondary)
    ...
    private var manualConnectDisabled: Bool {
    ...
    @ViewBuilder
    private var gatewayPairingSetupView: some View {
        Text("""
        1. Open a chat with your CoreBlow agent and send /pair
    ...
    @ViewBuilder
    private var deviceAdvancedSettingsView: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("Talk Voice (Gateway)")
                .font(.footnote.weight(.semibold))
                .foregroundStyle(.secondary)
            LabeledContent("Provider", value: "ElevenLabs")
            LabeledContent("API Key", value: self.gatewayTalkAPIKeyStatusText)
            LabeledContent(
                "Default Model",
        ...
        HStack(spacing: 8) {
            ...
            Button {
                self.activeFeatureHelp = FeatureHelp(
                    ...
                    message: "Appends this instruction when sharing content into CoreBlow from iOS.")
            } label: {
                Image(systemName: "info.circle")
                    .foregroundStyle(.secondary)
            }
    ...
    private var gatewayTalkAPIKeyStatusText: String {
    ...
    private func friendlyGatewayMessage(from raw: String) -> String? {
        ...
        if lower.contains("pairing required") {
            return "Pairing required. Go back to your CoreBlow chat and run /pair approve, then tap Connect again."
        }
