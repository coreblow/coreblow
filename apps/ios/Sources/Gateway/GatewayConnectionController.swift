...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [CoreBlowCapability.canvas.rawValue, CoreBlowCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(CoreBlowCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(CoreBlowCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = CoreBlowLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(CoreBlowCapability.location.rawValue) }

        caps.append(CoreBlowCapability.device.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(CoreBlowCapability.watch.rawValue)
        }
        caps.append(CoreBlowCapability.photos.rawValue)
        caps.append(CoreBlowCapability.contacts.rawValue)
        caps.append(CoreBlowCapability.calendar.rawValue)
        caps.append(CoreBlowCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(CoreBlowCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            CoreBlowScreenCommand.record.rawValue,
            CoreBlowSystemCommand.notify.rawValue,
            CoreBlowChatCommand.push.rawValue,
        ...
        if caps.contains(CoreBlowCapability.camera.rawValue) {
            commands.append(CoreBlowCameraCommand.list.rawValue)
            commands.append(CoreBlowCameraCommand.snap.rawValue)
            commands.append(CoreBlowCameraCommand.clip.rawValue)
        }
        if caps.contains(CoreBlowCapability.location.rawValue) {
            commands.append(CoreBlowLocationCommand.get.rawValue)
        }
        if caps.contains(CoreBlowCapability.device.rawValue) {
            commands.append(CoreBlowDeviceCommand.status.rawValue)
            commands.append(CoreBlowDeviceCommand.info.rawValue)
        }
        if caps.contains(CoreBlowCapability.watch.rawValue) {
            commands.append(CoreBlowWatchCommand.status.rawValue)
            commands.append(CoreBlowWatchCommand.notify.rawValue)
        }
        if caps.contains(CoreBlowCapability.photos.rawValue) {
            commands.append(CoreBlowPhotosCommand.latest.rawValue)
        }
        if caps.contains(CoreBlowCapability.contacts.rawValue) {
            commands.append(CoreBlowContactsCommand.search.rawValue)
            commands.append(CoreBlowContactsCommand.add.rawValue)
        }
        if caps.contains(CoreBlowCapability.calendar.rawValue) {
            commands.append(CoreBlowCalendarCommand.events.rawValue)
            commands.append(CoreBlowCalendarCommand.add.rawValue)
        }
        if caps.contains(CoreBlowCapability.reminders.rawValue) {
            commands.append(CoreBlowRemindersCommand.list.rawValue)
            commands.append(CoreBlowRemindersCommand.add.rawValue)
        }
        if caps.contains(CoreBlowCapability.motion.rawValue) {
            commands.append(CoreBlowMotionCommand.activity.rawValue)
            commands.append(CoreBlowMotionCommand.pedometer.rawValue)
        }
