...
@MainActor
final class CoreBlowAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.coreblow.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.coreblow.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.coreblow.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "coreblow.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "coreblow.watch.promptId"
    static let sessionKeyKey = "coreblow.watch.sessionKey"
    static let actionPrimaryIDKey = "coreblow.watch.action.primary.id"
    static let actionPrimaryLabelKey = "coreblow.watch.action.primary.label"
    static let actionSecondaryIDKey = "coreblow.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "coreblow.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "coreblow.watch.action.primary"
    static let actionSecondaryIdentifier = "coreblow.watch.action.secondary"
    static let actionIdentifierPrefix = "coreblow.watch.action."
    static let actionIDKeyPrefix = "coreblow.watch.action.id."
    static let actionLabelKeyPrefix = "coreblow.watch.action.label."
    static let categoryPrefix = "coreblow.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: CoreBlowWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> CoreBlowWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return CoreBlowWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "CoreBlow" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [CoreBlowWatchAction]) -> [UNNotificationAction] {
...
struct CoreBlowApp: App {
    ...
    @UIApplicationDelegateAdaptor(CoreBlowAppDelegate.self) private var appDelegate
...
extension CoreBlowApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("CoreBlow: installing uncaught exception handler")
