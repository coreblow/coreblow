...
@Suite(.serialized) struct GatewayConnectionSecurityTests {
    @MainActor private func makeController() -> GatewayConnectionController {
    ...
    {
        let endpoint: NWEndpoint = .service(name: "Test", type: "_coreblow-gw._tcp", domain: "local.", interface: nil)
    ...
    private func clearTLSFingerprint(stableID: String) {
        let suite = UserDefaults(suiteName: "ai.coreblow.shared") ?? .standard
    ...
    @Test @MainActor func manualConnectionsForceTLSForNonLoopbackHosts() async {
        ...
        #expect(controller._test_resolveManualUseTLS(host: "coreblow.local", useTLS: false) == true)
