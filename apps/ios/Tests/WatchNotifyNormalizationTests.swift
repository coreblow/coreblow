...
struct WatchNotifyNormalizationTests {
    @Test @MainActor func trimmedOrNilReturnsNilForBlanks() {
        #expect(NodeAppModel.trimmedOrNil(nil) == nil)
        #expect(NodeAppModel.trimmedOrNil("") == nil)
        #expect(NodeAppModel.trimmedOrNil("   ") == nil)
    }

    @Test @MainActor func trimmedOrNilReturnsTrimmedValue() {
        #expect(NodeAppModel.trimmedOrNil("  hello  ") == "hello")
    }

    @Test @MainActor func normalizedWatchRiskFromPriority() {
    ...
    @Test @MainActor func normalizedWatchPriorityFromRisk() {
    ...
    @Test @MainActor func normalizeWatchActionsLimitsToFour() {
        let actions = (0..<10).map { CoreBlowWatchAction(id: "a\($0)", label: "L\($0)") }
