...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.coreblow.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to CoreBlow", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to CoreBlow gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to CoreBlow.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "CoreBlowShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "CoreBlow is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "CoreBlow Share",
