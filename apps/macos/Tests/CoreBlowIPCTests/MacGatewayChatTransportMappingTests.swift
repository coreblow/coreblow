...
struct MacGatewayChatTransportMappingTests {
    @Test func `snapshot maps to health`() {
        let snapshot = Snapshot(
            presence: [],
            health: CoreBlowProtocol.AnyCodable(["ok": CoreBlowProtocol.AnyCodable(false)]),
    ...
    @Test func `health event maps to health`() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: CoreBlowProtocol.AnyCodable(["ok": CoreBlowProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func `chat event maps to chat`() {
        let payload = CoreBlowProtocol.AnyCodable([
            "runId": CoreBlowProtocol.AnyCodable("run-1"),
            "sessionKey": CoreBlowProtocol.AnyCodable("main"),
            "state": CoreBlowProtocol.AnyCodable("final"),
        ])
    ...
    @Test func `unknown event maps to nil`() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: CoreBlowProtocol.AnyCodable(["a": CoreBlowProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
