...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: CoreBlowProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: CoreBlowProtocol.AnyCodable] = [
            "presence": .init([CoreBlowProtocol.AnyCodable(entry)]),
        ]
        let payload = CoreBlowProtocol.AnyCodable(payloadMap)
