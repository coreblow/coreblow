...
struct VoiceWakeRuntimeTests {
    ...
    @Test func `has content after trigger false when only trigger`() {
        let triggers = ["coreblow"]
        let text = "hey coreblow"
    ...
    @Test func `gate requires gap between trigger and command`() {
        let transcript = "hey coreblow do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["coreblow"], minPostTriggerGap: 0.3)
    ...
    @Test func `gate command text handles foreign string ranges`() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "coreblow", start: 0.2, duration: 0.1, range: transcript.range(of: "coreblow")),
