...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["COREBLOW_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/coreblow")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("coreblow-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("coreblow-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/coreblow/ (rolling or stdout), or nil if none exist.
