...
enum ExecAllowlistPatternValidationReason: String, Codable, Equatable {
    ...
    var message: String {
        switch self {
        case .empty: "Pattern cannot be empty."
        case .missingPathComponent: "Path patterns only. Include '/', '~', or '\\\\'."
        }
    }
}
...
struct ExecAllowlistEntry: Codable, Hashable, Identifiable {
    ...
    init(
        id: UUID = UUID(), pattern: String,
        lastUsedAt: Double? = nil, lastUsedCommand: String? = nil,
    ...
    private enum CodingKeys: String, CodingKey {
        case id, pattern, lastUsedAt, lastUsedCommand, lastResolvedPath
    }
    ...
    func encode(to encoder: Encoder) throws {
        ...
        try container.encodeIfPresent(lastUsedCommand, forKey: .lastUsedCommand)
        try container.encodeIfPresent(lastResolvedPath, forKey: .lastResolvedPath)
    }
}
...
/// Local snapshot type (distinct from protocol's AnyCodable-based ExecApprovalsSnapshot).
struct ExecApprovalsLocalSnapshot {
    var path: String
    var exists: Bool
    var hash: String
...
enum ExecApprovalsStore {
    private static let logger = Logger(subsystem: "ai.coreblow", category: "exec-approvals")
    ...
    static func fileURL() -> URL {
        CoreBlowPaths.stateDirURL.appendingPathComponent("exec-approvals.json")
    }

    static func socketPath() -> String {
        CoreBlowPaths.stateDirURL.appendingPathComponent("exec-approvals.sock").path
    }
    ...
    static func loadFile() -> ExecApprovalsFile {
        ...
        do {
            ...
            return decoded.version == 1 ? decoded
    ...
    static func resolve(agentId: String?) -> ExecApprovalsResolved {
        let file = loadFile()
        return resolveFromFile(file, agentId: agentId)
    }
    ...
    private static func resolveFromFile(_ file: ExecApprovalsFile, agentId: String?) -> ExecApprovalsResolved {
        ...
        let allowlist = normalizeAllowlistEntries(
        ...
        return ExecApprovalsResolved(
            url: fileURL(), socketPath: socketPath, token: token,
            defaults: resolvedDefaults, agent: resolvedAgent,
            allowlist: allowlist, file: file)
    }

    // MARK: - Normalize Incoming
    ...
    static func normalizeIncoming(_ file: ExecApprovalsFile) -> ExecApprovalsFile {
        ...
        // Migrate "default" -> defaultAgentId
        if let legacyDefault = agents["default"] {
            if let main = agents[defaultAgentId] {
                agents[defaultAgentId] = mergeAgents(current: main, legacy: legacyDefault)
            } else {
        ...
        if !agents.isEmpty {
            ...
            for (key, var agent) in agents {
                if let allowlist = agent.allowlist {
                    let normalized = normalizeAllowlistEntries(allowlist, dropInvalid: false).entries
    ...
    static func readSnapshot() -> ExecApprovalsLocalSnapshot {
        let url = fileURL()
        guard FileManager.default.fileExists(atPath: url.path) else {
            return ExecApprovalsLocalSnapshot(
                path: url.path, exists: false,
                hash: hashRaw(nil),
        ...
        return ExecApprovalsLocalSnapshot(path: url.path, exists: true, hash: hashRaw(raw), file: decoded)
    }
    ...
    static func redactForSnapshot(_ file: ExecApprovalsFile) -> ExecApprovalsFile {
        ...
        if socketPath.isEmpty {
            return ExecApprovalsFile(
                version: file.version, socket: nil,
                defaults: file.defaults, agents: file.agents)
        }
        return ExecApprovalsFile(
    ...
    static func ensureFile() -> ExecApprovalsFile {
        ...
        let loaded = loadFile()
        let loadedHash = hashFile(loaded)
        ...
        if path.isEmpty { file.socket?.path = Self.socketPath() }
        let token = file.socket?.token?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        if token.isEmpty { file.socket?.token = generateToken() }
        if file.agents == nil { file.agents = [:] }
        if !existed || loadedHash != hashFile(file) { saveFile(file) }
        return file
    }
    ...
    static func saveDefaults(_ defaults: ExecApprovalsDefaults) {
        updateFile { file in file.defaults = defaults }
    }
    ...
    static func addAllowlistEntry(agentId: String?, pattern: String) -> ExecAllowlistPatternValidationReason? {
        ...
        case let .valid(validPattern): normalizedPattern = validPattern
    ...
    static func recordAllowlistUse(
        ...
        command: String, resolvedPath: String?)
    {
        updateFile { file in
            let key = agentKey(agentId)
            var agents = file.agents ?? [:]
            ...
            let allowlist = (entry.allowlist ?? []).map { item -> ExecAllowlistEntry in
                ...
                return ExecAllowlistEntry(
                    id: item.id, pattern: item.pattern,
                    lastUsedAt: Date().timeIntervalSince1970 * 1000,
                    lastUsedCommand: command, lastResolvedPath: resolvedPath)
            }
            entry.allowlist = allowlist
            agents[key] = entry
            file.agents = agents
    ...
    static func updateAllowlist(agentId: String?, allowlist: [ExecAllowlistEntry]) -> [ExecAllowlistRejectedEntry] {
        ...
        updateFile { file in
            ...
            var agents = file.agents ?? [:]
            var entry = agents[key] ?? ExecApprovalsAgent()
            let normalized = normalizeAllowlistEntries(allowlist, dropInvalid: true)
            rejected = normalized.rejected
            entry.allowlist = normalized.entries
            agents[key] = entry
            file.agents = agents
        }
        return rejected
    ...
    private static func ensureSecureStateDirectory() {
        let url = CoreBlowPaths.stateDirURL
        do {
            ...
            logger.warning("exec approvals state dir permission hardening failed: \(error.localizedDescription, privacy: .public)")
        }
    }
    ...
    private static func normalizedPattern(_ pattern: String?) -> String? {
        switch ExecApprovalHelpers.validateAllowlistPattern(pattern) {
        case let .valid(normalized): return normalized.lowercased()
        case .invalid(.empty): return nil
        case .invalid:
    ...
    private static func migrateLegacyPattern(_ entry: ExecAllowlistEntry) -> ExecAllowlistEntry {
        ...
        case let .valid(pattern):
            return ExecAllowlistEntry(
                id: entry.id, pattern: pattern,
                lastUsedAt: entry.lastUsedAt, lastUsedCommand: entry.lastUsedCommand,
        ...
        case .invalid:
            ...
            case let .valid(migratedPattern):
                return ExecAllowlistEntry(
                    id: entry.id, pattern: migratedPattern,
                    lastUsedAt: entry.lastUsedAt, lastUsedCommand: entry.lastUsedCommand,
                    lastResolvedPath: normalizedResolved)
            case .invalid:
                return ExecAllowlistEntry(
                    id: entry.id, pattern: trimmedPattern,
                    lastUsedAt: entry.lastUsedAt, lastUsedCommand: entry.lastUsedCommand,
                    lastResolvedPath: normalizedResolved)
            }
        }
    ...
    private static func normalizeAllowlistEntries(
        ...
        dropInvalid: Bool
    ) -> (entries: [ExecAllowlistEntry], rejected: [ExecAllowlistRejectedEntry]) {
        var normalized: [ExecAllowlistEntry] = []
        normalized.reserveCapacity(entries.count)
        var rejected: [ExecAllowlistRejectedEntry] = []

        for entry in entries {
            let migrated = migrateLegacyPattern(entry)
            ...
            switch ExecApprovalHelpers.validateAllowlistPattern(trimmedPattern) {
            case let .valid(pattern):
                normalized.append(ExecAllowlistEntry(
                    id: migrated.id, pattern: pattern,
                    lastUsedAt: migrated.lastUsedAt, lastUsedCommand: migrated.lastUsedCommand,
                    lastResolvedPath: normalizedResolvedPath))
            case let .invalid(reason):
                if dropInvalid {
                    rejected.append(ExecAllowlistRejectedEntry(
                        id: migrated.id, pattern: trimmedPattern, reason: reason))
                } else if reason != .empty {
                    normalized.append(ExecAllowlistEntry(
                        id: migrated.id, pattern: trimmedPattern,
                        lastUsedAt: migrated.lastUsedAt, lastUsedCommand: migrated.lastUsedCommand,
                        lastResolvedPath: normalizedResolvedPath))
                }
            }
    ...
    private static func mergeAgents(
        ...
        let currentAllowlist = normalizeAllowlistEntries(current.allowlist ?? [], dropInvalid: false).entries
        let legacyAllowlist = normalizeAllowlistEntries(legacy.allowlist ?? [], dropInvalid: false).entries
        var seen = Set<String>()
        var allowlist: [ExecAllowlistEntry] = []
        func append(_ entry: ExecAllowlistEntry) {
            guard let key = normalizedPattern(entry.pattern), !seen.contains(key) else { return }
            seen.insert(key)
            allowlist.append(entry)
        }
        for entry in currentAllowlist { append(entry) }
...
enum ExecApprovalHelpers {
    static func validateAllowlistPattern(_ pattern: String?) -> ExecAllowlistPatternValidation {
        ...
        guard containsPathComponent(trimmed) else { return .invalid(.missingPathComponent) }
        return .valid(trimmed)
    }

    static func isPathPattern(_ pattern: String?) -> Bool {
        switch validateAllowlistPattern(pattern) {
        case .valid: true
        case .invalid: false
        }
    }
    ...
    static func requiresAsk(
        ...
        allowlistMatch: ExecAllowlistEntry?, skillAllow: Bool) -> Bool
    {
    ...
    static func allowlistPattern(command: [String], resolvedPath: String?, rawExecutable: String?) -> String? {
        let pattern = resolvedPath ?? rawExecutable ?? command.first ?? ""
...
struct ExecEventPayload: Codable {
    ...
    static func truncateOutput(_ raw: String, maxChars: Int = 20000) -> String? {
        ...
        return "... (truncated) \(trimmed.suffix(maxChars))"
    }
}

// MARK: - Skill Bins Cache
...
actor SkillBinsCache {
    ...
    func currentBins(force: Bool = false) async -> Set<String> {
        if force || isStale() { await refresh() }
        return bins
    }
