...
private let webChatSwiftLogger = Logger(subsystem: "ai.coreblow", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "coreblow.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: CoreBlowChatTransport {
        ...
        case let .event(evt):
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    CoreBlowAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<CoreBlowChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any CoreBlowChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = CoreBlowChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: CoreBlowChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<CoreBlowChatView>) -> NSViewController
    {
