...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.coreblow/coreblow.json. Prefer COREBLOW_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.coreblow/coreblow.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.coreblow/coreblow.json. Restart the gateway to apply."
        }
