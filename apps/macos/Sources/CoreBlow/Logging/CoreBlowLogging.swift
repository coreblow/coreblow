...
enum CoreBlowLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = CoreBlowOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = CoreBlowFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        CoreBlowLogging.bootstrapIfNeeded()
        let label = CoreBlowLogging.makeLabel(subsystem: subsystem, category: category)
...
struct CoreBlowOSLogHandler: AppLogLevelBackedHandler {
...
struct CoreBlowFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = CoreBlowLogging.parseLabel(self.label)
