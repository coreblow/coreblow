...
private let gatewayConnectionLogger = Logger(subsystem: "ai.coreblow", category: "gateway.connection")
...
actor GatewayConnection {
    ...
    private func sessionDefaultString(_ defaults: [String: CoreBlowProtocol.AnyCodable]?, key: String) -> String {
...
extension GatewayConnection {
    ...
    func sessionsPreview(
        ...
        limit: Int? = nil,
        maxChars: Int? = nil,
        timeoutMs: Int? = nil) async throws -> CoreBlowSessionsPreviewPayload
    {
        let resolvedKeys = keys
            .map { self.canonicalizeSessionKey($0) }
            .filter { !$0.isEmpty }
        if resolvedKeys.isEmpty {
            return CoreBlowSessionsPreviewPayload(ts: 0, previews: [])
        }
    ...
    func chatHistory(
        sessionKey: String,
        limit: Int? = nil,
        timeoutMs: Int? = nil) async throws -> CoreBlowChatHistoryPayload
    {
    ...
    func chatSend(
        ...
        attachments: [CoreBlowChatAttachmentPayload],
        timeoutMs: Int = 30000) async throws -> CoreBlowChatSendResponse
    {
        let resolvedKey = self.canonicalizeSessionKey(sessionKey)
        var params: [String: AnyCodable] = [
