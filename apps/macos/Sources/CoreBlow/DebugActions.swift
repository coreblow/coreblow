...
enum DebugActions {
    private static let verboseDefaultsKey = "coreblow.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "coreblow.onboardingSeen"
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "CoreBlow", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = CoreBlowPaths.configURL
        guard
