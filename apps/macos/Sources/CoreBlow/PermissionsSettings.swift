    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 14) {
                ...
                Text("Allow these so CoreBlow can notify and capture when needed.")
                    .padding(.top, 4)
...
private struct LocationAccessSettings: View {
    @AppStorage(locationModeKey) private var locationModeRaw: String = CoreBlowLocationMode.off.rawValue
    ...
    var body: some View {
        ...
        .onChange(of: self.locationModeRaw) { _, newValue in
            ...
            guard let mode = CoreBlowLocationMode(rawValue: newValue) else { return }
            Task {
    ...
    private var locationMode: CoreBlowLocationMode {
        CoreBlowLocationMode(rawValue: self.locationModeRaw) ?? .off
    }

    private func requestLocationAuthorization(mode: CoreBlowLocationMode) async -> Bool {
