...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.coreblow.mac`) even if the shipped
    ...
    {
        if environment["COREBLOW_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "coreblow.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "coreblow.nixMode") { return true }
