...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: CoreBlowProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.coreblow", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: CoreBlowKit.AnyCodable]()) {
                $0[$1.key] = CoreBlowKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: CoreBlowProtocol.AnyCodable?) -> [String: CoreBlowProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: CoreBlowProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: CoreBlowKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: CoreBlowProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("coreblow.control.heartbeat")
    static let controlAgentEvent = Notification.Name("coreblow.control.agent")
}
