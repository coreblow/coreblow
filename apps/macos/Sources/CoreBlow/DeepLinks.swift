...
private let deepLinkLogger = Logger(subsystem: "ai.coreblow", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "CoreBlow is paused", message: "Unpause CoreBlow to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run CoreBlow agent?", message: body) else { return }
        }
