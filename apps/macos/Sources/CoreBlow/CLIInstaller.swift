...
enum CLIInstaller {
        ...
        fileManager: FileManager) -> String?
    {
        for basePath in searchPaths {
            let candidate = URL(fileURLWithPath: basePath).appendingPathComponent("coreblow").path
    ...
    static func install(statusHandler: @escaping @MainActor @Sendable (String) async -> Void) async {
        ...
        await statusHandler("Installing coreblow CLI…")
        ...
        if response.success {
            ...
            let summary = installedVersion.map { "Installed coreblow \($0)." } ?? "Installed coreblow."
    ...
    private static func installScriptCommand(version: String, prefix: String) -> [String] {
        ...
        let script = """
        curl -fsSL https://coreblow.bot/install-cli.sh | \
