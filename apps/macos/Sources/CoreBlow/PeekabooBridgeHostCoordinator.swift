#if canImport(PeekabooAutomationKit)
import Foundation
import CoreBlowKit
import OSLog
import os
...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.coreblow", category: "PeekabooBridge")
    ...
    private static var coreblowSocketPath: String {
        ...
        return Self.makeSocketPath(for: "CoreBlow", in: base)
    }
    ...
    private func startIfNeeded() async {
        ...
        let services = CoreBlowPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.coreblowSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.coreblowSocketPath, privacy: .public)")
    }
        ...
        do {
            ...
            if linkValues?.isSymbolicLink == true {
                ...
                let destinationURL = URL(fileURLWithPath: destination, relativeTo: linkURL.deletingLastPathComponent())
                    .standardizedFileURL
                if destinationURL.path == URL(fileURLWithPath: Self.coreblowSocketPath).standardizedFileURL.path {
                    return
                }
            ...
            try fileManager.createSymbolicLink(atPath: legacyPath, withDestinationPath: Self.coreblowSocketPath)
        } catch {
...
@MainActor
private final class CoreBlowPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.coreblow.peekaboo")
