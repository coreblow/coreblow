...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.coreblow", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> CoreBlowSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: CoreBlowSessionPreviewEntry,
    ...
    private static func cache(payload: CoreBlowSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: CoreBlowChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [CoreBlowKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [CoreBlowKit.AnyCodable]) -> [CoreBlowChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(CoreBlowChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: CoreBlowChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: CoreBlowChatMessage) -> Bool {
    ...
    private static func toolNames(for message: CoreBlowChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: CoreBlowChatMessage) -> String? {
