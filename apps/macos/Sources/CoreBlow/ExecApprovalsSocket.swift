...
enum ExecApprovalsSocketClient {
        ...
        timeoutMs: Int = 15000) async -> ExecApprovalDecision?
    {
        ...
        do {
            return try await withThrowingTaskGroup(of: ExecApprovalDecision?.self) { group in
                group.addTask {
                    try await Task.detached {
                        try requestDecisionSync(
                ...
                group.addTask {
                    ...
                    throw TimeoutError(message: "exec approvals socket timeout")
                }
        ...
        request: ExecApprovalPromptRequest) throws -> ExecApprovalDecision?
    {
        ...
        guard fd >= 0 else {
            throw NSError(domain: "ExecApprovals", code: 1,
                          userInfo: [NSLocalizedDescriptionKey: "socket create failed"])
        }
        ...
        if socketPath.utf8.count >= maxLen {
            throw NSError(domain: "ExecApprovals", code: 2,
        ...
        if result != 0 {
            throw NSError(domain: "ExecApprovals", code: 3,
                          userInfo: [NSLocalizedDescriptionKey: "socket connect failed"])
        }

        let handle = FileHandle(fileDescriptor: fd, closeOnDealloc: true)

        let message = ExecApprovalSocketRequest(
            type: "request", token: token,
...
final class ExecApprovalsPromptServer {
    ...
    func start() {
        ...
        let server = ExecApprovalsSocketServer(
            ...
            onPrompt: { request in await ExecApprovalsPromptPresenter.prompt(request) },
            onExec: { request in await ExecHostExecutor.handle(request) })
        server.start()
        self.server = server
    }
...
enum ExecApprovalsPromptPresenter {
    ...
    static func prompt(_ request: ExecApprovalPromptRequest) -> ExecApprovalDecision {
        ...
        if alert.buttons.indices.contains(2) { alert.buttons[2].hasDestructiveAction = true }
    ...
    private static func buildAccessoryView(_ request: ExecApprovalPromptRequest) -> NSView {
        ...
        commandText.isEditable = false; commandText.isSelectable = true
        commandText.drawsBackground = true; commandText.backgroundColor = NSColor.textBackgroundColor
        ...
        commandText.isHorizontallyResizable = false; commandText.isVerticallyResizable = true

        let commandScroll = NSScrollView()
        commandScroll.borderType = .lineBorder; commandScroll.hasVerticalScroller = true
        commandScroll.hasHorizontalScroller = false; commandScroll.autohidesScrollers = true
        ...
        contextStack.orientation = .vertical; contextStack.spacing = 4; contextStack.alignment = .leading
        if let cwd = request.cwd?.trimmingCharacters(in: .whitespacesAndNewlines), !cwd.isEmpty {
            addDetailRow(title: "Working directory", value: cwd, to: contextStack)
        }
        if let agent = request.agentId?.trimmingCharacters(in: .whitespacesAndNewlines), !agent.isEmpty {
            addDetailRow(title: "Agent", value: agent, to: contextStack)
        }
        if let path = request.resolvedPath?.trimmingCharacters(in: .whitespacesAndNewlines), !path.isEmpty {
            addDetailRow(title: "Executable", value: path, to: contextStack)
        }
        if let host = request.host?.trimmingCharacters(in: .whitespacesAndNewlines), !host.isEmpty {
            addDetailRow(title: "Host", value: host, to: contextStack)
        }
    ...
    private static func addDetailRow(title: String, value: String, to stack: NSStackView) {
        let row = NSStackView(); row.orientation = .horizontal; row.spacing = 6; row.alignment = .firstBaseline
        ...
        row.addArrangedSubview(titleLabel); row.addArrangedSubview(valueLabel)
        stack.addArrangedSubview(row)
    }
}

// ExecApprovalCommandDisplaySanitizer is defined in ExecApprovalCommandDisplaySanitizer.swift

// ExecHostValidatedRequest, ExecHostRequestEvaluationResult, and ExecHostRequestEvaluator
// are defined in ExecHostRequestEvaluator.swift


// MARK: - Exec Host Executor
...
private final class ExecApprovalsSocketServer: @unchecked Sendable {
    private let logger = Logger(subsystem: "ai.coreblow", category: "exec-approvals.socket")
    ...
    private func handleClient(fd: Int32) async {
        ...
        do {
            ...
            if type == "request" {
                ...
                guard timingSafeHexStringEquals(request.token, self.token) else {
    ...
    private func hmacHex(nonce: String, ts: Int, requestJson: String) -> String {
        let key = SymmetricKey(data: Data(token.utf8)); let message = "\(nonce):\(ts):\(requestJson)"
