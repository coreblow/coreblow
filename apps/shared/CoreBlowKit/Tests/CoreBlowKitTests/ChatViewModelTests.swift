...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> CoreBlowChatHistoryPayload
{
    CoreBlowChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> CoreBlowChatSessionEntry {
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> CoreBlowChatSessionEntry
{
    CoreBlowChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> CoreBlowChatModelChoice {
    CoreBlowChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [CoreBlowChatHistoryPayload],
    sessionsResponses: [CoreBlowChatSessionsListResponse] = [],
    modelResponses: [[CoreBlowChatModelChoice]] = [],
    ...
    -> (TestChatTransport, CoreBlowChatViewModel)
{
...
private func sendUserMessage(_ vm: CoreBlowChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, CoreBlowChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [CoreBlowChatHistoryPayload]
    private let sessionsResponses: [CoreBlowChatSessionsListResponse]
    private let modelResponses: [[CoreBlowChatModelChoice]]
    ...
    private let stream: AsyncStream<CoreBlowChatTransportEvent>
    private let continuation: AsyncStream<CoreBlowChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<CoreBlowChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> CoreBlowChatHistoryPayload {
        ...
        return self.historyResponses.last ?? CoreBlowChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
        ...
        attachments _: [CoreBlowChatAttachmentPayload]) async throws -> CoreBlowChatSendResponse
    {
        ...
        return CoreBlowChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> CoreBlowChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? CoreBlowChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: CoreBlowChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = CoreBlowChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = CoreBlowChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: CoreBlowChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = CoreBlowChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: CoreBlowChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(CoreBlowChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == CoreBlowChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = CoreBlowChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: CoreBlowChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = CoreBlowChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = CoreBlowChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = CoreBlowChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { CoreBlowChatViewModel(sessionKey: "main", transport: transport) }
