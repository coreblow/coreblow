        ...
        return try await withCheckedThrowingContinuation { cont in
            ...
            store.fetchReminders(matching: pred) { reminders in
                let items: [[String: Any]] = (reminders ?? [])
                    .filter { completed == nil || $0.isCompleted == completed }
                    .map { ["title": $0.title ?? "", "completed": $0.isCompleted] as [String: Any] }
                // Use nonisolated(unsafe) to bridge the Sendable gap for EKReminder callbacks
                nonisolated(unsafe) let sendableItems = items
                cont.resume(returning: sendableItems)
            }
        }
    }
}
