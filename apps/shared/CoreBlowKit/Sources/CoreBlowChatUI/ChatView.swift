...
@MainActor
public struct CoreBlowChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: CoreBlowChatViewModel
    ...
    public init(
        viewModel: CoreBlowChatViewModel,
    ...
    private var visibleMessages: [CoreBlowChatMessage] {
        let base: [CoreBlowChatMessage]
    ...
    private func mergeToolResults(in messages: [CoreBlowChatMessage]) -> [CoreBlowChatMessage] {
        ...
        for message in messages {
            ...
            let merged = CoreBlowChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: CoreBlowChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: CoreBlowChatMessage) -> Bool {
    ...
    private func primaryText(in message: CoreBlowChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: CoreBlowChatMessage) -> Bool {
    ...
    private func toolCalls(in message: CoreBlowChatMessage) -> [CoreBlowChatMessageContent] {
    ...
    private func inlineToolResults(in message: CoreBlowChatMessage) -> [CoreBlowChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: CoreBlowChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: CoreBlowChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(CoreBlowChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
