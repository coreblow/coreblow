...
struct ChatMessageBubble: View {
    let message: CoreBlowChatMessage
    let style: CoreBlowChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? CoreBlowChatTheme.userText : CoreBlowChatTheme.assistantText
    ...
    private var inlineAttachments: [CoreBlowChatMessageContent] {
    ...
    private var toolCalls: [CoreBlowChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [CoreBlowChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? CoreBlowChatTheme.userBubble
        }
        if self.style == .onboarding {
            return CoreBlowChatTheme.onboardingAssistantBubble
        }
        return CoreBlowChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return CoreBlowChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: CoreBlowChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? CoreBlowChatTheme.userText : CoreBlowChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: CoreBlowChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(CoreBlowChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [CoreBlowChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: CoreBlowChatTheme.assistantText)
            }
        }
    }
}
