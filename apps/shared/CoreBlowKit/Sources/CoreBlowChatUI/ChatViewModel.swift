...
private let chatUILogger = Logger(subsystem: "ai.coreblow", category: "CoreBlowChatUI")

@MainActor
@Observable
public final class CoreBlowChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [CoreBlowChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [CoreBlowChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [CoreBlowPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [CoreBlowChatPendingToolCall] = []
    public private(set) var sessions: [CoreBlowChatSessionEntry] = []
    private let transport: any CoreBlowChatTransport
    private var sessionDefaults: CoreBlowChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: CoreBlowChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any CoreBlowChatTransport,
    ...
    public var sessionChoices: [CoreBlowChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [CoreBlowChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: CoreBlowPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [CoreBlowChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: CoreBlowChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: CoreBlowChatMessage) -> CoreBlowChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> CoreBlowChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return CoreBlowChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: CoreBlowChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: CoreBlowChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: CoreBlowChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private static func reconcileMessageIDs(
        previous: [CoreBlowChatMessage],
        incoming: [CoreBlowChatMessage]) -> [CoreBlowChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [CoreBlowChatMessage]) -> [CoreBlowChatMessage] {
        var result: [CoreBlowChatMessage] = []
    ...
    private static func dedupeKey(for message: CoreBlowChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [CoreBlowChatMessageContent] = [
            CoreBlowChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> CoreBlowChatAttachmentPayload in
            CoreBlowChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> CoreBlowChatSessionEntry {
        CoreBlowChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = CoreBlowChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: CoreBlowChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: CoreBlowChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: CoreBlowAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = CoreBlowChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> CoreBlowPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
