...
If still empty, check `/data/.clawdbot/coreblow.json`:

```
cat /data/.clawdbot/coreblow.json | jq -r '.skills.entries["gh-issues"].apiKey // empty'
```
...
GH_TOKEN=$(cat ~/.coreblow/coreblow.json 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty') || GH_TOKEN=$(cat /data/.clawdbot/coreblow.json 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty')
...
export GH_TOKEN=$(node -e "const fs=require('fs'); const c=JSON.parse(fs.readFileSync('/data/.clawdbot/coreblow.json','utf8')); console.log(c.skills?.entries?.['gh-issues']?.apiKey || '')")
