---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for CoreBlow deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, CoreBlow cron scheduling for periodic checks, or version status checks on a machine running CoreBlow (laptop, workstation, Pi, VPS).
---

# CoreBlow Host Hardening

## Overview

Assess and harden the host running CoreBlow, then align it to a user-defined risk tolerance without breaking access. Use CoreBlow security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim CoreBlow changes the host firewall, SSH, or OS updates; it does not.
...
5. CoreBlow gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if CoreBlow can access sensitive data.
...
### 2) Run CoreBlow security audits (read-only)

As part of the default read-only checks, run `coreblow security audit --deep`. Only offer alternatives if the user requests them:

1. `coreblow security audit` (faster, non-probing)
2. `coreblow security audit --json` (structured output)

Offer to apply CoreBlow safe defaults (numbered):

1. `coreblow security audit --fix`

Be explicit that `--fix` only tightens CoreBlow defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check CoreBlow version/update status (read-only)

As part of the default read-only checks, run `coreblow update status`.
...
- Credential hygiene notes (location of CoreBlow creds, prefer disk encryption)
...
- CoreBlow security audit (re-run)
...
After CoreBlow install or first hardening pass, run at least one baseline audit and version check:

- `coreblow security audit`
- `coreblow security audit --deep`
- `coreblow update status`

Ongoing monitoring is recommended. Use the CoreBlow cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `coreblow cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `coreblow update status`
...
Before creating, `coreblow cron list` and match on exact `name`. If found, `coreblow cron edit <id> ...`.
If not found, `coreblow cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `coreblow update status` (preferred for source checkouts and channels)
2. `npm view coreblow version` (published npm version)

## CoreBlow command accuracy

Use only supported commands and flags:

- `coreblow security audit [--deep] [--fix] [--json]`
- `coreblow status` / `coreblow status --deep`
- `coreblow health --json`
- `coreblow update status`
- `coreblow cron add|list|runs|run`

Do not invent CLI flags or imply CoreBlow enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by CoreBlow compaction:
