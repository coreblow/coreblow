---
name: node-connect
description: Diagnose CoreBlow node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify CoreBlow is advertising that route, then fix pairing/auth.
...
- whether `coreblow devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `coreblow qr --json`. It uses the same setup-code payload Android scans.

```bash
coreblow config get gateway.mode
coreblow config get gateway.bind
coreblow config get gateway.tailscale.mode
coreblow config get gateway.remote.url
coreblow config get gateway.auth.mode
coreblow config get gateway.auth.allowTailscale
coreblow config get plugins.entries.device-pair.config.publicUrl
coreblow qr --json
coreblow devices list
coreblow nodes status
```

If this CoreBlow instance is pointed at a remote gateway, also run:

```bash
coreblow qr --remote --json
```
...
`coreblow qr --json` success means:
...
If `coreblow qr --json` says `Gateway is only bound to loopback`:
...
```bash
coreblow devices list
coreblow devices approve --latest
```
...
- `coreblow devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run coreblow qr again, rescan, then approve the pending device pairing.`
