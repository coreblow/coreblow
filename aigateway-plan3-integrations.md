...
**Berdasarkan Deep Research arsitektur OpenClaw (AgentSkills & Channel API)**
...
### 1.1 Perbedaan Mendasar (Berdasarkan OpenClaw Docs)
...
Dari OpenClaw docs, secret (seperti API Key 1Password, Notion) JANGAN dimasukkan ke prompt text.
...
Menyontek konsep `clawhub` OpenClaw, AIGateway memiliki `skillhub` di dalam binary CLI utamanya untuk mendownload SKILL.md dari repository kita.
...
| :--- | :---: | :--- |
| **Akurasi Konsep Skills vs Channels** | **10/10** | Memisahkan `AgentSkills` (Frontmatter markdown, per-workspace) dari NPM Plugins. Sangat selaras dengan docs OpenClaw. |
