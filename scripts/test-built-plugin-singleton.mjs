...
const warningFilterKey = Symbol.for("coreblow.warning-filter");
...
const { clearPluginCommands, getPluginCommandSpecs, loadCoreBlowPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadCoreBlowPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "coreblow-build-smoke-"));
...
fs.writeFileSync(
  ...
  JSON.stringify(
    {
      name: "@coreblow/build-smoke-plugin",
      type: "module",
      coreblow: {
        extensions: ["./index.js"],
      },
...
fs.writeFileSync(
  path.join(distPluginDir, "coreblow.plugin.json"),
  JSON.stringify(
    {
      id: pluginId,
      configSchema: {
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'coreblow/plugin-sdk';",
...
const registry = loadCoreBlowPlugins({
  ...
  env: {
    ...process.env,
    COREBLOW_BUNDLED_PLUGINS_DIR: path.join(tempRoot, "dist-runtime", "extensions"),
