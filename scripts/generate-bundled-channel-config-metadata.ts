...
function resolvePackageChannelMeta(source: BundledPluginSource) {
  const coreblowMeta =
    ...
    "coreblow" in source.packageJson
      ? (source.packageJson.coreblow as Record<string, unknown> | undefined)
      : undefined;
  const channelMeta =
    coreblowMeta &&
    typeof coreblowMeta.channel === "object" &&
    coreblowMeta.channel &&
    !Array.isArray(coreblowMeta.channel)
      ? (coreblowMeta.channel as Record<string, unknown>)
      : undefined;
  return channelMeta;
}
