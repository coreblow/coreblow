...
function shouldStageRuntimeDeps(packageJson) {
  return packageJson.coreblow?.bundle?.stageRuntimeDependencies === true;
}
...
function sanitizeBundledManifestForRuntimeInstall(pluginDir) {
  ...
  if (packageJson.peerDependencies?.coreblow) {
    const nextPeerDependencies = { ...packageJson.peerDependencies };
    delete nextPeerDependencies.coreblow;
  ...
  if (packageJson.peerDependenciesMeta?.coreblow) {
    const nextPeerDependenciesMeta = { ...packageJson.peerDependenciesMeta };
    delete nextPeerDependenciesMeta.coreblow;
  ...
  if (packageJson.devDependencies?.coreblow) {
    const nextDevDependencies = { ...packageJson.devDependencies };
    delete nextDevDependencies.coreblow;
...
function resolveRuntimeDepsStampPath(pluginDir) {
  return path.join(pluginDir, ".coreblow-runtime-deps-stamp.json");
}
  ...
  if (platform === "win32") {
    ...
    throw new Error(
      `failed to resolve a toolchain-local npm next to ${execPath}. ` +
        ...
        "CoreBlow refuses to shell out to bare npm on Windows; install a Node.js toolchain that bundles npm or run with a matching Node installation.",
    );
  }
