...
const tmpHome = mkdtempSync(path.join(os.tmpdir(), "coreblow-startup-memory-"));
...
const MAX_RSS_MARKER = "__COREBLOW_MAX_RSS_KB__=";
...
const cases = [
  {
    id: "help",
    label: "--help",
    args: ["coreblow.mjs", "--help"],
    limitMb: Number(process.env.COREBLOW_STARTUP_MEMORY_HELP_MB ?? DEFAULT_LIMITS_MB.help),
  },
  {
    id: "statusJson",
    label: "status --json",
    args: ["coreblow.mjs", "status", "--json"],
    limitMb: Number(
      process.env.COREBLOW_STARTUP_MEMORY_STATUS_JSON_MB ?? DEFAULT_LIMITS_MB.statusJson,
    ),
  },
  {
    id: "gatewayStatus",
    label: "gateway status",
    args: ["coreblow.mjs", "gateway", "status"],
    limitMb: Number(
      process.env.COREBLOW_STARTUP_MEMORY_GATEWAY_STATUS_MB ?? DEFAULT_LIMITS_MB.gatewayStatus,
    ),
  },
];
...
function formatFixGuidance(testCase, details) {
  ...
  const guidance = [
    ...
    "LLM prompt:",
    `"CoreBlow startup-memory CI failed for '${testCase.label}'. Analyze this failure, identify the first runtime/import side effect that makes startup heavier or broken, and propose the smallest safe patch. Failure output:\n${details}"`,
  ];
