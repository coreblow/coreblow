...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.coreblow) ? packageJson.coreblow : null;
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.coreblow?.channel?.id) || left.name;
    const rightId = trimString(right.coreblow?.channel?.id) || right.name;
