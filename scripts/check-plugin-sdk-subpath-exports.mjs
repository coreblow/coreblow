...
function parsePluginSdkSubpath(specifier) {
  if (!specifier.startsWith("coreblow/plugin-sdk/")) {
    return null;
  }
  const subpath = specifier.slice("coreblow/plugin-sdk/".length);
  return subpath || null;
}
...
async function main() {
  ...
  if (violations.length === 0) {
    console.log("OK: all referenced coreblow/plugin-sdk/<subpath> imports are exported.");
    return;
  }

  console.error(
    "Rule: every referenced coreblow/plugin-sdk/<subpath> must exist in the public package exports.",
  );
