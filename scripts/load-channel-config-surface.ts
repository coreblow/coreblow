...
function copyModuleImportGraphWithoutNodeModules(params: {
  ...
  const tempParent = path.join(params.repoRoot, ".coreblow-config-doc-cache");
...
export async function loadChannelConfigSurfaceModule(
  ...
  const buildChannelConfigSchemaUrl = pathToFileURL(
  ...
  const loadViaBun = (candidatePath: string) => {
    const script = `
      ...
      const { buildChannelConfigSchema } = await import(${JSON.stringify(buildChannelConfigSchemaUrl)});
      const modulePath = process.env.COREBLOW_CONFIG_SURFACE_MODULE;
      if (!modulePath) {
        throw new Error("missing COREBLOW_CONFIG_SURFACE_MODULE");
      }
    ...
    const result = spawnSync("bun", ["-e", script], {
      ...
      env: {
        ...process.env,
        COREBLOW_CONFIG_SURFACE_MODULE: path.resolve(candidatePath),
      },
    });
    if (result.error) {
  ...
  const loadFromPath = (
    ...
    try {
      ...
      // Bun is the fastest happy path, but source-aware Jiti handles plugin aliases better.
    }
