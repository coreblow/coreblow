...
function collectPluginSourceEntries(packageJson) {
  let packageEntries = Array.isArray(packageJson?.coreblow?.extensions)
    ? packageJson.coreblow.extensions.filter(
        (entry) => typeof entry === "string" && entry.trim().length > 0,
      )
    : [];
  const setupEntry =
    typeof packageJson?.coreblow?.setupEntry === "string" &&
    packageJson.coreblow.setupEntry.trim().length > 0
      ? packageJson.coreblow.setupEntry
      : undefined;
  if (setupEntry) {
...
export function listBundledPluginPackArtifacts(params = {}) {
  ...
  for (const { id, hasPackageJson, sourceEntries } of entries) {
    artifacts.add(`dist/extensions/${id}/coreblow.plugin.json`);
    if (hasPackageJson) {
