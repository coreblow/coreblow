...
export function createPluginSdkScope(_repoRoot: string): TopologyScope {
  const entrypoints = pluginSdkEntrypoints.map((entrypoint: string) => ({
    entrypoint,
    sourcePath: `src/plugin-sdk/${entrypoint}.ts`,
    importSpecifier:
      entrypoint === "index" ? "coreblow/plugin-sdk" : `coreblow/plugin-sdk/${entrypoint}`,
  }));
  return buildScopeFromEntrypoints("plugin-sdk", "CoreBlow plugin-sdk public surface", entrypoints);
}
