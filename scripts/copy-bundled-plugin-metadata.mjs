...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const manifestPath = path.join(pluginDir, "coreblow.plugin.json");
    ...
    const distManifestPath = path.join(distPluginDir, "coreblow.plugin.json");
    ...
    if (packageJson.coreblow && "extensions" in packageJson.coreblow) {
      packageJson.coreblow = {
        ...packageJson.coreblow,
        extensions: rewritePackageExtensions(packageJson.coreblow.extensions),
        ...(typeof packageJson.coreblow.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.coreblow.setupEntry) }
          : {}),
      };
    }
