...
export async function readJsonFile<T>(filePath: string): Promise<T | null> {
  try {
    ...
    logCaughtError('json-files:read', e);
    return null;
  }
}

// ─── Per-Path Lock Registry ─────────────────────────────────────
// Ensures serialized writes to the same file path.
// createAsyncLock() already existed (line 76+) but was never wired.
const pathLocks = new Map<string, ReturnType<typeof createAsyncLock>>();

function getPathLock(filePath: string): ReturnType<typeof createAsyncLock> {
    const key = path.resolve(filePath);
    if (!pathLocks.has(key)) pathLocks.set(key, createAsyncLock());
    return pathLocks.get(key)!;
}
...
export async function writeJsonAtomic(
  ...
  const lock = getPathLock(filePath);
  return lock(async () => {
