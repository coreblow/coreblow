...
type AgentEntry = NonNullable<NonNullable<CoreBlowConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: CoreBlowConfig): AgentEntry[] {
...
export function listAgentIds(cfg: CoreBlowConfig): string[] {
...
export function resolveDefaultAgentId(cfg: CoreBlowConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: CoreBlowConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentModelPrimary(cfg: CoreBlowConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: CoreBlowConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
