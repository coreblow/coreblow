...
vi.mock("coreblow/plugin-sdk/config-runtime", async () => {
  ...
  return {
    ...actual,
    loadConfig: () => config,
    resolveStorePath: vi.fn(() => "/tmp/coreblow-sessions.json"),
...
vi.mock("coreblow/plugin-sdk/conversation-runtime", async () => {
  const actual = await vi.importActual<typeof import("coreblow/plugin-sdk/conversation-runtime")>(
    "coreblow/plugin-sdk/conversation-runtime",
  );
  return {
    ...actual,
...
vi.mock("coreblow/plugin-sdk/security-runtime", async () => {
  const actual = await vi.importActual<typeof import("coreblow/plugin-sdk/security-runtime")>(
    "coreblow/plugin-sdk/security-runtime",
  );
  return {
    ...actual,
...
vi.mock("coreblow/plugin-sdk/channel-runtime", async () => {
  const actual = await vi.importActual<typeof import("coreblow/plugin-sdk/channel-runtime")>(
    "coreblow/plugin-sdk/channel-runtime",
  );
  return {
    ...actual,
...
export function installSignalToolResultTestHooks() {
  beforeEach(async () => {
    const [{ resetInboundDedupe }, { resetSystemEventsForTest }] = await Promise.all([
      import("coreblow/plugin-sdk/reply-runtime"),
      import("coreblow/plugin-sdk/channel-runtime"),
    ]);
    resetInboundDedupe();
    config = {
