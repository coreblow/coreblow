...
describe("canvas host state dir defaults", () => {
  it("uses COREBLOW_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("coreblow-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("CoreBlow Canvas");
      } finally {
        await handler.close();
      }
    });
