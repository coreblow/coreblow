...
function createRegistry(services: CoreBlowPluginService[]) {
...
function expectServiceContext(
  ctx: CoreBlowPluginServiceContext,
...
function expectServiceLogger(ctx: CoreBlowPluginServiceContext) {
...
function expectServiceContexts(
  contexts: CoreBlowPluginServiceContext[],
  config: Parameters<typeof startPluginServices>[0]["config"],
) {
...
async function startTrackingServices(params: {
  services: CoreBlowPluginService[];
  config?: Parameters<typeof startPluginServices>[0]["config"];
  workspaceDir?: string;
}) {
...
describe("startPluginServices", () => {
  ...
  it("starts services and stops them in reverse order", async () => {
    const starts: string[] = [];
    const stops: string[] = [];
    const contexts: CoreBlowPluginServiceContext[] = [];
