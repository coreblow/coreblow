// @ts-nocheck
import { loadConfig, type CoreBlowConfig } from "coreblow/plugin-sdk/config-runtime";
import { resolveMarkdownTableMode } from "coreblow/plugin-sdk/config-runtime";
import { generateSecureUuid } from "coreblow/plugin-sdk/core";
...
import { getChildLogger } from "coreblow/plugin-sdk/text-runtime";
import { redactIdentifier } from "coreblow/plugin-sdk/text-runtime";
import { convertMarkdownTables } from "coreblow/plugin-sdk/text-runtime";
import { markdownToWhatsApp } from "coreblow/plugin-sdk/text-runtime";
import { toWhatsappJid } from "coreblow/plugin-sdk/text-runtime";
...
export async function sendPollWhatsApp(
  to: string,
  poll: PollInput,
  options: { verbose: boolean; accountId?: string; cfg?: CoreBlowConfig },
): Promise<{ messageId: string; toJid: string }> {
  const correlationId = generateSecureUuid();
  const startedAt = Date.now();
  const { listener: active } = requireActiveWebListener(options.accountId);
