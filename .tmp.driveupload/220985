...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: CoreBlowConfig | null = null;
  ...
  get(key: string, cfg: CoreBlowConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: CoreBlowConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: CoreBlowConfig): void {
