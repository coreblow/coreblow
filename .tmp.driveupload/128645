...
import {
  ...
  type DmPolicy,
  type CoreBlowConfig,
} from "coreblow/plugin-sdk/setup";
import type { ChannelSetupWizard } from "coreblow/plugin-sdk/setup";
import { formatCliCommand, formatDocsLink } from "coreblow/plugin-sdk/setup-tools";
...
function mergeWhatsAppConfig(
  cfg: CoreBlowConfig,
  patch: Partial<NonNullable<NonNullable<CoreBlowConfig["channels"]>["whatsapp"]>>,
...
function setWhatsAppDmPolicy(cfg: CoreBlowConfig, dmPolicy: DmPolicy): CoreBlowConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: CoreBlowConfig, allowFrom?: string[]): CoreBlowConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: CoreBlowConfig, selfChatMode: boolean): CoreBlowConfig {
  return mergeWhatsAppConfig(cfg, { selfChatMode });
}

async function detectWhatsAppLinked(cfg: CoreBlowConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so CoreBlow can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppDmAccess(params: {
  ...
  const phoneMode = await params.prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for CoreBlow" },
    ],
  });
...
export const whatsappSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, accountId, forceAllowFrom, prompter, runtime }) => {
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("coreblow channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
