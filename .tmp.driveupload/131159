...
import {
  ...
  resolveStateDir,
  type CoreBlowConfig,
} from "coreblow/plugin-sdk/memory-core-host-runtime-core";
import {
  listMemoryFiles,
  normalizeExtraMemoryPaths,
} from "coreblow/plugin-sdk/memory-core-host-runtime-files";
...
function resolveAgent(cfg: CoreBlowConfig, agent?: string) {
...
function resolveAgentIds(cfg: CoreBlowConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
