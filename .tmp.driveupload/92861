...
function ensureNodePathEnv(): string {
  ensureCoreBlowCliOnPath({ pathEnv: process.env.PATH ?? "" });
...
export async function resolveNodeHostGatewayCredentials(params: {
  ...
  return await resolveGatewayConnectionAuth({
    config: configForResolution as any,
    env: params.env,
...
function buildNodeHostLocalAuthConfig(config: CoreBlowConfig): CoreBlowConfig {
...
export async function runNodeHost(opts: NodeHostRunOptions): Promise<void> {
  ...
  const browserProxyEnabled =
    cfg.nodeHost?.browserProxy?.enabled !== false && (resolvedBrowser as Record<string, unknown>)?.enabled;
