...
const workspaceHarness = createCommandWorkspaceHarness("coreblow-command-plugins-");

async function createClaudeBundlePlugin(params: { workspaceDir: string; pluginId: string }) {
  const pluginDir = path.join(params.workspaceDir, ".coreblow", "extensions", params.pluginId);
...
function buildCfg(): CoreBlowConfig {
  return {
    plugins: {
      enabled: true,
    },
