...
async function withOutsideHardlinkAlias(params: {
  ...
  const outside = await tempDirs.make("coreblow-fs-safe-outside-");
...
async function setupSymlinkWriteRaceFixture(options?: { seedInsideTarget?: boolean }): Promise<{
  ...
  const root = await tempDirs.make("coreblow-fs-safe-root-");
...
describe("fs-safe", () => {
  it("reads a local file safely", async () => {
    const dir = await tempDirs.make("coreblow-fs-safe-");
  ...
  it("copies a file within root safely", async () => {
    const root = await tempDirs.make("coreblow-fs-safe-root-");
    const sourceDir = await tempDirs.make("coreblow-fs-safe-source-");
  ...
  it("writes a file within root from another local source path safely", async () => {
    const root = await tempDirs.make("coreblow-fs-safe-root-");
    const outside = await tempDirs.make("coreblow-fs-safe-src-");
