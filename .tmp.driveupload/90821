 ...
 */
export function getSubagentDepthFromKey(sessionKey: string): number {
    if (!sessionKey) return 0;
    const parts = sessionKey.split(':');
    let depth = 0;
    for (let i = 0; i < parts.length; i++) {
        if (parts[i] === 'sub' || parts[i] === 'subagent') depth++;
    }
    return depth;
}
 ...
 */
export function isSubagentKeyStrictFormat(sessionKey: string): boolean {
    if (!sessionKey) return false;
 ...
 */
export function extractSubagentSegments(sessionKey: string): string[] {
 ...
 */
export function parseSubagentDepthFromSegments(segments: string[]): number {
    let depth = 0;
    for (const seg of segments) {
        if (seg === 'sub' || seg === 'subagent') depth++;
    }
    return depth;
}
...
function buildKeyCandidates(rawKey: string): string[] {
