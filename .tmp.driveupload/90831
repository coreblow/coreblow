/**
 * CoreBlow — Subagent Announce Dispatch (CoreBlow Parity)
 *
 * Dispatch types and multi-phase delivery strategy:
 * queue-primary → direct-primary → queue-fallback
 */
...
export async function runSubagentAnnounceDispatch(params: {
    ...
    const appendPhase = (
        ...
        phases.push({ phase, delivered: result.delivered, path: result.path, error: result.error });
    };
    ...
    if (params.signal?.aborted) {
        return withPhases({ delivered: false, path: 'none' });
    }

    // Non-completion: try queue first, then direct
    ...
    // Completion: try direct first, then queue fallback
    const primaryDirect = await params.direct();
    appendPhase('direct-primary', primaryDirect);
