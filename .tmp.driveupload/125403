...
function normalizeDenyList(cfg: CoreBlowPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: CoreBlowPluginApi["config"]): string[] {
...
export default definePluginEntry({
  ...
  register(api: CoreBlowPluginApi) {
    ...
    const timerService: CoreBlowPluginService = {
