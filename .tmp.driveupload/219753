...
const READ_SCOPE_HEADER = { "x-coreblow-scopes": "operator.read" };
...
async function createSessionStoreFile(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-session-history-"));
  ...
  test("returns session history over direct REST", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      expect(
        (
          body.messages?.[0] as {
            __coreblow?: { id?: string; seq?: number };
          }
        )?.__coreblow,
      ).toMatchObject({
        seq: 1,
  ...
  test("supports cursor pagination over direct REST while preserving the messages field", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      const firstBody = (await firstPage.json()) as {
        sessionKey?: string;
        items?: Array<{ content?: Array<{ text?: string }>; __coreblow?: { seq?: number } }>;
        messages?: Array<{ content?: Array<{ text?: string }>; __coreblow?: { seq?: number } }>;
        nextCursor?: string;
        hasMore?: boolean;
      };
      ...
      expect(firstBody.messages?.map((message) => message.__coreblow?.seq)).toEqual([2, 3]);
      ...
      const secondBody = (await secondPage.json()) as {
        items?: Array<{ content?: Array<{ text?: string }>; __coreblow?: { seq?: number } }>;
        messages?: Array<{ __coreblow?: { seq?: number } }>;
        nextCursor?: string;
        hasMore?: boolean;
      };
      ...
      expect(secondBody.messages?.map((message) => message.__coreblow?.seq)).toEqual([1]);
    ...
    try {
      ...
      const httpHistory = await fetch(
        `http://127.0.0.1:${harness.port}/sessions/${encodeURIComponent("agent:main:main")}/history?limit=1`,
        {
          headers: {
            ...AUTH_HEADER,
            "x-coreblow-scopes": "operator.approvals",
          },
        },
      );
