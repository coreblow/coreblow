...
export function getPreauthHandshakeTimeoutMsFromEnv(env: NodeJS.ProcessEnv = process.env): number {
  const configuredTimeout =
    env.COREBLOW_HANDSHAKE_TIMEOUT_MS || (env.VITEST && env.COREBLOW_TEST_HANDSHAKE_TIMEOUT_MS);
...
// Backward-compat aliases for existing CoreBlow tests
export const HANDSHAKE_TIMEOUT_MS = DEFAULT_PREAUTH_HANDSHAKE_TIMEOUT_MS;

export function resolveHandshakeTimeout(configTimeout?: number): number {
