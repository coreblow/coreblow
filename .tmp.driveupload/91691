 ...
 * Runtime and installation requirement evaluation.
 * Ported from CoreBlow shared/requirements.ts.
 */
...
export type RequirementsEvaluationContext = {
    always: boolean;
...
export type RequirementsEvaluationRemoteContext = {
...
export class RequirementChecker {
    constructor(
        private hasBin: (bin: string) => boolean,
        private hasEnv: (env: string) => boolean,
    ) {}

    checkBinaryRequirements(bins: string[]): RequirementResult[] {
        return bins.map(bin => ({
            ok: this.hasBin(bin),
            name: bin,
            message: this.hasBin(bin) ? undefined : `Missing binary: ${bin}`
        }));
    }

    checkEnvRequirements(envVars: string[]): RequirementResult[] {
        return envVars.map(env => ({
            ok: this.hasEnv(env),
            name: env,
            message: this.hasEnv(env) ? undefined : `Missing env var: ${env}`
        }));
    }
}
