...
type ConfigGroupPolicyWarningCollector<Params extends { cfg: CoreBlowConfig }> = (
  params: Params,
) => string[];
...
export function projectConfigWarningCollector<Params extends { cfg: CoreBlowConfig }>(
  collector: WarningCollector<{ cfg: CoreBlowConfig }>,
): WarningCollector<Params> {
  return projectWarningCollector((params) => ({ cfg: params.cfg }), collector);
}

export function projectConfigAccountIdWarningCollector<
  Params extends { cfg: CoreBlowConfig; accountId?: string | null },
>(
  collector: WarningCollector<{ cfg: CoreBlowConfig; accountId?: string | null }>,
): WarningCollector<Params> {
...
export function projectAccountConfigWarningCollector<
  ResolvedAccount,
  ProjectedCfg,
  Params extends { account: ResolvedAccount; cfg: CoreBlowConfig },
>(
  projectCfg: (cfg: CoreBlowConfig) => ProjectedCfg,
...
export function createAllowlistProviderRestrictSendersWarningCollector<ResolvedAccount>(
  params: {
    providerConfigPresent: (cfg: CoreBlowConfig) => boolean;
  ...
  return createAllowlistProviderGroupPolicyWarningCollector({
    providerConfigPresent: params.providerConfigPresent,
    resolveGroupPolicy: ({ account }: { account: ResolvedAccount; cfg: CoreBlowConfig }) =>
...
export function createAllowlistProviderGroupPolicyWarningCollector<
  Params extends { cfg: CoreBlowConfig },
>(params: {
  providerConfigPresent: (cfg: CoreBlowConfig) => boolean;
