...
export function normalizeCompatibilityConfigValues(cfg: CoreBlowConfig): {
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as CoreBlowConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as CoreBlowConfig["browser"],
    };
    changes.push(
  ...
  const normalizeLegacyNanoBananaSkill = () => {
    type ModelProviderEntry = Partial<
      NonNullable<NonNullable<CoreBlowConfig["models"]>["providers"]>[string]
    >;
    type ModelsConfigPatch = Partial<NonNullable<CoreBlowConfig["models"]>>;
    ...
    if (!hasGoogleApiKey && legacyApiKey) {
      ...
      rawModels.providers = rawProviders as NonNullable<CoreBlowConfig["models"]>["providers"];
      next = {
        ...next,
        models: rawModels as CoreBlowConfig["models"],
      };
      changes.push(
  ...
  const normalizeLegacyTalkConfig = () => {
    ...
    const normalizedTalk = normalizeTalkSection(rawTalk as CoreBlowConfig["talk"]);
    if (!normalizedTalk) {
      return;
    }
  ...
  const normalizeLegacyMediaProviderOptions = () => {
    ...
    next = {
      ...
      tools: {
        ...next.tools,
        media: nextMedia as NonNullable<CoreBlowConfig["tools"]>["media"],
      },
    };
  };
  ...
  const normalizeLegacyMistralModelMaxTokens = () => {
    ...
    next = {
      ...
      models: {
        ...next.models,
        providers: nextProviders as NonNullable<CoreBlowConfig["models"]>["providers"],
      },
    };
  };
