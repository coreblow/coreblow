...
} from "coreblow/plugin-sdk/channel-send-result";
import { createStaticReplyToModeResolver } from "coreblow/plugin-sdk/conversation-runtime";
import { createChatChannelPlugin } from "coreblow/plugin-sdk/core";
import { buildPassiveProbedChannelStatusSummary } from "coreblow/plugin-sdk/extension-shared";
import {
...
function resolveZalouserOutboundChunkMode(cfg: CoreBlowConfig, accountId?: string) {
  return getZalouserRuntime().channel.text.resolveChunkMode(cfg, "zalouser", accountId);
}

function resolveZalouserOutboundTextChunkLimit(cfg: CoreBlowConfig, accountId?: string) {
