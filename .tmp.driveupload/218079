...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("coreblow-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("coreblow-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("coreblow-sbx-");
