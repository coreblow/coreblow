...
import { markCoreBlowExecEnv } from "../../infra/coreblow-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "coreblow.sandbox=1");
  args.push("--label", `coreblow.sessionKey=${params.scopeKey}`);
  args.push("--label", `coreblow.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `coreblow.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markCoreBlowExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "coreblow.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`coreblow sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`coreblow sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("coreblow sandbox recreate --all");
}
