...
function renderDiffCard(payload: DiffViewerPayload): string {
  return `<section class="oc-diff-card">
    <diffs-container class="oc-diff-host" data-coreblow-diff-host>
      <template shadowrootmode="open">${payload.prerenderedHTML}</template>
    </diffs-container>
    <script type="application/json" data-coreblow-diff-payload>${escapeJsonScript(payload)}</script>
  </section>`;
}
...
async function renderPatchDiff(
  ...
  const files = parsePatchFiles(input.patch).flatMap((entry: any) => entry.files ?? []);
  if (files.length === 0) {
  ...
  const totalLines = files.reduce((sum: any, fileDiff: any) => {
