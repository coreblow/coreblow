import type { CoreBlowConfig, ProviderAuthResult } from "coreblow/plugin-sdk/provider-auth";
import { readClaudeCliCredentialsCached } from "coreblow/plugin-sdk/provider-auth";

const DEFAULT_CLAUDE_CLI_MODEL = "claude-cli/claude-sonnet-4-6";
type AgentDefaultsModel = NonNullable<NonNullable<CoreBlowConfig["agents"]>["defaults"]>["model"];
type AgentDefaultsModels = NonNullable<NonNullable<CoreBlowConfig["agents"]>["defaults"]>["models"];
...
export function buildAnthropicCliMigrationResult(config: CoreBlowConfig): ProviderAuthResult {
