...
function createPerplexityConfig(apiKey: string, enabled?: boolean): CoreBlowConfig {
  return {
    tools: {
      web: {
        search: {
...
function pluginWebSearchApiKey(config: CoreBlowConfig, pluginId: string): unknown {
  const entry = (
...
function createDisabledFirecrawlConfig(apiKey?: string): CoreBlowConfig {
  return {
    tools: {
      web: {
        search: {
...
function readFirecrawlPluginApiKey(config: CoreBlowConfig): string | undefined {
...
async function runQuickstartPerplexitySetup(
  apiKey: string,
  enabled?: boolean,
): Promise<{ result: CoreBlowConfig; prompter: WizardPrompter }> {
  const cfg = createPerplexityConfig(apiKey, enabled);
