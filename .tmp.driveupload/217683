...
type CoreToolDefinition = {
  ...
  includeInCoreBlowGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const coreblowTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInCoreBlowGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:coreblow": coreblowTools,
