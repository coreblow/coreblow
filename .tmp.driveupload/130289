...
import { createChannelPluginBase } from "coreblow/plugin-sdk/core";
import {
  formatDocsLink,
...
import { getChatChannelMeta, type ChannelPlugin, type CoreBlowConfig } from "./runtime-api.js";

export const SLACK_CHANNEL = "slack" as const;

function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "CoreBlow";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for CoreBlow`,
    },
    features: {
      bot_user: {
      ...
      slash_commands: [
        {
          command: "/coreblow",
          description: "Send a message to CoreBlow",
          should_escape: false,
        },
      ],
    },
...
export function buildSlackSetupLines(botName = "CoreBlow"): string[] {
  return [
