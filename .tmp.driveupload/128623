...
import { info, success } from "coreblow/plugin-sdk/runtime-env";
import { getChildLogger } from "coreblow/plugin-sdk/runtime-env";
import { defaultRuntime, type RuntimeEnv } from "coreblow/plugin-sdk/runtime-env";
import { resolveOAuthDir } from "coreblow/plugin-sdk/state-paths";
import type { WebChannel } from "coreblow/plugin-sdk/text-runtime";
import { resolveUserPath } from "coreblow/plugin-sdk/text-runtime";
...
export async function pickWebChannel(
  ...
  if (!hasWeb) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("coreblow channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
