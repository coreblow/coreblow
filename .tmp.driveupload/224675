...
type PluginLoadConfig = NonNullable<Parameters<typeof loadCoreBlowPlugins>[0]>["config"];
type PluginRegistry = ReturnType<typeof loadCoreBlowPlugins>;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "coreblow.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function writeWorkspacePlugin(params: {
  ...
  const workspacePluginDir = path.join(workspaceDir, ".coreblow", "extensions", params.id);
      ...
      JSON.stringify(
        {
          ...
          coreblow: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadCoreBlowPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
function loadRegistryFromAllowedPlugins(
  plugins: TempPlugin[],
  options?: Omit<Parameters<typeof loadCoreBlowPlugins>[0], "cache" | "config">,
) {
  return loadCoreBlowPlugins({
    cache: false,
    ...options,
...
function expectNoUnwiredBundleDiagnostic(
  registry: ReturnType<typeof loadCoreBlowPlugins>,
  pluginId: string,
) {
  expect(
    registry.diagnostics.some(
...
function expectCachePartitionByPluginSource(params: {
  pluginId: string;
  loadFirst: () => ReturnType<typeof loadCoreBlowPlugins>;
  loadSecond: () => ReturnType<typeof loadCoreBlowPlugins>;
...
function expectCacheMissThenHit(params: {
  loadFirst: () => ReturnType<typeof loadCoreBlowPlugins>;
  loadVariant: () => ReturnType<typeof loadCoreBlowPlugins>;
}) {
  const first = params.loadFirst();
...
function createEnvResolvedPluginFixture(pluginId: string) {
  ...
  const stateDir = makeTempDir();
  const pluginDir = path.join(coreblowHome, "plugins", pluginId);
  mkdirSafe(pluginDir);
  const plugin = writePlugin({
    id: pluginId,
    dir: pluginDir,
...
describe("bundle plugins", () => {
  it("reports Codex bundles as loaded bundle plugins without importing runtime code", () => {
    ...
    const workspaceDir = makeTempDir();
    const stateDir = makeTempDir();
    const bundleRoot = path.join(workspaceDir, ".coreblow", "extensions", "sample-bundle");
  ...
  it.each([
    {
      ...
      assert: (registry: ReturnType<typeof loadCoreBlowPlugins>) => {
...
module.exports = { id: "skipped-scoped-only", register() { throw new Error("skipped plugin should not load"); } };`,
    ...
    },
    {
      ...
      run: () => {
        ...
        const full = loadCoreBlowPlugins(options);
        const scoped = loadCoreBlowPlugins({
          ...options,
          onlyPluginIds: ["allowed-cache-scope"],
        });
        const scopedAgain = loadCoreBlowPlugins({
          ...options,
          onlyPluginIds: ["allowed-cache-scope"],
        });
  ...
  it("only publishes plugin commands to the global registry during activating loads", async () => {
    ...
    const active = loadCoreBlowPlugins({
      cache: false,
      workspaceDir: plugin.dir,
      config: {
        plugins: {
  ...
  it("throws when activate:false is used without cache:false", () => {
    expect(() => loadCoreBlowPlugins({ activate: false })).toThrow(
      "activate:false requires cache:false",
    );
    expect(() => loadCoreBlowPlugins({ activate: false, cache: true })).toThrow(
      "activate:false requires cache:false",
    );
  });
  ...
  it("re-initializes global hook runner when serving registry from cache", () => {
    ...
    const first = loadCoreBlowPlugins(options);
    expect(getGlobalHookRunner()).not.toBeNull();

    resetGlobalHookRunner();
    expect(getGlobalHookRunner()).toBeNull();

    const second = loadCoreBlowPlugins(options);
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const ignoredHome = makeTempDir();
        const stateDir = makeTempDir();
        const pluginDir = path.join(coreblowHome, "plugins", "tracked-install-cache");
        mkdirSafe(pluginDir);
        const plugin = writePlugin({
    ...
    },
    {
      ...
      setup: () => {
        ...
        return {
          loadFirst: () => loadCoreBlowPlugins(options),
          loadVariant: () =>
            loadCoreBlowPlugins({
              ...options,
              runtimeOptions: {
  ...
  it("prefers OPENCLAW_HOME over HOME for env-expanded load paths", () => {
    ...
    const bundledDir = makeTempDir();
    const plugin = writePlugin({
      id: "coreblow-home-demo",
      dir: path.join(coreblowHome, "plugins", "coreblow-home-demo"),
      filename: "index.cjs",
      body: `module.exports = { id: "coreblow-home-demo", register() {} };`,
    });
    ...
    const registry = loadCoreBlowPlugins({
      ...
      config: {
        plugins: {
          allow: ["coreblow-home-demo"],
          entries: {
            "coreblow-home-demo": { enabled: true },
          },
          load: {
            paths: ["~/plugins/coreblow-home-demo"],
          },
        },
      },
    });
    ...
    expect(
      fs.realpathSync(
        registry.plugins.find((entry) => entry.id === "coreblow-home-demo")?.source ?? "",
      ),
    ).toBe(fs.realpathSync(plugin.file));
  });
  ...
  api.registerHook("gateway:startup", () => {}, { name: "shared-hook" });
        ...
        selectCount: (registry: ReturnType<typeof loadCoreBlowPlugins>) =>
  ...
  api.registerCli(() => {}, { commands: ["shared-cli"] });
        ...
        assertPrimaryOwner: (registry: ReturnType<typeof loadCoreBlowPlugins>) => {
...
module.exports = {
  ...
  register(api) {
    ...
    fs.writeFileSync(
      path.join(plugin.dir, "coreblow.plugin.json"),
      JSON.stringify(
        {
          id: "lazy-channel",
          configSchema: EMPTY_PLUGIN_SCHEMA,
    ...
    const setupRegistry = loadCoreBlowPlugins({
      cache: false,
      config,
  ...
  it.each([
    {
      ...
      fixture: {
        ...
        packageName: "@coreblow/setup-entry-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@coreblow/setup-runtime-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@coreblow/setup-runtime-preferred-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@coreblow/setup-runtime-not-preferred-test",
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          fs.writeFileSync(
            path.join(memoryADir, "coreblow.plugin.json"),
            JSON.stringify(
              {
                id: "memory-a",
                kind: "memory",
          ...
          fs.writeFileSync(
            path.join(memoryBDir, "coreblow.plugin.json"),
            JSON.stringify(
              {
                id: "memory-b",
                kind: "memory",
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", async () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("coreblow/plugin-sdk").emptyPluginConfigSchema)(),
        register() {},
      };`,
    });
  ...
  it("supports legacy plugins subscribing to diagnostic events from the root sdk", async () => {
    useNoBundledPlugins();
    const seenKey = "__coreblowLegacyRootDiagnosticSeen";
  ...
  configSchema: (require("coreblow/plugin-sdk").emptyPluginConfigSchema)(),
  register() {
    const { onDiagnosticEvent } = require("coreblow/plugin-sdk");
