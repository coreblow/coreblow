...
export class PluginCommands {
    ...
    private list(flags: Record<string, string | boolean>): PluginCommandResult {
        ...
        if (flags.json) {
            return {
                output: JSON.stringify(plugins.map((p: any) => ({
                    id: p.id,
                    name: p.name,
                    status: p.status,
                    version: p.version,
    ...
    private enable(args: string[]): PluginCommandResult {
        ...
        const plugin = this.context.registryData!.plugins.find((p: any) => p.id === pluginId);
        if (!plugin) {
            return { output: '', error: `Plugin not found: ${pluginId}`, success: false };
        }
    ...
    private info(args: string[]): PluginCommandResult {
        ...
        const installed = this.context.registryData!.plugins.find((p: any) => p.id === pluginId);
        if (installed) {
            const lines = [
    ...
    private doctor(): PluginCommandResult {
        ...
        const warnings = diagnostics.filter((d: any) => d.level === 'warn');

        const lines = [
            '🩺 Plugin Doctor',
            '',
            `  Plugins: ${plugins.length} total, ${plugins.filter((p: any) => p.status === 'loaded').length} loaded`,
