...
function expandHomePrefix(raw: string) { return raw.startsWith("~") ? raw.replace(/^~/, os.homedir()) : raw; }
function resolveConfigDir() { return process.env.COREBLOW_CONFIG_DIR || path.join(os.homedir(), ".coreblow"); }
function parseJsonWithJson5Fallback(raw: string) { return JSON.parse(raw); }
...
async function setSecureFileMode(filePath: string): Promise<void> {
  try { await fs.promises.chmod(filePath, 0o600) } catch (e) {}
}
...
export async function saveCronStore(
  ...
  try { await fs.promises.chmod(storeDir, 0o700) } catch (e) {}
