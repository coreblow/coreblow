...
function collectModelRefs(cfg: CoreBlowConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: CoreBlowConfig, providerId: string): boolean {
...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.coreblow) || !isRecord(entry.coreblow.channel)) {
      continue;
    }
    const channel = entry.coreblow.channel;
...
function collectCandidateChannelIds(cfg: CoreBlowConfig): string[] {
...
function configMayNeedPluginManifestRegistry(cfg: CoreBlowConfig): boolean {
  const configuredChannels = cfg.channels as Record<string, unknown> | undefined;
  if (!configuredChannels || typeof configuredChannels !== "object") {
    return false;
  }
...
function isPluginExplicitlyDisabled(cfg: CoreBlowConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: CoreBlowConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: CoreBlowConfig, pluginId: string): CoreBlowConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
