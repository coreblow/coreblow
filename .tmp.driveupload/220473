...
function withFakeRuntimeBin<T>(params: { binName: string; run: () => T }): T {
  return withFakeRuntimeBins({
    ...
    tmpPrefix: `coreblow-${params.binName}-bin-`,
    run: params.run,
  });
}
...
function withFakeRuntimeBins<T>(params: {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), params.tmpPrefix ?? "coreblow-runtime-bins-"));
...
const unsafeRuntimeInvocationCases: UnsafeRuntimeInvocationCase[] = [
  ...
  },
  {
    ...
    binName: "ruby",
    tmpPrefix: "coreblow-ruby-require-",
  ...
  },
  {
    ...
    binName: "ruby",
    tmpPrefix: "coreblow-ruby-load-path-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "coreblow-perl-module-preload-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "coreblow-perl-load-path-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "coreblow-perl-preload-load-path-",
  ...
  },
  {
    ...
    binName: "node",
    tmpPrefix: "coreblow-inline-shell-node-",
...
describe("hardenApprovedExecutionPaths", () => {
  ...
  it.runIf(process.platform !== "win32").each(cases)("$name", (testCase) => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "coreblow-approval-hardening-"));
    const oldPath = process.env.PATH;
      ...
      withFakeRuntimeBins({
        ...
        run: () => {
          withScriptOperandPlanFixture(
            {
              tmpPrefix: "coreblow-approval-script-plan-",
              fixture: runtimeCase,
  ...
  it("captures the real shell script operand after value-taking shell flags", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "coreblow-shell-option-value-"));
    try {
