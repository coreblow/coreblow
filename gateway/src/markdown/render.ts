...
export function renderMarkdownWithMarkers(ir: MarkdownIR, options: RenderOptions): string {
    ...
    for (const s of styled) { boundaries.add(s.start); boundaries.add(s.end); }
...
const DISCORD_MARKERS: RenderStyleMap = {
    bold: { open: '**', close: '**' },
    italic: { open: '*', close: '*' },
...
const TELEGRAM_MARKERS: RenderStyleMap = {
    bold: { open: '<b>', close: '</b>' },
...
const PLAIN_MARKERS: RenderStyleMap = {};
...
export function renderForPlainText(ir: MarkdownIR): string {
 ...
 */
export function renderForChannel(ir: MarkdownIR, channel: string): string {
    switch (channel) {
