...
export function sampleString(str: string, maxLen: number): string {
    if (!str || str.length <= maxLen) return str;
    return str.slice(0, Math.max(0, maxLen - 3)) + "...";
}

export function sampleLines(str: string, maxLines: number): string {
    if (!str) return str;
