 ...
 * AI request error formatting and retry logic.
 * Ported from CoreBlow shared/assistant-error-format.ts.
 */
...
export function isRetryableError(error: unknown): boolean {
    const info = parseApiErrorInfo(error instanceof Error ? error.message : String(error));
    if (!info?.httpCode) return false;
    
    const code = Number(info.httpCode);
    // Standard retryable HTTP codes: 408 (Timeout), 429 (Rate Limit), 500+ (Server Errors)
