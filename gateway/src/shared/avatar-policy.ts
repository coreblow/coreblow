 ...
 * Avatar resolution and validation.
 * Ported from CoreBlow shared/avatar-policy.ts.
 */
...
export function isAvatarUrlValid(url: string): boolean {
    if (!url) return false;
    return isAvatarHttpUrl(url) || isAvatarImageDataUrl(url);
}

export function resolveAvatarUrl(config: { avatar?: string; defaultAvatar?: string }): string | undefined {
    if (config.avatar && isAvatarUrlValid(config.avatar)) {
        return config.avatar;
    }
    if (config.defaultAvatar && isAvatarUrlValid(config.defaultAvatar)) {
