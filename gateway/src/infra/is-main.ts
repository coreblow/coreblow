/**
 * CoreBlow — Main Module Detection
 *
 * Detects whether the current file is being run as the main entry
 * point (vs imported as a dependency). Prevents double-boot.
 *
...
export type IsMainModuleOptions = {
  currentFile: string;
  argv?: string[];
...
/**
 * Returns true if the current file is the main module entry point.
 * Handles PM2 environments and wrapper→entry launcher mappings
 * (e.g. coreblow.mjs → entry.js).
 */
