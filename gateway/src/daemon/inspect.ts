 ...
 * Ported from CoreBlow daemon/inspect.ts.
 */
...
export type ExtraGatewayService = {
    ...
    detail: string;
    scope: "user" | "system";
    marker?: "coreblow" | "coreblow" | "clawdbot" | "moltbot";
    legacy?: boolean;
};
...
const EXTRA_MARKERS = ["coreblow", "coreblow", "clawdbot", "moltbot"] as const;
...
function hasGatewayServiceMarker(content: string): boolean {
    const lower = content.toLowerCase();
    const markerKeys = ["coreblow_service_marker", "coreblow_service_marker"];
    const kindKeys = ["coreblow_service_kind", "coreblow_service_kind"];
...
function isCoreBlowGatewayLaunchdService(label: string, contents: string): boolean {
    ...
    return label.startsWith("ai.coreblow.") || label.startsWith("ai.coreblow.");
}

function isCoreBlowGatewaySystemdService(name: string, contents: string): boolean {
    if (hasGatewayServiceMarker(contents)) {
        return true;
    }
    if (!name.startsWith("coreblow-gateway") && !name.startsWith("coreblow-gateway")) {
        return false;
    }
...
function isCoreBlowGatewayTaskName(name: string): boolean {
    ...
    return normalized === defaultName || normalized.startsWith("coreblow gateway") || normalized.startsWith("coreblow gateway");
}
...
function isLegacyLabel(label: string): boolean {
    ...
    return lower.includes("coreblow") || lower.includes("clawdbot") || lower.includes("moltbot");
}
    ...
    for (const { name: labelFromName, fullPath, contents } of candidates) {
        ...
        if (!marker) {
            ...
            results.push({
                ...
                scope: params.scope,
                marker: label.includes("coreblow") ? "coreblow" : (isLegacyLabel(label) ? "clawdbot" : "moltbot"),
                legacy: true,
            });
            continue;
        }
        ...
        if (marker === "coreblow" && isCoreBlowGatewayLaunchdService(label, contents)) {
            continue;
        }
        results.push({
            platform: "darwin",
...
async function scanSystemdDir(params: {
    ...
    for (const { entry, name, fullPath, contents } of candidates) {
        ...
        if (marker === "coreblow" && isCoreBlowGatewaySystemdService(name, contents)) {
            continue;
        }
        results.push({
            platform: "linux",
...
export async function findExtraGatewayServices(
    ...
    if (process.platform === "darwin") {
        try {
            ...
            for (const svc of await scanLaunchdDir({ dir: userDir, scope: "user" })) push(svc);
    ...
    if (process.platform === "linux") {
        try {
            ...
            for (const svc of await scanSystemdDir({ dir: userDir, scope: "user" })) push(svc);
            ...
            if (opts.deep) {
                for (const dir of [
                    ...
                    for (const svc of await scanSystemdDir({ dir, scope: "system" })) push(svc);
                }
            }
        } catch {
            return results;
    ...
    if (process.platform === "win32") {
        ...
        for (const task of tasks) {
            ...
            if (isCoreBlowGatewayTaskName(name)) continue;
