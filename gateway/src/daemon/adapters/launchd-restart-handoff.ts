/**
 * src/daemon/adapters/launchd-restart-handoff.ts
 * launchd detached restart handoff script generation and execution.
 * Ported from CoreBlow daemon/launchd-restart-handoff.ts.
 */
...
function resolveLaunchAgentLabel(env?: Record<string, string | undefined>): string {
    const envLabel = env?.COREBLOW_LAUNCHD_LABEL?.trim() ?? env?.COREBLOW_LAUNCHD_LABEL?.trim();
    if (envLabel) {
        return envLabel;
    }
...
export function isCurrentProcessLaunchdServiceLabel(
    ...
    const configuredLabel = env.COREBLOW_LAUNCHD_LABEL?.trim() ?? env.COREBLOW_LAUNCHD_LABEL?.trim();
    ...
    try {
        const child = spawn(
            ...
            [
                ...
                "coreblow-launchd-restart-handoff",
