/**
 * src/daemon/adapters/launchd.ts
 * Adapter for managing macOS launchd daemon.
 * Ported from CoreBlow daemon/launchd.ts.
 */
...
function resolveLaunchAgentLabel(args?: { env?: Record<string, string | undefined> }): string {
    const envLabel = args?.env?.COREBLOW_LAUNCHD_LABEL?.trim() ?? args?.env?.COREBLOW_LAUNCHD_LABEL?.trim();
    if (envLabel) {
        return envLabel;
    }
    const profile = args?.env?.COREBLOW_PROFILE ?? args?.env?.COREBLOW_PROFILE;
...
export function resolveGatewayLogPaths(env: GatewayServiceEnv): {
    ...
    const prefixRaw = env.COREBLOW_LOG_PREFIX ?? env.COREBLOW_LOG_PREFIX;
    const prefix = prefixRaw?.trim() || "gateway";
    return {
        logDir,
...
async function resolveLaunchAgentGatewayPort(env: GatewayServiceEnv): Promise<number | null> {
    ...
    const fromEnv = parseStrictPositiveInteger(env.COREBLOW_GATEWAY_PORT ?? env.COREBLOW_GATEWAY_PORT ?? "");
    return fromEnv ?? null;
}
...
export function parseLaunchctlPrint(output: string): LaunchctlPrintInfo {
    ...
    if (entries.state) info.state = entries.state;
    if (entries.pid) {
        const pid = parseStrictPositiveInteger(entries.pid);
        if (pid !== undefined) info.pid = pid;
    }
    if (entries["last exit status"]) {
        const status = parseStrictInteger(entries["last exit status"]);
        if (status !== undefined) info.lastExitStatus = status;
    }
    if (entries["last exit reason"]) {
        info.lastExitReason = entries["last exit reason"];
    }
    return info;
}
...
async function activateLaunchAgent(params: { env: GatewayServiceEnv; plistPath: string }) {
    ...
    await bootstrapLaunchAgentOrThrow({
        ...
        actionHint: "coreblow gateway install --force",
    });
}
...
export async function restartLaunchAgent({
    ...
    await bootstrapLaunchAgentOrThrow({
        ...
        serviceTarget,
        plistPath,
        actionHint: "coreblow gateway restart",
    });
