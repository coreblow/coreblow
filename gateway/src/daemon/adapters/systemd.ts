/**
 * src/daemon/adapters/systemd.ts
 * Adapter for managing systemd on Linux.
 * Ported from CoreBlow daemon/systemd.ts.
 */
...
function resolveSystemdServiceName(env: GatewayServiceEnv): string {
    const override = env.COREBLOW_SYSTEMD_UNIT?.trim() ?? env.COREBLOW_SYSTEMD_UNIT?.trim();
    if (override) {
...
export function parseSystemdShow(output: string): SystemdServiceInfo {
    ...
    if (mainPidValue) {
        ...
        if (pid !== undefined) info.mainPid = pid;
    }
    ...
    if (execMainStatusValue) {
        ...
        if (status !== undefined) info.execMainStatus = status;
    }
...
export async function stageSystemdService({
    ...
    writeFormattedLines(
        stdout,
        [
            { label: "Staged systemd service", value: unitPath },
            ...(backedUp
                ? [{ label: "Previous unit backed up to", value: `${unitPath}.bak` }]
                : []),
        ],
...
async function activateSystemdService(params: { env: GatewayServiceEnv }) {
    const profile = params.env.COREBLOW_PROFILE ?? params.env.COREBLOW_PROFILE;
