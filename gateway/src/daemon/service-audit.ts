 ...
 * Ported from CoreBlow daemon/service-audit.ts.
 */

import fs from "node:fs/promises";
import path from "node:path";
import { resolveLaunchAgentPlistPath } from "./adapters/launchd.js";
...
import { resolveSystemdUserUnitPath } from "./adapters/systemd.js";
...
function auditGatewayToken(
    ...
    issues.push({
        ...
        message: "Gateway service embeds COREBLOW_GATEWAY_TOKEN and should be reinstalled.",
        detail: "Run `coreblow gateway install --force` to remove embedded service token.",
        level: "recommended",
    });
    ...
    issues.push({
        ...
        message:
            "Gateway service COREBLOW_GATEWAY_TOKEN does not match gateway.auth.token in config",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
    ...
    // CoreBlow first, then fallback to CoreBlow
    if (command.environmentValueSources?.COREBLOW_GATEWAY_TOKEN === "file" || 
        (command.environmentValueSources?.COREBLOW_GATEWAY_TOKEN as any) === "file") {
        return undefined;
    }
    const token = command.environment?.COREBLOW_GATEWAY_TOKEN?.trim() ?? 
                  command.environment?.COREBLOW_GATEWAY_TOKEN?.trim();
...
async function auditGatewayRuntime(
    ...
    if (isBunRuntime(execPath)) {
        issues.push({
            ...
            message: "Gateway service uses Bun; Bun is incompatible with typical deployment scenarios.",
            detail: execPath,
            level: "recommended",
        });
        return;
...
export function checkTokenDrift(params: {
    ...
    if (configToken && serviceToken !== configToken) {
        return {
            ...
            detail: "Run `coreblow gateway install --force` to sync the token.",
            level: "recommended",
        };
    }
