...
export function chunkMessage(text: string, platform: string = 'default'): string[] {
...
function findSplitPoint(text: string, maxLen: number): number {
    ...
    if (paraBreak > maxLen * 0.5) return paraBreak + 2;

    // 2. Try single newline
    const lineBreak = text.lastIndexOf('\n', maxLen);
    if (lineBreak > maxLen * 0.5) return lineBreak + 1;
...
export function chunkMessageSmart(text: string, platform: string = 'default'): string[] {
    const maxLen = PLATFORM_LIMITS[platform] ?? PLATFORM_LIMITS.default;
    if (text.length <= maxLen) return [text];
