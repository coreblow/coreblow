...
type CronConfig = any;
function parseByteSize(val: string): number { return parseInt(val, 10) || 5 * 1024 * 1024; }
...
export async function readCronRunLogEntries(
  ...
  return page.entries .slice().reverse();
}
...
export async function readCronRunLogEntriesPage(
  ...
  const sorted =
    sortDir === "asc"
      ? filtered.slice().sort((a, b) => a.ts - b.ts)
      : filtered.slice().sort((a, b) => b.ts - a.ts);
