...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `coreblow-e2e-${name}-`));
  const configDir = path.join(homeDir, ".coreblow");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          COREBLOW_SKIP_PROVIDERS: "1",
          COREBLOW_SKIP_GMAIL_WATCHER: "1",
          COREBLOW_SKIP_CRON: "1",
          COREBLOW_SKIP_BROWSER_CONTROL_SERVER: "1",
          COREBLOW_SKIP_CANVAS_HOST: "1",
          COREBLOW_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
