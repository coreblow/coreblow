...
function installDiscoveryHooks(state: DiscoveryState) {
  beforeEach(async () => {
    vi.resetModules();
    vi.doMock("coreblow/plugin-sdk/agent-runtime", async () => {
    ...
    vi.doMock("coreblow/plugin-sdk/provider-auth", async () => {
      const actual = await vi.importActual<object>("coreblow/plugin-sdk/provider-auth");
      return {
        ...actual,
        ensureAuthProfileStore: ensureAuthProfileStoreMock,
        listProfilesForProvider: listProfilesForProviderMock,
    ...
    vi.doMock("coreblow/plugin-sdk/provider-setup", async () => {
      const actual = await vi.importActual<object>("coreblow/plugin-sdk/provider-setup");
      return {
        ...actual,
    ...
    vi.doMock("coreblow/plugin-sdk/self-hosted-provider-setup", async () => {
      const actual = await vi.importActual<object>(
        "coreblow/plugin-sdk/self-hosted-provider-setup",
      );
      return {
        ...actual,
        buildVllmProvider: (...args: unknown[]) => buildVllmProviderMock(...args),
    ...
    it("keeps self-hosted discovery provider-owned", async () => {
      ...
      await expect(
        runCatalog(state, {
          ...
          config: {},
          env: {
            VLLM_API_KEY: "env-vllm-key", // pragma: allowlist secret
          } as NodeJS.ProcessEnv,
          resolveProviderApiKey: () => ({
            apiKey: "VLLM_API_KEY", // pragma: allowlist secret
            discoveryApiKey: "env-vllm-key", // pragma: allowlist secret
          }),
          resolveProviderAuth: () => ({
            apiKey: "VLLM_API_KEY", // pragma: allowlist secret
            discoveryApiKey: "env-vllm-key", // pragma: allowlist secret
      ...
      expect(buildVllmProviderMock).toHaveBeenCalledWith({
        apiKey: "env-vllm-key", // pragma: allowlist secret
      });
    });
  });
}
    ...
    it("keeps self-hosted discovery provider-owned", async () => {
      ...
      await expect(
        runCatalog(state, {
          ...
          config: {},
          env: {
            SGLANG_API_KEY: "env-sglang-key", // pragma: allowlist secret
          } as NodeJS.ProcessEnv,
          resolveProviderApiKey: () => ({
            apiKey: "SGLANG_API_KEY", // pragma: allowlist secret
            discoveryApiKey: "env-sglang-key", // pragma: allowlist secret
          }),
          resolveProviderAuth: () => ({
            apiKey: "SGLANG_API_KEY", // pragma: allowlist secret
            discoveryApiKey: "env-sglang-key", // pragma: allowlist secret
      ...
      expect(buildSglangProviderMock).toHaveBeenCalledWith({
        apiKey: "env-sglang-key", // pragma: allowlist secret
      });
    });
  });
}
    ...
    it("keeps API catalog provider-owned", async () => {
      await expect(
        state.runProviderCatalog({
          ...
          config: {},
          env: {
            MINIMAX_API_KEY: "minimax-key", // pragma: allowlist secret
          } as NodeJS.ProcessEnv,
          resolveProviderApiKey: () => ({ apiKey: "minimax-key" }), // pragma: allowlist secret
          resolveProviderAuth: () => ({
            apiKey: "minimax-key", // pragma: allowlist secret
            discoveryApiKey: undefined,
            mode: "api_key",
            source: "env",
          }),
      ...
      await expect(
        runCatalog(state, {
          ...
          resolveProviderAuth: () => ({
            ...
            discoveryApiKey: "access-token", // pragma: allowlist secret
            mode: "oauth",
            source: "profile",
    ...
    it("keeps catalog provider-owned", async () => {
      await expect(
        state.runProviderCatalog({
          ...
          env: {
            MODELSTUDIO_API_KEY: "modelstudio-key", // pragma: allowlist secret
          } as NodeJS.ProcessEnv,
          resolveProviderApiKey: () => ({ apiKey: "modelstudio-key" }), // pragma: allowlist secret
          resolveProviderAuth: () => ({
            apiKey: "modelstudio-key", // pragma: allowlist secret
            discoveryApiKey: undefined,
            mode: "api_key",
            source: "env",
          }),
    ...
    it("keeps env-managed catalog provider-owned", async () => {
      ...
      await expect(
        runCatalog(state, {
          ...
          config: {},
          env: {
            CLOUDFLARE_AI_GATEWAY_API_KEY: "secret-value", // pragma: allowlist secret
          } as NodeJS.ProcessEnv,
          resolveProviderApiKey: () => ({ apiKey: undefined }),
          resolveProviderAuth: () => ({
            apiKey: undefined,
        ...
        provider: {
          ...
          api: "anthropic-messages",
          apiKey: "CLOUDFLARE_AI_GATEWAY_API_KEY", // pragma: allowlist secret
