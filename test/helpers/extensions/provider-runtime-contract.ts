...
export function describeAnthropicProviderRuntimeContract() {
  describe("anthropic provider runtime contract", { timeout: CONTRACT_SETUP_TIMEOUT_MS }, () => {
    ...
    it("owns auth doctor hint generation", () => {
      ...
      expect(hint).toContain("coreblow doctor --yes");
    });
    ...
    it("owns usage auth resolution", async () => {
      ...
      await expect(
        provider.resolveUsageAuth?.({
          config: {} as never,
          env: {
            ZAI_API_KEY: "env-zai-token", // pragma: allowlist secret
          } as NodeJS.ProcessEnv,
          provider: "zai",
          resolveApiKeyFromConfigAndStore: () => "env-zai-token", // pragma: allowlist secret
          resolveOAuthToken: async () => null,
        }),
      ).resolves.toEqual({
        token: "env-zai-token", // pragma: allowlist secret
      });
    });

    it("falls back to legacy pi auth tokens for usage auth", async () => {
      const provider = requireProviderContractProvider("zai");
      const home = await fs.mkdtemp(path.join(os.tmpdir(), "coreblow-zai-contract-"));
