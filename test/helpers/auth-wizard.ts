...
export function requireCoreBlowAgentDir(): string {
  const agentDir = process.env.COREBLOW_AGENT_DIR;
  if (!agentDir) {
    throw new Error("COREBLOW_AGENT_DIR not set");
  }
  return agentDir;
}
